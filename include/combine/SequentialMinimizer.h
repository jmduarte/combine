...
namespace cmsmath {
  ...
  public:
    ...
    OneDimMinimizer(const MinimizerContext &ctx, unsigned int idx) : f_(&ctx), idx_(idx) {}
    OneDimMinimizer(
        const MinimizerContext &ctx, unsigned int idx, double xmin, double xmax, double xstep, const std::string &name)
    ...
    void init(const MinimizerContext &ctx,
              unsigned int idx,
              double xmin,
              double xmax,
              double xstep,
  ...
  private:
    ...
    inline void assign(int to, int from) {
      xi_[to] = xi_[from];
      yi_[to] = yi_[from];
    }
  };
  ...
  public:
    ...
    virtual bool SetLimitedVariable(
