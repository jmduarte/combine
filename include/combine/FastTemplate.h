...
public:
  ...
  FastHistoAxis_t(const TAxis &axis) {
    ...
    for (int ix = 0; ix <= nbins; ++ix)
      binEdges_.push_back(U(axis.GetBinLowEdge(ix + 1)));
  }
  ...
  unsigned int GetNbins() const {
    int s = size();
    return (unsigned int)std::max(s - 1, 0);
  }

  void resize(unsigned int newsize) {
    if (newsize != size())
      binEdges_.resize(newsize);
  }
  ...
  FastHistoAxis_t<U> &operator=(const FastHistoAxis_t<U> &other) {
  ...
  int FindBin(const U &x) const {
    if (x == binEdges_.at(size() - 1) && size() > 1)
      return (int)(size() - 2);
  ...
  U GetBinLowEdge(const int bin = -1) const {
    if (size() == 0)
      return U(0);
    else if (bin >= 0 && bin < (int)size())
      return binEdges_[bin];
    else if (bin >= (int)size())
  ...
  U GetBinUpEdge(const int bin = -1) const {
    if (size() == 0)
      return U(0);
    else if (bin >= 0 && bin < (int)(size() - 1))
      return binEdges_[bin + 1];
    else if (bin >= (int)(size() - 1))
      return binEdges_[size() - 1];
    else
      return binEdges_[0];
  }
...
public:
  ...
  static void SumDiff(const FastTemplate_t<T> &h1,
                      const FastTemplate_t<T> &h2,
                      FastTemplate_t<T> &sum,
                      FastTemplate_t<T> &diff);
  ...
  FastTemplate_t(const TH3 &other) : size_(other.GetNbinsX() * other.GetNbinsY() * other.GetNbinsZ()), values_(size_) {
    CopyValues(other);
  }
...
public:
  ...
  U GetBinWidthX(const unsigned int bin) const {
    if (normX_)
      return U(1);
    else
      return axisX_.GetBinWidth(bin);
  }
  ...
  U GetBinWidthY(const unsigned int bin) const {
    if (normY_)
      return U(1);
    else
      return axisY_.GetBinWidth(bin);
  }
  ...
  T GetAt(const U &x, const U &y) const {
    ...
    if (xbin < 0 || ybin < 0)
      ...
      return GetBinContent((unsigned int)xbin, (unsigned int)ybin);
  }
...
public:
  ...
  U GetBinWidthZ(const unsigned int bin) const {
    if (normZ_)
      return U(1);
    else
      return axisZ_.GetBinWidth(bin);
  }
  ...
  T GetAt(const U &x, const U &y, const U &z) const {
    ...
    if (xbin < 0 || ybin < 0 || zbin < 0)
      ...
      return GetBinContent((unsigned int)xbin, (unsigned int)ybin, (unsigned int)zbin);
  }

  T IntegralWidth(
      int xbinmin = -1, int xbinmax = -1, int ybinmin = -1, int ybinmax = -1, int zbinmin = -1, int zbinmax = -1) const;
  unsigned int binX() const { return GetNbinsX(); }
  unsigned int binY() const { return GetNbinsY(); }
