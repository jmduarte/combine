...
public:
  ...
  FastTemplateFunc_t(const char* name, const char* title, RooArgList& inObsList)
      : RooAbsReal(name, title), obsList("obsList", "obsList", this) {
    setProxyList(obsList, inObsList);
  }
  FastTemplateFunc_t(const FastTemplateFunc_t& other, const char* name = 0)
      : RooAbsReal(other, name), obsList("obsList", this, other.obsList) {}
...
public:
  ...
  FastHistoFunc_t(const char* name, const char* title, RooArgList& inObsList, FastHisto_t<T, U>& inTpl)
      : FastTemplateFunc_t<T>(name, title, inObsList), tpl(inTpl), fullIntegral(tpl.IntegralWidth()) {
    if ((this->obsList).getSize() != 1) {
      std::cerr << "FastHistoFunc_t::FastHistoFunc_t(" << this->GetName() << "): obsList.size()!=1!" << std::endl;
      assert(0);
    }
  }
  FastHistoFunc_t(const FastHistoFunc_t& other, const char* name = 0)
      : FastTemplateFunc_t<T>(other, name), tpl(other.tpl), fullIntegral(other.fullIntegral) {}
  ~FastHistoFunc_t() {}
  ...
  Int_t getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName = 0) const {
    ...
    for (int ic = 0; ic < (this->obsList).getSize(); ic++) {
      ...
      if (var != 0) {
        if (this->matchArgs(allVars, analVars, RooArgSet(*var)))
          code *= code_prime[ic];
      }
    }
    if (code == 1)
  ...
  Double_t analyticalIntegral(Int_t code, const char* rangeName = 0) const {
    ...
    if (std::abs((Float_t)(xmin / tpl.GetXmin() - U(1))) < 1e-5 &&
...
public:
  ...
  FastHisto2DFunc_t(const char* name, const char* title, RooArgList& inObsList, FastHisto2D_t<T, U>& inTpl)
      : FastTemplateFunc_t<T>(name, title, inObsList), tpl(inTpl), fullIntegral(tpl.IntegralWidth()) {
  ...
  FastHisto2DFunc_t(const FastHisto2DFunc_t& other, const char* name = 0)
      : FastTemplateFunc_t<T>(other, name), tpl(other.tpl), fullIntegral(other.fullIntegral) {}
  ~FastHisto2DFunc_t() {}
  ...
  Double_t analyticalIntegral(Int_t code, const char* rangeName = 0) const {
    ...
    if (code % 6 == 0 && fabs((Float_t)(xmin / tpl.GetXmin() - U(1))) < 1e-5 &&
        fabs((Float_t)(xmax / tpl.GetXmax() - U(1))) < 1e-5 && fabs((Float_t)(ymin / tpl.GetYmin() - U(1))) < 1e-5 &&
      ...
      if (code % 2 != 0)
        result /= tpl.GetBinWidthX(xbinmin);
      if (code % 3 != 0)
        result /= tpl.GetBinWidthY(ybinmin);
...
public:
  ...
  FastHisto3DFunc_t(const char* name, const char* title, RooArgList& inObsList, FastHisto3D_t<T, U>& inTpl)
      : FastTemplateFunc_t<T>(name, title, inObsList), tpl(inTpl), fullIntegral(tpl.IntegralWidth()) {
  ...
  FastHisto3DFunc_t(const FastHisto3DFunc_t& other, const char* name = 0)
      : FastTemplateFunc_t<T>(other, name), tpl(other.tpl), fullIntegral(other.fullIntegral) {}
  ~FastHisto3DFunc_t() {}
  ...
  Double_t analyticalIntegral(Int_t code, const char* rangeName = 0) const {
    ...
    if (code % 30 == 0 && fabs((Float_t)(xmin / tpl.GetXmin() - U(1))) < 1e-5 &&
        fabs((Float_t)(xmax / tpl.GetXmax() - U(1))) < 1e-5 && fabs((Float_t)(ymin / tpl.GetYmin() - U(1))) < 1e-5 &&
        fabs((Float_t)(ymax / tpl.GetYmax() - U(1))) < 1e-5 && fabs((Float_t)(zmin / tpl.GetZmin() - U(1))) < 1e-5 &&
      ...
      if (code % 3 != 0)
        result /= tpl.GetBinWidthY(ybinmin);
      if (code % 5 != 0)
        result /= tpl.GetBinWidthZ(zbinmin);
      return (Double_t)result;
    }
  }
