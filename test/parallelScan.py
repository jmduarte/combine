...
if points == 0:
    raise RuntimeError(
        "parallelScan requires that there be a --points=<n> or --points <n> option in the command line\n"
    )
if jobs == 0:
...
if hadd:
    if not method:
        ...
        print(
            "All workers done, doing the hadd to make %s out of higgsCombine%s.{0..%d}.%s.mH%g.root"
            % (output, name, jobs - 1, method, mass)
        )
