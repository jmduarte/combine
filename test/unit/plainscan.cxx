...
int main(int argc, char **argv) {
  if (argc <= 1) {
    printf(
        "Usage: %s file -w workspace(=w) -c modelConfig(=ModelConfig) -D dataset(=data_obs)  -S snapshot  -s "
        "strategy(=0) -t tolerance(=1) -o outfile(=scan.root) -P poi(=r) -m min -M max -n points(=10)  \n",
        argv[0]);
    return 1;
  }
  ...
  do {
    ...
    switch (opt) {
      ...
      case 'A':
        ...
        break;
      case '?':
        std::cerr << "Unsupported option. Please see the code. " << std::endl;
        return 1;
        break;
    }
    if (opt == -1)
  ...
  if (!f) {
    std::cerr << "ERROR: could not open " << argv[optind] << std::endl;
    return 2;
  }
  RooWorkspace *w = (RooWorkspace *)f->Get(workspace);
  if (!w) {
    std::cerr << "ERROR: could not find workspace '" << workspace << "' in " << argv[optind] << std::endl;
    f->ls();
    return 2;
  }
  RooStats::ModelConfig *mc = (RooStats::ModelConfig *)w->genobj(modelConfig);
  if (!mc) {
    std::cerr << "ERROR: could not find ModelConfig '" << modelConfig << "' in worskpace" << std::endl;
    return 2;
  }
  RooAbsData *d = w->data(dataset);
  if (!d) {
    std::cerr << "ERROR: could not find dataset '" << dataset
              << "' in workspace. Available datasets are: " << std::endl;
  ...
  if (tofix) {
    ...
    for (RooAbsArg *a = (RooAbsArg *)iter.Next(); a != 0; a = (RooAbsArg *)iter.Next()) {
      ...
      if (rrv) {
        std::cout << "Fixing " << a->GetName() << std::endl;
  ...
  if (tofloat) {
    ...
    for (RooAbsArg *a = (RooAbsArg *)iter.Next(); a != 0; a = (RooAbsArg *)iter.Next()) {
      RooRealVar *rrv = dynamic_cast<RooRealVar *>(a);
      if (rrv) {
        std::cout << "Floating " << a->GetName() << std::endl;
