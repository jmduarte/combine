...
void runTest(RooWorkspace *w, const char *gen, const char *fit, bool diago, int N = 500) {
  ...
  RooDataSet *data = w->pdf(gen)->generate(obs, N);
  data->SetName("data");
  RooFitResult *result =
      w->pdf(fit)->fitTo(*data, RooFit::Save(1), RooFit::Minimizer("Minuit2"), RooFit::PrintLevel(-1));
  result->Print("V");
...
void testThree() {
  ...
  w->factory(
      "SUM::fit0(c0[0.5,0.3,1]*Exponential(x, slope[-0.6,-1,0]), Gaussian(x, mu[0.4,0.2,.8], sig[0.1,0.05,0.2]))");
  w->defineSet("obs", "x");
