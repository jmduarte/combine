...
void testCachingPdf(RooAbsPdf *pdf, RooAbsData *data) {
  ...
  for (int i = 0; i < data->numEntries(); ++i) {
    std::cout << "plain: " << refvals[i] << ", opt: " << vals[i] << ", " << (refvals[i] == vals[i] ? "OK" : "FAIL")
              << std::endl;
  }
  const std::vector<Double_t> &vals2 = cpdf.eval(*data);
  for (int i = 0; i < data->numEntries(); ++i) {
    std::cout << "plain: " << refvals[i] << ", opt: " << vals2[i] << ", " << (refvals[i] == vals2[i] ? "OK" : "FAIL")
              << std::endl;
  }
}
...
template <int ID> RooFitResult *fitNLL(RooAbsReal &nll) {
...
void testCachingSimFit(RooSimultaneous *pdf, RooAbsData *data, RooArgSet *constraints, int nattempts) {
  ...
  std::cout << "Done fits. plain fit result: " << std::endl;
  plainFit->Print("V");
  std::cout << "Done fits. alter fit result: " << std::endl;
  alterFit->Print("V");
...
void testCachingPdf(RooStats::ModelConfig &mc, RooAbsData *data, int tries = 0) {
  ...
  for (int i = 0; i < allPdfs.getSize(); ++i) {
    ...
    if (pdf->dependsOn(*obs) && !pdf->InheritsFrom("RooSimultaneous") && !pdf->InheritsFrom("RooAddPdf") &&
        !pdf->InheritsFrom("RooProdPdf")) {
...
void testCachingSimNLL(RooStats::ModelConfig &mc, RooAbsData *data, int tries = 0) {
  ...
  for (int i = 0; i < allPdfs.getSize(); ++i) {
    ...
    if (pdf->dependsOn(*obs) && pdf->InheritsFrom("RooSimultaneous")) {
      std::cout << "Will use pdf " << pdf->GetName() << " (" << pdf->ClassName() << ")" << std::endl;
      if (tries > 0)
        testCachingSimNLL((RooSimultaneous *)pdf, data, &nuis, tries);
      else
        testCachingSimFit((RooSimultaneous *)pdf, data, &nuis, 1 - tries);
      //if (tries-- == 0) break;
    } else {
      //std::cout << "Will NOT use pdf " << pdf->GetName() << " (" << pdf->ClassName() << ")" << std::endl;
    }
...
void runExternal(const char *file, int n, const char *wsp, const char *datan, const char *mcn) {
  ...
  if (f == 0)
    return;
  w = (RooWorkspace *)f->Get(wsp);
  if (w == 0)
    return;
  if (w->var("MH"))
    w->var("MH")->setVal(130.);
  w->Print("V");
  RooAbsData *data = w->data(datan);
  if (data == 0)
    return;
  RooStats::ModelConfig *mc = (RooStats::ModelConfig *)w->genobj(mcn);
  if (mc == 0)
    return;
  //testChecker(*mc);
