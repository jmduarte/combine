...
int runReuseTS(RooStats::ModelConfig &mc, RooAbsData *data, int tries = 0) {
  ...
  if (mc.GetNuisanceParameters())
    snapB.addClone(*mc.GetNuisanceParameters());
  ((RooRealVar *)poi.first())->setVal(1.0);
  RooArgSet snapS;
  snapS.addClone(poi);
  if (mc.GetNuisanceParameters())
    snapS.addClone(*mc.GetNuisanceParameters());
  ...
  for (int i = 0; i < plainTimes.size(); ++i) {
    std::cout << " time " << i << ": plain = " << plainTimes[i] << ", opt = " << optTimes[i]
...
int runPerfTS(bool opt, RooStats::ModelConfig &mc, RooAbsData *data, int tries = 0) {
  ...
  if (opt)
    ts = new SimplerLikelihoodRatioTestStatOpt(*mc.GetObservables(), *mc.GetPdf(), *mc.GetPdf(), snapB, snapS);
  else
    ts = new RooStats::SimpleLikelihoodRatioTestStat(*mc.GetPdf(), *mc.GetPdf(), snapB, snapS);
  TStopwatch timer;
  timer.Stop();
  timer.Reset();
...
int main(int argc, char **argv) {
  ...
  if (argc >= 2) {
    ...
    printf(
        "   - testSimplerLikelihoodRatioTestStatOpt (opt|plain) N rootfile [ workspaceName dataName  modelConfig ]\n");
