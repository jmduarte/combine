...
void makeNplot(RooWorkspace *w, TString pdf, TString var, double refval, double newval, double x, TString algo) {
  ...
  TH1 *h_nominal = w->pdf(pdf)->createHistogram("x");
  w->var(var)->setVal(newval);
  TH1 *h_shift = w->pdf(pdf)->createHistogram("x");
  w->var(var)->setVal(x * newval + (1 - x) * refval);
  TH1 *h_mid = w->pdf(pdf)->createHistogram("x");
  w->var(var)->setVal(refval);
  ...
  w->var(var)->setVal(refval);
  w->pdf(pdf)->plotOn(frame, LineColor(kBlue), LineWidth(2));
  p_shift->plotOn(frame, LineColor(kRed), LineWidth(3));
  w->var(var)->setVal(newval);
  w->pdf(pdf)->plotOn(frame, LineColor(kRed), LineWidth(2));
  p_mid->plotOn(frame, LineColor(kBlack), LineWidth(3), LineStyle(2));
  w->var(var)->setVal(x * newval + (1 - x) * refval);
  w->pdf(pdf)->plotOn(frame, LineColor(kBlack), LineWidth(3), LineStyle(2));
  morph->plotOn(frame, LineColor(209), LineWidth(3));
  frame->Draw();
  c1->Print(TString::Format(
      "mplots/%s_%s_ref%.1f_shift%.1f_x%.1f_%s.png", pdf.Data(), var.Data(), refval, newval, x, algo.Data()));
  delete morph;
  delete p_nominal;
  delete p_shift;
  delete p_mid;
