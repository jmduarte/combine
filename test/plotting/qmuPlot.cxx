...
double tailReal(TTree *t, std::string br, double cv, int mode) {
  ...
  if (br == "qB") {
    if (mode == 0)
      tpass = t->Draw("max(2*q,0)>>qTMP", Form("weight*(type==-1 && max(2*q,0)> %g)", cv));
    else
      tpass = t->Draw("2*q>>qTMP", Form("weight*(type==-1 && 2*q < %g)", cv));
...
TCanvas *q0Plot(float mass, std::string poinam, int rebin = 0) {
  if (gFile == 0) {
    std::cerr << "You must have a file open " << std::endl;
    return 0;
  }
  TTree *t = (TTree *)gFile->Get("q");
  if (t == 0) {
    std::cerr << "File " << gFile->GetName() << " does not contain a tree called 'q'" << std::endl;
    return 0;
  }
  ...
  double sigerr = 0.5 * (TMath::Abs(RooStats::PValueToSignificance(clB + clBerr) - sig) +
                         TMath::Abs(RooStats::PValueToSignificance(clB - clBerr) - sig));
  ...
  qB1->SetFillColor(qB1->GetLineColor());
  qB1->SetLineWidth(0);
  qB1->SetFillStyle(3345);
...
TCanvas *qmuPlot(float mass,
  ...
  if (mode == 0)
    t->Draw("max(2*q,0)>>qS", "weight*(type==+1)", "SAME");
  else
    t->Draw("2*q>>qS", "weight*(type==+1)", "SAME");
  ...
  TH1F *oneSig = getQuantHist(qB, 0.16, 0.84);
  oneSig->SetFillColor(kGreen + 1);
  TH1F *twoSig = getQuantHist(qB, 0.025, 0.975);
  twoSig->SetFillColor(kYellow);
