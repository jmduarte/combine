...
void addLepPoint(LepBand &b, double x, TString file, double r = 1.0) {
  ...
  double summer68 = samples[floor(nd * 0.5 * (1 - 0.68) + 0.5)],
         winter68 = samples[TMath::Min(int(floor(nd * 0.5 * (1 + 0.68) + 0.5)), nd - 1)];
  double summer95 = samples[floor(nd * 0.5 * (1 - 0.95) + 0.5)],
         winter95 = samples[TMath::Min(int(floor(nd * 0.5 * (1 + 0.95) + 0.5)), nd - 1)];
  res->SetTestStatisticData(median + 1e-6);
  double clsMid = res->CLs(), clsbMid = res->CLsplusb();
  res->SetTestStatisticData(summer68 + 1e-6);
  double cls68L = res->CLs(), clsb68L = res->CLsplusb();
  res->SetTestStatisticData(winter68 + 1e-6);
  double cls68H = res->CLs(), clsb68H = res->CLsplusb();
  res->SetTestStatisticData(summer95 + 1e-6);
  double cls95L = res->CLs(), clsb95L = res->CLsplusb();
  res->SetTestStatisticData(winter95 + 1e-6);
  double cls95H = res->CLs(), clsb95H = res->CLsplusb();
  /*
    ...
    */
  b.obs->Set(b.n + 1);
  b.obs->SetPoint(b.n, x, clsObs);
  b.obs->SetPointError(b.n, 0, 0, clsObsErr, clsObsErr);
  b.exp68->Set(b.n + 1);
  b.exp68->SetPoint(b.n, x, clsMid);
  b.exp68->SetPointError(b.n, 0, 0, clsMid - cls68L, cls68H - clsMid);
  b.exp95->Set(b.n + 1);
  b.exp95->SetPoint(b.n, x, clsMid);
  b.exp95->SetPointError(b.n, 0, 0, clsMid - cls95L, cls95H - clsMid);
  b.n++;
}
