...
TH2 *treeToHist2D(TTree *t,
                  ...
                  int xbins,
                  int ybins) {
  t->Draw(Form("2*deltaNLL:%s:%s>>%s_prof(%d,%10g,%10g,%d,%10g,%10g)",
               y.Data(),
               x.Data(),
               name.Data(),
               xbins,
...
TList *contourFromTH2(TH2 *h2in, double threshold, int minPoints = 20) {
  ...
  if (h2in->GetNbinsX() * h2in->GetNbinsY() > 10000)
    minPoints = 50;
  if (h2in->GetNbinsX() * h2in->GetNbinsY() <= 100)
    minPoints = 10;
...
TH2D *frameTH2D(TH2D *in, double threshold) {
  ...
  if (TString(in->GetName()).Contains("bayes"))
    frameValue = -1000;
  ...
  TH2D *framed =
      new TH2D(Form("%s framed", in->GetName()), Form("%s framed", in->GetTitle()), nx + 2, xbins, ny + 2, ybins);
