...
void hypoTestResultTree(TString fOutName, double mass, double rValue = 1.0, const char *poiName = "r") {
  ...
  float q, mh = mass, r = rValue, weight;
  int type;
  tree->Branch("q", &q, "q/F");
  ...
  for (int i = 0, n = gROOT->GetListOfFiles()->GetSize() - 1; i < n; ++i) {
    ...
    if (toyDir == 0) {
      std::cerr << "Error in file " << gROOT->GetListOfFiles()->At(i)->GetName() << ": directory /toys not found"
                << std::endl;
      continue;
    }
  ...
  tree->Write();
  fOut->Close();
  std::cout << "Saved test statistics distributions for " << nS << " signal toys and " << nB << " background toys to "
            << fOutName << "." << std::endl;
}
