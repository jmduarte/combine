...
TGraphAsymmErrors *theBand(TFile *file, int doSyst, int whichChannel, BandType type, double width = 0.68) {
  ...
  if (t == 0)
    t = (TTree *)file->Get("test");  // backwards compatibility
  if (t == 0) {
  ...
  if (use_precomputed_quantiles) {
    if (t->GetBranch("quantileExpected") == 0) {
      std::cerr << "TFile " << file->GetName() << " does not have precomputed quantiles" << std::endl;
      return 0;
    }
  ...
  for (size_t i = 0, n = t->GetEntries(); i < n; ++i) {
    ...
    if (type == Asimov) {
      ...
      if (iToy != 0)
        continue;
    } else if (type == ObsQuantile && iToy == 0) {
      obsValues[iMass] = limit;
      continue;
    } else if (iToy <= 0 && !use_precomputed_quantiles)
      continue;
    if (limit == 0 && !zero_is_valid)
      continue;
    if (type == MeanCPUTime) {
      if (limit < 0)
        continue;
    ...
    if (use_precomputed_quantiles) {
      ...
      if (type == Observed && quant > 0)
        continue;
      if (type == Median) {
        if (fabs(quant - 0.5) > 0.005 && fabs(quant - (1 - width) / 2) > 0.005 &&
            fabs(quant - (1 + width) / 2) > 0.005) {
  ...
  for (std::map<int, std::vector<double> >::iterator it = dataset.begin(), ed = dataset.end(); it != ed; ++it) {
    ...
    for (int j = 0; j < nd; ++j)
      mean += data[j];
    mean /= nd;
    double summer68 = data[floor(nd * 0.5 * (1 - width) + 0.5)],
           winter68 = data[std::min(int(floor(nd * 0.5 * (1 + width) + 0.5)), nd - 1)];
    if (use_precomputed_quantiles && type == Median) {
      if (precomputed_median_only && data.size() == 1) {
        mean = median = summer68 = winter68 = data[0];
      } else if (data.size() != 3) {
        std::cerr << "Error for expected quantile for mass " << it->first << ": size of data is " << data.size()
                  << std::endl;
        continue;
      } else {
        mean = median = data[1];
    ...
    switch (type) {
      ...
      case Quantile:  // get the quantile equal to width, and it's uncertainty
        ...
        break;
      case ObsQuantile: {
        if (obsValues.find(it->first) == obsValues.end())
          continue;
        int pass = 0, fail = 0;
  ...
  for (std::map<int, TGraphErrors *>::iterator it = dataset.begin(), ed = dataset.end(); it != ed; ++it) {
    ...
    if (graph->GetY()[0] > 1 - width || imax == 0) {
      ...
      for (ilo = 1; ilo < imax; ++ilo) {
        if (graph->GetEY()[ilo] == 0)
          continue;
        if (graph->GetY()[ilo] >= 0.05 * (1 - width))
          break;
      }
      ilo -= 1;
      for (ihi = imax; ihi > ilo + 1; --ihi) {
        if (graph->GetEY()[ihi] == 0)
          continue;
        if (graph->GetY()[ihi] <= 3 * (1 - width))
          break;
      }
      double xmin = graph->GetX()[ilo], xmax = graph->GetX()[ihi];
      if (ilo <= 1)
        xmin = 0.0001;
      fitErf.SetRange(xmin, xmax);
      fitErf.SetNpx(1000);
    ...
    if (graph->GetY()[n - 1] > 1 - width || imax == n - 1) {
      ...
      if (bhigh < graph->GetX()[0]) {
        bhigh = graph->GetX()[0] + ((1 - width) - graph->GetY()[0]) * (graph->GetX()[1] - graph->GetX()[0]) /
                                       (graph->GetY()[1] - graph->GetY()[0]);
      ...
      for (ilo = imax + 1; ilo < n - 2; ++ilo) {
        if (graph->GetEY()[ilo] == 0)
          continue;
        if (graph->GetY()[ilo] <= 3 * (1 - width))
          break;
      }
      if (ilo > 0 && graph->GetEY()[ilo - 1] != 0)
        ilo--;
      for (ihi = ilo + 1; ihi < n; ++ihi) {
        ...
        if (graph->GetY()[ihi] >= 0.05 * (1 - width))
          break;
      }
      if (ihi - ilo <= 1) {
        double xmin = graph->GetX()[ilo], xmax = graph->GetX()[ihi];
...
void makeBand(TDirectory *bands, TString name, TFile *file, int doSyst, int whichChannel, BandType type) {
  ...
  if (!doSyst && (type != AdHoc))
    suffix = "_nosyst" + suffix;
...
void makeBand(TDirectory *bands, TString name, TString filename, int doSyst, int whichChannel, BandType type) {
  ...
  if (in == 0) {
    std::cerr << "Filename " << filename << " missing" << std::endl;
    return;
  }
...
void makeBands(TDirectory *bands, TString name, TString filename, int channel = 0, bool quantiles = false) {
  ...
  for (int s = do_bands_nosyst ? 0 : 1; s <= 1; ++s) {
    ...
    if (do_bands_cputime)
      makeBand(bands, name, in, s, channel, MeanCPUTime);
    if (do_bands_realtime)
      makeBand(bands, name, in, s, channel, MeanRealTime);
  }
  if (quantiles) {
...
void cutBand(TDirectory *bands, TString inName, TString outName, float mMin, float mMax) {
  ...
  for (int i = 0; i < n; ++i) {
    if (mMin <= b1->GetX()[i] && b1->GetX()[i] <= mMax) {
      ...
      b2->SetPointError(m, b1->GetErrorXlow(i), b1->GetErrorXhigh(i), b1->GetErrorYlow(i), b1->GetErrorYhigh(i));
      m++;
    }
  }
  b2->SetName(outName);
...
void cutFcBands(
    TDirectory *bands, TString inName, TString outName, float mMin, float mMax, int npostfix, char **postfixes) {
...
void combineBand(TDirectory *in, TString band1, TString band2, TString comb) {
  ...
  for (int i = 0; i < first; ++i, ++k) {
    ...
    bc->SetPointError(k, b1->GetErrorXlow(i), b1->GetErrorXhigh(i), b1->GetErrorYlow(i), b1->GetErrorYhigh(i));
  }
  for (int i = 0; i < m; ++i, ++k) {
    bc->SetPoint(k, b2->GetX()[i], b2->GetY()[i]);
    bc->SetPointError(k, b2->GetErrorXlow(i), b2->GetErrorXhigh(i), b2->GetErrorYlow(i), b2->GetErrorYhigh(i));
  }
...
void mergeBand(TDirectory *in, TString band1, TString band2, TString comb) {
  ...
  for (int i = 0; i < m; ++i) {
    if (findBin(b1, b2->GetX()[i], 0.001) != -1)
      continue;
    bc->Set(k);
    bc->SetPoint(k, b2->GetX()[i], b2->GetY()[i]);
    bc->SetPointError(k, b2->GetErrorXlow(i), b2->GetErrorXhigh(i), b2->GetErrorYlow(i), b2->GetErrorYhigh(i));
...
void stripPoint(TGraph *band, float m) {
  ...
  if ((band->GetN() > 0) && (band->GetX()[0] <= m) && (band->GetX()[band->GetN() - 1] >= m)) {
  }
}
...
void selectedPointsBand(TDirectory *in,
                        TString band1,
                        TString band2,
                        float m1,
                        float m2 = 0,
  ...
  if (m2)
    copyPoint(band, m2, ret);
  if (m3)
    copyPoint(band, m3, ret);
  if (m4)
    copyPoint(band, m4, ret);
  if (m5)
    copyPoint(band, m5, ret);
  if (m6)
    copyPoint(band, m6, ret);
  if (m7)
    copyPoint(band, m7, ret);
  ret->SetName(band2);
  in->WriteTObject(ret, ret->GetName(), "Overwrite");
}

void selectedPointsBands(TDirectory *in,
                         TString bandIn,
                         TString bandOut,
                         float m1,
                         float m2 = 0,
...
void printLine(TDirectory *bands, TString who, FILE *fout, TString header = "value") {
  ...
  if (mean == 0) {
    std::cerr << "MISSING " << who << std::endl;
    return;
  }
  fprintf(fout, "%4s \t %7s\n", "mass", header.Data());
  fprintf(fout, "%5s\t %7s\n", "-----", "-----");
  TString fmtstring = TString() + (halfint_masses ? "%5.1f" : "%4.0f ") + "\t " +
...
void printLineAErr(TDirectory *bands, TString who, FILE *fout, TString header = "value") {
  ...
  TString fmtstring =
      TString() + (halfint_masses ? "%5.1f" : "%4.0f ") + "\t " +
      (who.Contains("pval") || who.Contains("smcls") ? "%7.5f  -%7.5f / +%7.5f" : "%7.3f  -%6.3f / +%6.3f") + "\n";
  fprintf(fout, "%5s\t %7s------%6s----%6s-\n", "-----", " ------", "------", "------");
  for (int i = 0, n = mean->GetN(); i < n; ++i) {
    fprintf(fout, fmtstring.Data(), mean->GetX()[i], mean->GetY()[i], mean->GetErrorYlow(i), mean->GetErrorYhigh(i));
  }
}
...
void printBand(TDirectory *bands, TString who, FILE *fout, bool mean = false) {
  ...
  if (mean68 == 0) {
    printLineErr(bands, who + "_obs", fout);
    return;
  }
  fprintf(fout,
  ...
  TString fmtstring = TString() + (halfint_masses ? "%5.1f" : "%4.0f ") + "\t " +
                      (who.Contains("pval") || who.Contains("smcls") ? "%8.6f  %8.6f  %8.6f  %8.6f  %8.6f  %8.6f"
...
void printFcBand(TDirectory *bands, TString who, FILE *fout, int npostfix, char **postfixes) {
  ...
  for (int i = 0; i < nbands; ++i)
    fprintf(fout, " -%-5s  ", names[nbands - i - 1]);
  fprintf(fout, "  %8s  ", "  mid.  ");
  for (int i = 0; i < nbands; ++i)
    fprintf(fout, "   +%-5s", names[i]);
  fprintf(fout, "\n");
...
void printFcBand(TDirectory *bands, TString who, TString fileName, int npostfix, char **postfixes) {
  ...
  if (first == 0) {
    std::cerr << "MISSING " << who << "_" << postfixes[0] << std::endl;
    return;
  }
  FILE *fout = fopen(fileName.Data(), "w");
...
void printQuantiles(TDirectory *bands, TString who, FILE *fout) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    if (graphs[i] == 0) {
      std::cout << "Missing quantile band for p = " << quants[i] << std::endl;
      return;
    }
  }
  fprintf(fout,
          "%4s \t %6s %5s   %6s %5s   %6s %5s   %6s %5s   %6s %5s\n",
          "mass",
          "-95%",
          "err",
          "-68%",
...
void printQuantiles(TDirectory *bands, TString who, TString fileName) {
  ...
  if (mean68 == 0) {
    std::cerr << "MISSING " << who << "_quant025" << std::endl;
    return;
  }
  FILE *fout = fopen(fileName.Data(), "w");
...
void importLine(TDirectory *bands, TString name, const char *fileName) {
  ...
  TGraphAsymmErrors *inObs = new TGraphAsymmErrors();
  inObs->SetName(name);
  float mH, yObs;
...
void importBands(TDirectory *bands, TString name, const char *fileName, bool hasObs = false, bool has95 = true) {
  ...
  TGraphAsymmErrors *inObs = new TGraphAsymmErrors();
  inObs->SetName(name + "_obs");
  TGraphAsymmErrors *in68 = new TGraphAsymmErrors();
  in68->SetName(name + "_median");
  TGraphAsymmErrors *in95 = new TGraphAsymmErrors();
  in95->SetName(name + "_median_95");
  ...
  if (hasObs) {
    for (int n = 0; fscanf(in, "%f %f %f %f %f %f %f", &mH, &yObs, &yLL, &yLo, &y, &yHi, &yHH) == 7; ++n) {
      inObs->SetPoint(n, mH, yObs);
      in68->SetPoint(n, mH, y);
      in68->SetPointError(n, 0, 0, y - yLo, yHi - y);
      in95->SetPoint(n, mH, y);
      in95->SetPointError(n, 0, 0, y - yLL, yHH - y);
    }
  } else {
    if (has95) {
...
void importLandS(TDirectory *bands, TString name, TFile *file, bool doObserved = true, bool doExpected = true) {
  ...
  int nobs = 0;
  TGraphAsymmErrors *median = new TGraphAsymmErrors();
  median->SetName(name + "_median");
  int nmedian = 0;
  TGraphAsymmErrors *median_95 = new TGraphAsymmErrors();
  median_95->SetName(name + "_median_95");
  int nmedian_95 = 0;
  for (size_t i = 0, n = t->GetEntries(); i < n; ++i) {
    t->GetEntry(i);
    if (doObserved) {
  ...
  if (obs->GetN()) {
    obs->Sort();
    bands->WriteTObject(obs);
    std::cout << " imported " << obs->GetName() << " with " << obs->GetN() << " points." << std::endl;
  }
  if (median->GetN()) {
    median->Sort();
    bands->WriteTObject(median);
    std::cout << " imported " << median->GetName() << " with " << median->GetN() << " points." << std::endl;
  }
  if (median_95->GetN()) {
    median_95->Sort();
    bands->WriteTObject(median_95);
    std::cout << " imported " << median_95->GetName() << " with " << median_95->GetN() << " points." << std::endl;
  }
}
...
void printValueFromScan1D(TDirectory *bands, TString name, TString out) {
  ...
  for (int i = 0; i < n - 1; ++i) {
    if (y[i] > t1 && y[i + 1] < t1) {
      ...
      lo68 = (x[i] * d2 + x[i + 1] * d1) / (d1 + d2);
      lo68ok = true;
    } else if (y[i] < t1 && y[i + 1] > t1) {
      double d1 = fabs(y[i] - t1), d2 = fabs(y[i + 1] - t1);
      hi68 = (x[i] * d2 + x[i + 1] * d1) / (d1 + d2);
      hi68ok = true;
    }
    if (y[i] > t2 && y[i + 1] < t2) {
      double d1 = fabs(y[i] - t2), d2 = fabs(y[i + 1] - t2);
      lo95 = (x[i] * d2 + x[i + 1] * d1) / (d1 + d2);
      lo95ok = true;
    } else if (y[i] < t2 && y[i + 1] > t2) {
      double d1 = fabs(y[i] - t2), d2 = fabs(y[i + 1] - t2);
      hi95 = (x[i] * d2 + x[i + 1] * d1) / (d1 + d2);
      hi95ok = true;
    }
  }
  ...
  if (lo68ok)
    fprintf(log, "Crossing at 1.00 from left:  % 8.4f \n", lo68);
  if (hi68ok)
    fprintf(log, "Crossing at 1.00 from right: % 8.4f \n", hi68);
  if (lo95ok)
    fprintf(log, "Crossing at 3.84 from left:  % 8.4f \n", lo95);
  if (hi95ok)
    fprintf(log, "Crossing at 3.84 from right: % 8.4f \n", hi95);
  fclose(log);
}
