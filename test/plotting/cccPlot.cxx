...
void cccPlot(const char *poi = "r", double rMax = 4, const char *filename = "ccc.pdf") {
  ...
  if (gFile == 0) {
    std::cout << "No input file open" << std::endl;
    return;
  }
  ...
  if (fit_nominal == 0 || fit_alternate == 0) {
    std::cout << "Input file " << gFile->GetName() << " does not contain fit_nominal or fit_alternate" << std::endl;
    return;
  }
  RooRealVar *rFit = (RooRealVar *)fit_nominal->floatParsFinal().find(poi);
  if (rFit == 0) {
    std::cout << "Nominal fit does not contain parameter " << poi << std::endl;
    return;
  }
  ...
  TH2F frame(
      "frame", ";best fit #sigma/#sigma_{SM};", 1, rFit->getMin(), TMath::Min(rFit->getMax(), rMax), nChann, 0, nChann);
