...
def _readRootFile(fname):
    ...
    if abs(result["limit"]) > 1e6:
        return {"status": "aborted", "comment": "rootfile %s contains bogus data" % fname}
    return result
    ...
    for i in range(g.GetN()):
        ...
        if x < 0:
            ...
            results.append(
                (
                    " E%03d" % (x * 1000),
                    {"status": "done", "comment": "", "limit": float(y), "limitErr": 0, "t_real": 0},
                )
            )
    f.Close()
    return results
...
class MultiDatacardWithExpectedTest(MultiDatacardTest):
    ...
    def readOutput(self, dir):
        ret = {"results": {}}
        for dc, mass in self._datacards:
            thisres = _readRootFileWithExpected(
                "%s/higgsCombine%s.%s.mH%s.root" % (dir, self._name, self._method, mass)
            )
    ...
    def readOutput(self, dir):
        if "MultiDimFit" in self._method:
            out = _readRootFileWithPOI(
                "%s/higgsCombine%s.%s.mH%s.root" % (dir, self._name, self._method, self._mass), self._poi
            )
        else:
            out = _readRootFileWithExpected(
...
class MultiOptionTest(Test):
    def __init__(self, name, datacard, method, commonOptions, optionsMap):
        ...
        self._options = [
            (pn + on, commonOptions + " " + ov, i * 10 + 100) for i, (on, ov) in enumerate(optionsMap.items())
        ]
    ...
    def createScript(self, dir, file):
        ...
        for on, ov, mass in self._options:
            ...
            file.write(
                "combine %s -n %s -M %s %s -m %s >> %s.%s.log 2>&1\n"
                % (datacard_full, self._name, self._method, ov, mass, self._name, mass)
            )
