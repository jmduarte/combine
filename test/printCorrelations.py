...
for i in range(all_pars.getSize()):
    ...
    if par.GetName() == args.parameter:
        ...
        continue
    correlations.append((par.GetName(), rfr.correlation(args.parameter, par.GetName()), par.getVal(), par.getError()))
...
for i in range(min(len(correlations), args.max)):
    print(
        "%-50s %+.3f +/- %.3f    %+.3f"
        % (correlations[i][0], correlations[i][2], correlations[i][3], correlations[i][1])
    )
