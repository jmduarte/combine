...
parser.add_option(
    "-c", "--category", dest="cat", type="string", default="CMS_channel", help="Category to split channels along"
)
parser.add_option("-b", "--binned", dest="binned", default=False, action="store_true", help="produce a RooDataHist")
parser.add_option(
    "-C", "--counting", dest="counting", default=False, action="store_true", help="Input data is a counting experiment"
)
parser.add_option(
    "-S",
    "--manual-split",
...
parser.add_option(
    "-v", "--verbose", dest="verbose", default=0, type="int", help="Verbosity level (0 = quiet, 1 = verbose, 2+ = more)"
)
parser.add_option(
    "-n", "--name", dest="outn", type="string", default="{channel}_bData_{number}", help="Naming convention for output"
)
parser.add_option(
    "-o", "--out", dest="outf", type="string", default="splittedToys.root", help="Name of the output file"
)
parser.add_option(
    "--first",
    dest="first",
    ...
    while True:
        ...
        if options.counting:
            ...
            for i in range(obsin.getSize()):
                ...
                data = ROOT.RooDataSet(
                    name, name, ROOT.RooArgSet(outw.var("CMS_fakeObs"), outw.var("_weight_")), "_weight_"
                )
            ...
            if options.verbose > 2:
                for d in datasets:
                    print(
                        "Dumping dataset %15s, %6d entries, %8.1f events"
                        % (d.GetName(), d.numEntries(), d.sumEntries())
                    )
        ...
        if options.verbose:
            ...
            for i, d in enumerate(datasets):
                print(
                    "   dataset %d: %15s, %6d entries, %8.1f events" % (i, d.GetName(), d.numEntries(), d.sumEntries())
                )
                if options.verbose > 1:
                    d.get().Print("V")
        for d in datasets:
