...
parser.add_option(
    "-a", "--all", dest="all", default=False, action="store_true", help="Report all nuisances (default is only lnN)"
)
parser.add_option(
    "",
    "--noshape",
            ...
            if pdf == "gmN":
                ...
                if "shape" in pdf and MB.isShapeSystematic(b, p, lsyst):
                    ...
                    if objC.InheritsFrom("TH1"):
                        ...
                        valU, valD, valC = objU.sumEntries(), objD.sumEntries(), objC.sumEntries()
...
"""
    ...
    for nuis in names:
        ...
        print(
            '<a id="%s_chann_toggle" href="#%s" onclick="toggleChann(&quot;%s&quot;)">[+]</a></td>' % (nuis, nuis, nuis)
        )
        print("</tr>")
        ...
        for x in sorted(val["bins"]):
            print(
                ...
                % (x, ", ".join(["%s(%s)" % (k, v) for (k, v) in six.iteritems(errlines[nuis][x]) if v != 0]))
            )
        print("\t</table></td>")
        print("</tr>\n")
    for x in outParams.keys():
        print(
            "\t\t<tr><td><b>%s(%s)</b></td><td>%s</td></li>"
            % (x, outParams[x][0], ", ".join([a for a in outParams[x][1]]))
        )
        print("</tr>\n")
    print(
        """
