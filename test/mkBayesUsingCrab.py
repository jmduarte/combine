...
parser.add_option(
    "-j", "--jobs", dest="jobs", default=10, type="int", help="Total number of jobs (can be changed in .cfg file)"
)
...
if len(masses) == 1 and len(workspaces) != 1:
    ...
    raise RuntimeError(
        "You must specify a number of masses equal to the number of workspaces, or just one mass for all of them"
    )
