...
parser.add_option(
    "-o", "--out", dest="out", default="plots2DFC.root", type="str", help="Output File for 2D histos/1D confidence scan"
)
parser.add_option(
    "-t", "--tdir", dest="treename", default="toys", type=str, help="Name of TDirectory for toys inside grid files"
)
parser.add_option(
    "", "--teststat", dest="teststat", default="PL", type=str, help="Test statistic used (chooses 1 or 2 sided)"
)
parser.add_option(
    "-d",
    "--dataNLL",
...
parser.add_option(
    "", "--minToys", dest="minToys", default="-10", type=int, help="Minimum number of toys to accept a point"
)
parser.add_option(
    "",
    "--storeToys",
...
# Also one TGraph with each stored point in it
    ...
    print(
        "Found points in grid: %s = %.2f->%.2f (%d points), %s = %.2f->%.2f (%d points) "
