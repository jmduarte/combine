...
parser.add_option(
    ...
    default="DEFAULT",
    type="str",
    help="Name of model used in model builder (note its the name of the scaling functions, eg the L1/K models use 'c7'",
)
...
parser.add_option(
    ...
    type="str",
    default="13TeV",
    help="A comma separated list of the energies to show expressions for",
)
parser.add_option(
    "", "--skipTree", default=False, action="store_true", help="Skip filling the tree of points in the ND space (slow)"
)
...
parser.add_option(
    ...
    type="str",
    default="",
    help="Overwrite default production channels (comma separated list in a string)",
)
parser.add_option(
    "-D",
    "--decay",
    ...
    type="str",
    default="",
    help="Overwrite default decay channels (comma separated list in a string)",
)
...
if not options.out:
    ...
    os.system("mkdir -p %s" % options.out)
...
def makePlot(name, tgraph):
    ...
    if "xparameter_name" not in list(config.keys()):
        for pcounter in range(nparams):
            ...
            if pcounter == config["xparameter"]:
                config["xparameter_name"] = p.GetName()
            if pcounter == config["yparameter"]:
                config["yparameter_name"] = p.GetName()

    xparam = params.find(config["xparameter_name"])
    yparam = params.find(config["yparameter_name"])

    if options.sliceval:
        zparam = params.find(config["fixparameter_name"])
    ...
    if options.sliceval:
        name += "_%s_%.3f" % (zparam.GetName(), config["sliceval"])
    thist.SetTitle(name)
...
def produceScan(modelname, extname, proddecaystring, work, energy=""):
    ...
    if not options.skipTree:
        fillOutputs(func, tgraph, txtfile, tr)

    # make a 2D plot
    if nparams == 2 or (options.sliceval and nparams < 4):
        makePlot("%s_%s" % (modelname, proddecay), tgraph)
    else:
        print("Skipping 2D plots (nparams != 2, and nparams!=3 with a slice value given)")
