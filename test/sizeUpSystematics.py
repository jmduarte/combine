...
masses = list()
parser = OptionParser(
    usage="usage: %prog [options] datacard.txt [--dir output] \nrun with --help to get list of options"
)
parser.add_option(
    "--depth",
    dest="depth",
...
parser.add_option(
    "--dir", dest="dir", default="systs", type="string", help="Output directory for results and intermediates."
)
parser.add_option(
    "--masses",
    dest="masses",
...
combinationsHash = dict(
    [(combination, hashlib.md5(pickle.dumps(combination)).hexdigest()) for combination in combinationsToRemove]
)
...
for combo in combinationsToRemove:
    ...
    for mass in options.masses:
        ...
        combineCmd = "combine --verbose=1 root/%s %s -n %s &> log/%s.log && mv %s root/" % (
            tempOut,
            combineOpts,
            fName,
            combineOut,
