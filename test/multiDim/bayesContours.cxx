TGraph *bestFit(TH2 *hist) {
  ...
  ret->SetMarkerStyle(34);
  ret->SetMarkerSize(2.0);
  return ret;
}
...
TGraph *smValue(double x0 = 1.0, double y0 = 1.0) {
  ...
  ret->SetMarkerStyle(29);
  ret->SetMarkerSize(4.0);
...
TGraph *bayesContour(TH2 *hist, double cl) {
  ...
  for (int i = 1, k = 0; i <= nx; ++i) {
    if (hist->GetBinContent(i, ny) > threshold) {
      points->Set(++p);
      points->SetPoint(p - 1, ax->GetBinCenter(i), ay->GetBinUpEdge(ny));
    } else {
      for (int j = ny - 1; j > 0; --j) {
        if (hist->GetBinContent(i, j) > threshold) {
          points->Set(++p);
          points->SetPoint(p - 1, ax->GetBinCenter(i), ay->GetBinCenter(j));
          break;
        }
      }
    }
  ...
  for (int i = nx; i > 0; --i) {
    if (hist->GetBinContent(i, 1) > threshold) {
      points->Set(++p);
      points->SetPoint(p - 1, ax->GetBinCenter(i), ay->GetBinLowEdge(1));
    } else {
      for (int j = 2; j <= ny; ++j) {
        if (hist->GetBinContent(i, j) > threshold) {
          points->Set(++p);
...
void threeContours(TString name, TH2 *posterior) {
  ...
  gr99->SetLineStyle(3);
  gr99->SetLineColor(1);
  gr99->SetFillStyle(1001);
  gr99->SetFillColor(93);
  TH2 *frame = (TH2 *)posterior->Clone();
  frame->Reset();
  frame->Draw();
  gr99->Draw("LF SAME");
  gr95->Draw("LF SAME");
  ...
  gFile->WriteTObject(gr0);
  gr68->SetName(name + "_cl68");
  gFile->WriteTObject(gr68);
  gr95->SetName(name + "_cl95");
  gFile->WriteTObject(gr95);
  gr99->SetName(name + "_cl99");
  gFile->WriteTObject(gr99);
  sm->SetName(name + "_sm");
  gFile->WriteTObject(sm);
}
