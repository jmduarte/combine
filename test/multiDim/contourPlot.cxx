...
TGraph *contourPlot(TTree *t, TString x, TString y, double pmin, double pmax, TGraph *bestFit) {
  int n = t->Draw(y + ":" + x,
                  Form("%f <= quantileExpected && quantileExpected <= %f && quantileExpected != 1", pmin, pmax));
  std::cout << "Drawing for "
            << Form("%f <= quantileExpected && quantileExpected <= %f && quantileExpected != 1", pmin, pmax)
            << " yielded " << n << " points." << std::endl;
...
void contourPlot(const char *name, const char *title, double qqHmax = 10., double ggHmax = 4.) {
  ...
  TH2F *frame = new TH2F(name,
                         Form("%s;#sigma/#sigma_{SM} pp #rightarrow qqH;#sigma/#sigma_{SM} gg #rightarrow H;", title),
                         50,
                         0,
                         qqHmax,
                         50,
