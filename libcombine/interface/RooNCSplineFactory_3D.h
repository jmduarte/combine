...
protected:
  ...
  const std::vector<splineQuadruplet_t> getPoints(const std::vector<RooNCSplineCore::T>& XList,
...
public:
  RooNCSplineFactory_3D(RooAbsReal& XVar_,
                        RooAbsReal& YVar_,
                        RooAbsReal& ZVar_,
                        TString appendName_ = "",
  ...
  void setEndConditions(RooNCSplineCore::BoundaryCondition const bcBegin,
  ...
  void setPoints(const std::vector<inType>& XList,
    ...
    for (unsigned int ip = 0; ip < XList.size(); ip++)
      transXList.push_back((RooNCSplineCore::T)XList.at(ip));
    for (unsigned int ip = 0; ip < YList.size(); ip++)
      transYList.push_back((RooNCSplineCore::T)YList.at(ip));
    for (unsigned int ip = 0; ip < ZList.size(); ip++)
      transZList.push_back((RooNCSplineCore::T)ZList.at(ip));
    for (unsigned int ip = 0; ip < FcnList.size(); ip++)
      transFcnList.push_back((RooNCSplineCore::T)FcnList.at(ip));
    const std::vector<splineQuadruplet_t> pList = getPoints(transXList, transYList, transZList, transFcnList);
    setPoints(pList);
  }
};

template void RooNCSplineFactory_3D::setPoints<Float_t>(const std::vector<Float_t>& XList,
                                                        const std::vector<Float_t>& YList,
                                                        const std::vector<Float_t>& ZList,
                                                        const std::vector<Float_t>& FcnList);
template void RooNCSplineFactory_3D::setPoints<Double_t>(const std::vector<Double_t>& XList,
