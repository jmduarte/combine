...
ClassImp(RooNCSpline_3D_fast)
    ...
    : RooNCSplineCore(),
      ...
      rangeZmin(1),
      rangeZmax(-1),
      bcBeginX(RooNCSplineCore::bcNaturalSpline),
      bcEndX(RooNCSplineCore::bcNaturalSpline),
      bcBeginY(RooNCSplineCore::bcNaturalSpline),
      bcEndY(RooNCSplineCore::bcNaturalSpline),
      bcBeginZ(RooNCSplineCore::bcNaturalSpline),
      bcEndZ(RooNCSplineCore::bcNaturalSpline),
      theYVar("theYVar", "theYVar", this),
      theZVar("theZVar", "theZVar", this) {}

RooNCSpline_3D_fast::RooNCSpline_3D_fast(const char* name, const char* title)
...
RooNCSpline_3D_fast::RooNCSpline_3D_fast(const char* name,
                                         const char* title,
                                         RooAbsReal& inXVar,
                                         RooAbsReal& inYVar,
                                         RooAbsReal& inZVar,
  ...
  if (npointsX() > 1 && npointsY() > 1 && npointsZ() > 1) {
    ...
    vector<vector<RooNCSplineCore::T>> xA;
    getKappas(kappaX, 0);
    getAArray(kappaX, xA, bcBeginX, bcEndX);
    npoints = kappaX.size();
    TMatrix_t xAtrans(npoints, npoints);
    for (int i = 0; i < npoints; i++) {
      for (int j = 0; j < npoints; j++) {
        xAtrans[i][j] = xA.at(i).at(j);
      }
    }
    det = 0;
    TMatrix_t xAinv = xAtrans.Invert(&det);
    if (det == 0.) {
      coutE(InputArguments) << "RooNCSpline_3D_fast::interpolateFcn: Matrix xA could not be inverted. Something is "
                               "wrong with the x coordinates of points!"
                            << endl;
      assert(0);
    }

    vector<vector<RooNCSplineCore::T>> yA;
    getKappas(kappaY, 1);
    getAArray(kappaY, yA, bcBeginY, bcEndY);
    npoints = kappaY.size();
    TMatrix_t yAtrans(npoints, npoints);
    for (int i = 0; i < npoints; i++) {
      for (int j = 0; j < npoints; j++) {
        yAtrans[i][j] = yA.at(i).at(j);
      }
    }
    det = 0;
    TMatrix_t yAinv = yAtrans.Invert(&det);
    if (det == 0.) {
      coutE(InputArguments) << "RooNCSpline_3D_fast::interpolateFcn: Matrix yA could not be inverted. Something is "
                               "wrong with the y coordinates of points!"
                            << endl;
      assert(0);
    }

    vector<vector<RooNCSplineCore::T>> zA;
    getKappas(kappaZ, 2);
    getAArray(kappaZ, zA, bcBeginZ, bcEndZ);
    npoints = kappaZ.size();
    TMatrix_t zAtrans(npoints, npoints);
    for (int i = 0; i < npoints; i++) {
      for (int j = 0; j < npoints; j++) {
        zAtrans[i][j] = zA.at(i).at(j);
      }
    }
    det = 0;
    TMatrix_t zAinv = zAtrans.Invert(&det);
    if (det == 0.) {
      coutE(InputArguments) << "RooNCSpline_3D_fast::interpolateFcn: Matrix zA could not be inverted. Something is "
    ...
    int nybins = 0;
    int nypoldim = 0;
    std::vector<std::vector<std::vector<std::vector<std::vector<RooNCSplineCore::T>>>>>
    ...
    for (unsigned int k = 0; k < npointsZ(); k++) {
      ...
      std::vector<std::vector<std::vector<std::vector<RooNCSplineCore::T>>>>
      ...
      for (unsigned int j = 0; j < npointsY(); j++) {
        vector<vector<RooNCSplineCore::T>> xcoefsAtYjZk =
            getCoefficientsPerYPerZ(kappaX, xAinv, j, k, bcBeginX, bcEndX, -1);  // [ix][Ax,Bx,Cx,Dx] at each y_j z_k
        ...
        if (nxbins != (int)xcoefsAtYjZk.size() || nxpoldim != (int)xcoefsAtYjZk.at(0).size()) {
          coutE(InputArguments) << "RooNCSpline_3D_fast::interpolateFcn: nxbins!=(int)xcoefsAtYjZk.size() || "
                                   "nxpoldim!=(int)xcoefsAtYjZk.at(0).size()!"
                                << endl;
          assert(0);
        }
        for (int ix = 0; ix < nxbins; ix++) {
          for (int icx = 0; icx < nxpoldim; icx++)
            coefsAlongY.at(ix).at(icx).push_back(xcoefsAtYjZk.at(ix).at(icx));
        }
      }
      ...
      for (int ix = 0; ix < nxbins; ix++) {
        ...
        for (int icx = 0; icx < nxpoldim; icx++) {
          vector<vector<RooNCSplineCore::T>> yCoefs = getCoefficientsAlongDirection(
              kappaY, yAinv, coefsAlongY.at(ix).at(icx), bcBeginY, bcEndY, -1);  // [iy][A,B,C,D]
    ...
    for (int ix = 0; ix < nxbins; ix++) {
      ...
      for (int icx = 0; icx < nxpoldim; icx++) {
        ...
        for (int iy = 0; iy < nybins; iy++) {
          ...
          for (int icy = 0; icy < nypoldim; icy++) {
            vector<vector<RooNCSplineCore::T>> yCoefsAlongZ = getCoefficientsAlongDirection(
...
RooNCSpline_3D_fast::RooNCSpline_3D_fast(const RooNCSpline_3D_fast& other, const char* name)
    : RooNCSplineCore(other, name),
      rangeYmin(other.rangeYmin),
      rangeYmax(other.rangeYmax),
      rangeZmin(other.rangeZmin),
      rangeZmax(other.rangeZmax),
      bcBeginX(other.bcBeginX),
      bcEndX(other.bcEndX),
      bcBeginY(other.bcBeginY),
      bcEndY(other.bcEndY),
      bcBeginZ(other.bcBeginZ),
      bcEndZ(other.bcEndZ),
...
RooNCSplineCore::T RooNCSpline_3D_fast::interpolateFcn(Int_t code, const char* rangeName) const {
  ...
  if (verbosity == RooNCSplineCore::kVerbose)
    cout << "RooNCSpline_3D_fast(" << GetName() << ")::interpolateFcn begin with code: " << code << endl;

  // Get bins
  vector<Int_t> varprime;
  varprime.push_back(2);
  varprime.push_back(3);
  varprime.push_back(5);
  ...
  vector<const vector<RooNCSplineCore::T>*> varkappa;
  varkappa.push_back(&kappaX);
  varkappa.push_back(&kappaY);
  varkappa.push_back(&kappaZ);
  vector<const RooRealProxy*> varcoord;
  varcoord.push_back(&theXVar);
  varcoord.push_back(&theYVar);
  varcoord.push_back(&theZVar);
  for (Int_t idim = 0; idim < ndims; idim++) {
    ...
    if (code == 0 || code % varprime.at(idim) != 0) {
      if (!testRangeValidity(*(varcoord.at(idim)), idim))
        return 0;
      binval = getWhichBin(*(varcoord.at(idim)), idim);
      tval = getTVar(*(varkappa.at(idim)), *(varcoord.at(idim)), binval, idim);
    } else {
      RooNCSplineCore::T coordmin = varcoord.at(idim)->min(rangeName);
      cropValueForRange(coordmin, idim);
      RooNCSplineCore::T coordmax = varcoord.at(idim)->max(rangeName);
      cropValueForRange(coordmax, idim);
      ...
      for (int iy = 0; iy < (int)coefficients.at(ix).at(icx).size(); iy++) {
        ...
        for (int icy = 0; icy < (int)coefficients.at(ix).at(icx).at(iy).size(); icy++) {
          ...
          for (int iz = 0; iz < (int)coefficients.at(ix).at(icx).at(iy).at(icy).size(); iz++) {
            ...
            theYCoef += evalSplineSegment(coefficients.at(ix).at(icx).at(iy).at(icy).at(iz),
                                          varkappa[2]->at(iz),
                                          tzhigh,
                                          tzlow,
                                          (code > 0 && code % varprime[2] == 0));
          }
...
void RooNCSpline_3D_fast::getKappas(vector<RooNCSplineCore::T>& kappas, const Int_t whichDirection) {
  ...
  for (Int_t j = 0; j < npoints - 1; j++) {
    ...
    if (fabs(val_diff) > RooNCSplineCore::T(0))
      kappa = 1. / val_diff;
    else
      kappa = 0;
...
RooNCSplineCore::T RooNCSpline_3D_fast::getTVar(const vector<RooNCSplineCore::T>& kappas,
                                                const RooNCSplineCore::T& val,
                                                const Int_t& bin,
                                                const Int_t whichDirection) const {
...
vector<vector<RooNCSplineCore::T>> RooNCSpline_3D_fast::getCoefficientsPerYPerZ(
    const std::vector<RooNCSplineCore::T>& kappaX,
    const TMatrix_t& xAinv,
    const Int_t& ybin,
    const Int_t& zbin,
    RooNCSplineCore::BoundaryCondition const& bcBegin,
    RooNCSplineCore::BoundaryCondition const& bcEnd,
    const Int_t xbin) const {
  vector<RooNCSplineCore::T> fcnList;
  for (unsigned int bin = 0; bin < npointsX(); bin++) {
    fcnList.push_back(FcnList.at(zbin).at(ybin).at(bin));
  }
  vector<vector<RooNCSplineCore::T>> coefs =
      getCoefficientsAlongDirection(kappaX, xAinv, fcnList, bcBegin, bcEnd, xbin);
  return coefs;
}
...
Double_t RooNCSpline_3D_fast::evaluate() const {
  ...
  if (useFloor && value < floorEval) {
    if (verbosity >= RooNCSplineCore::kError)
      coutE(Eval) << "RooNCSpline_3D_fast ERROR::RooNCSpline_3D_fast(" << GetName() << ") evaluation returned " << value
                  << " at (x, y, z) = (" << theXVar << ", " << theYVar << ", " << theZVar << ")" << endl;
    value = floorEval;
  }
  if (verbosity == RooNCSplineCore::kVerbose) {
    cout << "RooNCSpline_3D_fast(" << GetName() << ")::evaluate = " << value << " at (x, y, z) = (" << theXVar << ", "
         << theYVar << ", " << theZVar << ")" << endl;
  }
  return value;
}
Int_t RooNCSpline_3D_fast::getAnalyticalIntegral(RooArgSet& allVars,
                                                 RooArgSet& analVars,
  ...
  if (rrv_x == 0)
    theXVar.absArg()->leafNodeServerList(&Xdeps, 0, true);
  if (rrv_y == 0)
    theYVar.absArg()->leafNodeServerList(&Ydeps, 0, true);
  if (rrv_z == 0)
    theZVar.absArg()->leafNodeServerList(&Zdeps, 0, true);

  if (rrv_x != 0) {
    if ((Ydeps.find(*rrv_x) == 0 || rrv_y != 0) && (Zdeps.find(*rrv_x) == 0 || rrv_z != 0)) {
      if (matchArgs(allVars, analVars, theXVar))
        code *= 2;
    }
  }
  if (rrv_y != 0) {
    if ((Xdeps.find(*rrv_y) == 0 || rrv_x != 0) && (Zdeps.find(*rrv_y) == 0 || rrv_z != 0)) {
      if (matchArgs(allVars, analVars, theYVar))
        code *= 3;
    }
  }
  if (rrv_z != 0) {
    if ((Xdeps.find(*rrv_z) == 0 || rrv_x != 0) && (Ydeps.find(*rrv_z) == 0 || rrv_y != 0)) {
      if (matchArgs(allVars, analVars, theZVar))
        code *= 5;
    }
  }
...
Double_t RooNCSpline_3D_fast::analyticalIntegral(Int_t code, const char* rangeName) const {
  ...
  if (useFloor && value < floorInt) {
    if (verbosity >= RooNCSplineCore::kError)
      coutE(Integration) << "RooNCSpline_3D_fast ERROR::RooNCSpline_3D_fast(" << GetName() << ") integration returned "
                         << value << " for code = " << code << endl;
    value = floorInt;
  }
  if (verbosity == RooNCSplineCore::kVerbose) {
    cout << "RooNCSpline_3D_fast(" << GetName() << ")::analyticalIntegral = " << value << " for code = " << code
         << endl;
  }
  return value;
}
...
void RooNCSpline_3D_fast::cropValueForRange(T& val, const Int_t whichDirection) const {
  if (testRangeValidity(val, whichDirection))
    return;
  const T* range[2];
  if (whichDirection == 0) {
    range[0] = &rangeXmin;
