      ...
      for (std::vector<Algo>::const_iterator it = fallbacks_.begin(), ed = fallbacks_.end(); it != ed; ++it) {
        ...
        if (nominalType != ROOT::Math::MinimizerOptions::DefaultMinimizerType() ||
          ...
          if (verbose > 0) {
            ...
            Logger::instance().log(
                std::string(Form(
                    "CascadeMinimizer.cc: %d -- Will fallback to minimization using %s, strategy %d and tolerance %g",
                    __LINE__,
                    (it->algo).c_str(),
                    myStrategy,
                    it->tolerance)),
...
bool CascadeMinimizer::improveOnce(int verbose, bool noHesse) {
  ...
  if (oldFallback_) {
    ...
    if (verbose + 2 > 0)
      Logger::instance().log(
          std::string(Form(
              "CascadeMinimizer.cc: %d -- Minimisation configured with Type=%s, Algo=%s, strategy=%d, tolerance=%g",
              __LINE__,
              myType.c_str(),
              myAlgo.c_str(),
              myStrategy,
...
bool CascadeMinimizer::multipleMinimize(const RooArgSet &reallyCleanParameters,
  ...
  if (mode == 1 || mode == 0)
    numberOfCombinations = myCombos.size();
  ...
  if (vm.count("cminFallbackAlgo")) {
    ...
    for (vector<string>::const_iterator it = falls.begin(), ed = falls.end(); it != ed; ++it) {
      ...
      std::cout << "Configured fallback algorithm "
                << ", type " << fallbacks_.back().type << ", algo " << fallbacks_.back().algo << ", strategy "
                << fallbacks_.back().strategy << ", tolerance " << fallbacks_.back().tolerance << std::endl;
    }
  }
