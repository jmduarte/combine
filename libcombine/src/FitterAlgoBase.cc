...
double FitterAlgoBase::findCrossing(
  ...
  if (verbose) {
    std::cout << "Searching for crossing at nll = " << level << " in the interval " << rStart << ", " << rBound
              << std::endl;
    Logger::instance().log(
        std::string(Form("FitterAlgoBase.cc: %d -- Searching for crossing at nll = %g, in the interval %g < %s < %g",
                         __LINE__,
                         level,
                         rStart,
                         r.GetName(),
  ...
  if (verbose > 0) {
    ...
    fflush(stdout);
    Logger::instance().log(
        std::string(Form("FitterAlgoBase.cc: %d -- %s lvl-here lvl-there 	stepping ", __LINE__, r.GetName())),
        Logger::kLogLevelInfo,
        __func__);
  }
  do {
    ...
    if (!ok && !keepFailures_) {
      ...
      if (nfail >= maxFailedSteps_) {
        ...
        if (verbose)
          Logger::instance().log(
              std::string(Form(
                  "FitterAlgoBase.cc: %d -- Maximum failed steps (max=%d) reached and Minimization failed at %s = %g ",
                  __LINE__,
                  maxFailedSteps_,
                  r.GetName(),
                  rStart)),
    ...
    if (verbose > 0) {
      ...
      Logger::instance().log(
          std::string(Form(
              "FitterAlgoBase.cc: %d -- %f    %+.5f  %+.5f    %f", __LINE__, rStart, level - here, level - there, rInc)),
          Logger::kLogLevelInfo,
          __func__);
    }
...
double FitterAlgoBase::findCrossingNew(
  ...
  if (verbose) {
    ...
    Logger::instance().log(
        std::string(Form("FitterAlgoBase.cc: %d -- Searching for crossing at nll = %g in the interval [ %g , %g ] ",
                         __LINE__,
                         level,
                         rStart,
                         rBound)),
  ...
  for (int iter = 0; iter < 20; ++iter) {
    ...
    if (nll.numEvalErrors() > 0 || std::isnan(yStart) || std::isinf(yStart)) {
      ...
      if (verbose > 0)
        Logger::instance().log(
            std::string(Form(
                "FitterAlgoBase.cc: %d -- logEvalErrors reported from NLL on start of loop for iteration %d, x %+10.6f",
                __LINE__,
                iter,
                rVal)),
    ...
    if (verbose > 1) {
      ...
      Logger::instance().log(
          std::string(
              Form("FitterAlgoBase.cc: %d -- x %+10.6f   y %+10.6f    step %+10.6f [ START OF ITER %d, bound %+10.6f ]",
                   __LINE__,
                   rVal,
                   yStart - level,
                   rInc,
    ...
    if (verbose > 1) {
      ...
      Logger::instance().log(
          std::string(Form("FitterAlgoBase.cc: %d x %+10.6f   y %+10.6f   yCorr %+10.6f   yProf  %+10.6f   (P-U) "
    ...
    if ((level - yStart) * (level - yProf) > 0) {
      ...
      if (hitbound) {
        ...
        if (verbose > 1) {
          ...
          Logger::instance().log(
              std::string(
                  Form("FitterAlgoBase.cc: %d -- ---> change search window to [ %g , %g ]", __LINE__, rStart, rBound)),
              Logger::kLogLevelInfo,
              __func__);
        }
      }
