...
rVrFLikelihood::rVrFLikelihood(const char *name, const char *title) : RooAbsReal(name, title) {}

void rVrFLikelihood::addChannel(const TH2 *chi2, RooAbsReal &muV, RooAbsReal &muF) {
  channels_.push_back(Channel(this, chi2, muV, muF));
}

rVrFLikelihood::~rVrFLikelihood() {}

TObject *rVrFLikelihood::clone(const char *newname) const {
...
Double_t rVrFLikelihood::evaluate() const {
  ...
  for (std::vector<Channel>::const_iterator it = channels_.begin(), ed = channels_.end(); it != ed; ++it) {
    double x = it->muF;
    double y = it->muV;
    if (!(x > it->chi2->GetXaxis()->GetXmin()))
      return 9999;
    if (!(x < it->chi2->GetXaxis()->GetXmax()))
      return 9999;
    if (!(y > it->chi2->GetYaxis()->GetXmin()))
      return 9999;
    if (!(y < it->chi2->GetYaxis()->GetXmax()))
      return 9999;
