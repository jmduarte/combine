...
ClassImp(HZZ4L_RooCTauPdf_2D)
    ...
    HZZ4L_RooCTauPdf_2D::HZZ4L_RooCTauPdf_2D(const char* name,
                                             const char* title,
                                             RooAbsReal& _kd,
                                             RooAbsReal& _ksmd,
                                             RooAbsReal& _ctau,
  ...
  while ((func = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "ERROR: :HZZ4L_RooCTauPdf_2D(" << GetName() << ") funcficient " << func->GetName()
  ...
  for (int mp = 0; mp < nbins_ctau; mp++)
    Integral_T[mp] = dynamic_cast<const RooHistFunc*>(_coefList.at(mp))->analyticalIntegral(1000);
}

HZZ4L_RooCTauPdf_2D::HZZ4L_RooCTauPdf_2D(const HZZ4L_RooCTauPdf_2D& other, const char* name)
...
Int_t HZZ4L_RooCTauPdf_2D::getAnalyticalIntegral(RooArgSet& allVars,
                                                 RooArgSet& analVars,
                                                 const char* /*rangeName*/) const {
  if (matchArgs(allVars, analVars, RooArgSet(*kd.absArg(), *ksmd.absArg())))
    return 1;
  return 0;
}
