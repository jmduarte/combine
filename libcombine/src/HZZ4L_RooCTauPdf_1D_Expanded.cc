...
ClassImp(HZZ4L_RooCTauPdf_1D_Expanded)
    ...
    HZZ4L_RooCTauPdf_1D_Expanded::HZZ4L_RooCTauPdf_1D_Expanded(const char* name,
                                                               const char* title,
                                                               RooAbsReal& _kd,
                                                               RooAbsReal& _ctau,
  ...
  while ((coef = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(coef)) {
      coutE(InputArguments) << "ERROR::HZZ4L_RooCTauPdf_1D_Expanded(" << GetName() << ") coefficient "
  ...
  if (nCoef > 2) {
    coutE(InputArguments) << "ERROR::HZZ4L_RooCTauPdf_1D_Expanded(" << GetName() << ") number coefficients " << nCoef
  ...
  while ((func = (RooAbsArg*)funcIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "ERROR::HZZ4L_RooCTauPdf_1D_Expanded(" << GetName() << ") function " << func->GetName()
  ...
  if (nLinearVariations > nCoef) {
    coutE(InputArguments) << "ERROR::HZZ4L_RooCTauPdf_1D_Expanded(" << GetName() << ") number of linear variations ("
                          << nLinearVariations << ") is more than the number of coefficients (" << nCoef << ")!!!"
                          << endl;
    assert(0);
  }
  ...
  for (int mp = 0; mp < integralArraySize; mp++)
    Integral_T[mp] = dynamic_cast<const RooHistFunc*>(_funcList.at(mp))->analyticalIntegral(1000);
  for (int mp = nbins_ctau; mp < (9 * 101); mp++)
    Integral_T[mp] = Integral_T[nbins_ctau - 1];
...
HZZ4L_RooCTauPdf_1D_Expanded::HZZ4L_RooCTauPdf_1D_Expanded(const HZZ4L_RooCTauPdf_1D_Expanded& other, const char* name)
  ...
  int nTrueSyst = nCoef - nLinearVariations;
  nbins_ctau = nFuncs / ((2 * nTrueSyst + 1) * (2 * nLinearVariations + 1));
  for (int mp = 0; mp < (9 * 101); mp++)
    Integral_T[mp] = (other.Integral_T)[mp];
}
...
Double_t HZZ4L_RooCTauPdf_1D_Expanded::interpolateBin() const {
  ...
  for (int ss = 0; ss < (2 * nLinearVariations + 1); ss++) {
    ...
    for (int iarr = 0; iarr < 2; iarr++)
      iArray[iarr] = iArray[iarr] + addIndex;
...
Double_t HZZ4L_RooCTauPdf_1D_Expanded::interpolateVariation(Double_t theta_,
                                                            Double_t valueCenter_,
                                                            Double_t valueHigh_,
                                                            Double_t valueLow_) const {
  if (smoothAlgo < 0)
    return 0;
  else {
    if (fabs(theta_) >= smoothRegion)
      return theta_ * (theta_ > 0 ? valueHigh_ - valueCenter_ : valueCenter_ - valueLow_);
...
Int_t HZZ4L_RooCTauPdf_1D_Expanded::getAnalyticalIntegral(RooArgSet& allVars,
                                                          RooArgSet& analVars,
                                                          const char* /*rangeName*/) const {
  if (matchArgs(allVars, analVars, RooArgSet(*kd.absArg())))
    return 1;
  return 0;
}
