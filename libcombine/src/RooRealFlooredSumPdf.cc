...
RooRealFlooredSumPdf::RooRealFlooredSumPdf(
  ...
  while ((coef = (RooAbsArg*)coefIter->Next())) {
    ...
    if (!dynamic_cast<RooAbsReal*>(coef)) {
      coutW(InputArguments) << "RooRealFlooredSumPdf::RooRealFlooredSumPdf(" << GetName() << ") coefficient "
                            << coef->GetName() << " is not of type RooAbsReal, ignored" << endl;
      continue;
    }
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutW(InputArguments) << "RooRealFlooredSumPdf::RooRealFlooredSumPdf(" << GetName() << ") func "
  ...
  if (func) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "RooRealFlooredSumPdf::RooRealFlooredSumPdf(" << GetName() << ") last func "
...
Double_t RooRealFlooredSumPdf::evaluate() const {
  ...
  while ((coef = (RooAbsReal*)coefIter.next())) {
    ...
    if (coefVal) {
      cxcoutD(Eval) << "RooRealFlooredSumPdf::eval(" << GetName() << ") coefVal = " << coefVal
  ...
  if (!_haveLastCoef) {
    ...
    cxcoutD(Eval) << "RooRealFlooredSumPdf::eval(" << GetName() << ") lastCoef = " << lastCoef
...
Bool_t RooRealFlooredSumPdf::checkObservables(const RooArgSet* nset) const {
  ...
  while ((coef = (RooAbsReal*)_coefIter->Next())) {
    ...
    if (func->observableOverlaps(nset, *coef)) {
      coutE(InputArguments) << "RooRealFlooredSumPdf::checkObservables(" << GetName() << "): ERROR: coefficient "
...
Double_t RooRealFlooredSumPdf::analyticalIntegralWN(Int_t code,
  ...
  if (result <= 0. && _doFloor) {
    coutW(Eval) << "RooRealFlooredSumPdf::integral(" << GetName() << ") WARNING: Integral " << result
