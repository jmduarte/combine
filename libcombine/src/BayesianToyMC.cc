...
void BayesianToyMC::applyOptions(const boost::program_options::variables_map &vm) {
  if (!withSystematics) {
    std::cout << "BayesianToyMC: when running with no systematics, BayesianToyMC is identical to BayesianSimple."
              << std::endl;
    tries_ = 1;
    numIters_ = 0;
    integrationType_ = "";
...
bool BayesianToyMC::run(RooWorkspace *w,
  ...
  for (;;) {
    ...
    for (unsigned int i = 0; i < tries_; ++i) {
      ...
      // check against bound
      if (lim >= 0.5 * r->getMax()) {
        std::cout << "Limit " << r->GetName() << " < " << lim << "; " << r->GetName() << " max < " << r->getMax()
                  << std::endl;
        if (r->getMax() / rMax > 20)
          return false;
        r->setMax(r->getMax() * 2);
...
bool BayesianToyMC::runBayesFactor(RooWorkspace *w,
                                   RooStats::ModelConfig *mc_s,
                                   RooStats::ModelConfig *mc_b,
                                   RooAbsData &data,
                                   double &limit,
  ...
  if (verbose > -1) {
    ...
    std::cout << "    (min(1/BF,BF) expressed as sigma's: "
              << ROOT::Math::normal_quantile_c(std::min(limit, 1.0 / limit), 1.0) << ")" << std::endl;
  }
  return true;
}

std::pair<double, double> BayesianToyMC::priorPredictiveDistribution(RooStats::ModelConfig *mc,
                                                                     RooAbsData &data,
  ...
  for (unsigned int t = 0; t < tries_; ++t) {
    ...
    if (poiToGen.getSize() > 0) {
      if (mc->GetPriorPdf() == 0)
        throw std::logic_error(std::string("Missing prior in model: ") + mc->GetName());
      poiValues.reset(mc->GetPriorPdf()->generate(poiToGen, numIters_));
    }
    for (int i = 0; i < numIters_; ++i) {
      if (nuisanceValues.get() != 0)
        *params = *nuisanceValues->get(i);
      if (poiValues.get() != 0)
