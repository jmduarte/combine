...
VBFHZZ4L_RooSpinZeroPdf_fast::VBFHZZ4L_RooSpinZeroPdf_fast(const char* name,
                                                           const char* title,
                                                           RooAbsReal& in_a1,
                                                           RooAbsReal& in_ai1,
  ...
  while ((coef = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(coef)) {
      coutE(InputArguments) << "VBFHZZ4L_RooSpinZeroPdf_fast(" << GetName() << ") observable " << coef->GetName()
  ...
  while ((coef = (RooAbsArg*)coefIter->Next())) {
    if (dynamic_cast<FastTemplateFunc_f*>(coef) == 0 && dynamic_cast<FastTemplateFunc_d*>(coef) == 0) {
      coutE(InputArguments) << "VBFHZZ4L_RooSpinZeroPdf_fast(" << GetName() << ") component " << coef->GetName()
...
VBFHZZ4L_RooSpinZeroPdf_fast::VBFHZZ4L_RooSpinZeroPdf_fast(const VBFHZZ4L_RooSpinZeroPdf_fast& other, const char* name)
...
Float_t VBFHZZ4L_RooSpinZeroPdf_fast::interpolateFcn(Int_t code, const char* rangeName) const {
  ...
  if (coefList.getSize() != (Int_t)coefs.size()) {
    cerr << "VBFHZZ4L_RooSpinZeroPdf_fast::interpolateFcn: coefList.getSize()=" << coefList.getSize()
...
Int_t VBFHZZ4L_RooSpinZeroPdf_fast::getAnalyticalIntegral(RooArgSet& allVars,
                                                          RooArgSet& analVars,
