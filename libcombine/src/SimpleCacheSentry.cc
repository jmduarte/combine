...
SimpleCacheSentry::SimpleCacheSentry(const RooRealVar &var) : _deps("deps", "deps", this) { addVar(var); }

SimpleCacheSentry::SimpleCacheSentry(const RooAbsCollection &vars) : _deps("deps", "deps", this) { addVars(vars); }

SimpleCacheSentry::SimpleCacheSentry(const RooAbsArg &func, const RooArgSet *obs) : _deps("deps", "deps", this) {
  addFunc(func, obs);
}
...
Bool_t SimpleCacheSentry::isIdentical(const RooAbsArg &other, Bool_t /*assumeSameType*/) {
  bool ret = kFALSE;
  ...
  if ((common->getSize() == otherSentry._deps.getSize()) && (common->getSize() == _deps.getSize()))
    ret = kTRUE;
  delete common;
  return ret;
}
