...
void HybridNew::validateOptions() {
  ...
  if (testStat_ != "LEP" && testStat_ != "TEV" && testStat_ != "LHC" && testStat_ != "LHCFC" &&
    ...
    throw std::invalid_argument(
        "HybridNew: Test statistics should be one of 'LEP' or 'TEV' or 'LHC' (previously known as 'Atlas') or "
        "'Profile'");
  }
  if (testStat_ == "LEP")
...
void HybridNew::readGrid(TDirectory *toyDir, double rMin, double rMax) {
  ...
  while ((k = (TKey *)next()) != 0) {
    ...
    if (name.Index("HypoTestResult_mh") == 0) {
      ...
      std::cout << "HybridNew::readGrid: HypoTestResult with non-conformant name " << name << " will be skipped"
                << std::endl;
      if (verbose)
