...
namespace {
  ...
  template <typename T>
  void sumdiff(
      T *__restrict__ sum, T *__restrict__ diff, unsigned int n, const T *__restrict__ h1, const T *__restrict__ h2) {
  ...
  template <typename T>
  void meld(T *__restrict__ out, unsigned int n, T const *__restrict__ diff, T const *__restrict__ sum, T x, T y) {
    for (unsigned int i = 0; i < n; ++i) {
...
template <typename T>
void FastTemplate_t<T>::SumDiff(const FastTemplate_t &h1,
                                const FastTemplate_t &h2,
                                FastTemplate_t &sum,
                                FastTemplate_t &diff) {
...
template <typename T> void FastTemplate_t<T>::CropUnderflows(T minimum, bool activebinsonly) {
  for (unsigned int i = 0, n = activebinsonly ? this->size_ : (this->values_).size(); i < n; ++i) {
    if ((this->values_)[i] < minimum)
      (this->values_)[i] = minimum;
  }
}
...
template <typename T> void FastTemplate_t<T>::CopyValues(const TH1 &other) {
  for (unsigned int i = 0; i < this->size_; ++i)
    (this->values_)[i] = other.GetBinContent(i + 1);
}
...
template <typename T> void FastTemplate_t<T>::CopyValues(const TH3 &other) {
  for (unsigned int i = 0, ix = 1, nx = other.GetNbinsX(), ny = other.GetNbinsY(), nz = other.GetNbinsZ(); ix <= nx;
       ++ix) {
...
template <typename T> void FastTemplate_t<T>::Dump() const {
  printf("--- dumping template with %d bins (%d active bins) (@%p) ---\n",
         int((this->values_).size()),
         this->size_,
         (void *)(&(this->values_)[0]));
  for (unsigned int i = 0; i < (this->values_).size(); ++i)
    printf(" bin %3d: yval = %9.5f\n", i, (this->values_)[i]);
  printf("\n");
}
...
FastHisto_t<T, U>::FastHisto_t(const TH1 &hist, bool normX)
    : FastTemplate_t<T>(hist), axis_(*(hist.GetXaxis())), normX_(normX) {}
template <typename T, typename U>
FastHisto_t<T, U>::FastHisto_t(const FastHisto_t &other)
    : FastTemplate_t<T>(other), axis_(other.axis_), normX_(other.normX_) {}
...
template <typename T, typename U> T FastHisto_t<T, U>::IntegralWidth(int binmin, int binmax) const {
  ...
  for (unsigned int i = 0; i < std::min(GetNbinsX(), this->size_); ++i) {
    if (binmin >= 0 && (int)i < binmin)
      continue;
    if (binmax >= 0 && (int)i > binmax)
      continue;
...
template <typename T, typename U>
FastHisto2D_t<T, U>::FastHisto2D_t(const TH2 &hist, bool normX, bool normY)
    : FastTemplate_t<T>(hist), axisX_(*(hist.GetXaxis())), axisY_(*(hist.GetYaxis())), normX_(normX), normY_(normY) {}

template <typename T, typename U>
FastHisto2D_t<T, U>::FastHisto2D_t(const FastHisto2D_t &other)
...
template <typename T, typename U>
T FastHisto2D_t<T, U>::IntegralWidth(int xbinmin, int xbinmax, int ybinmin, int ybinmax) const {
  DefaultAccumulator<T> total = 0;
  const unsigned int binX_ = GetNbinsX();
  const unsigned int binY_ = GetNbinsY();
  for (unsigned int ix = 0; ix < binX_; ix++) {
    if (xbinmin >= 0 && (int)ix < xbinmin)
      continue;
    if (xbinmax >= 0 && (int)ix > xbinmax)
      continue;

    for (unsigned int iy = 0; iy < binY_; iy++) {
      if (ybinmin >= 0 && (int)iy < ybinmin)
        continue;
      if (ybinmax >= 0 && (int)iy > ybinmax)
        continue;
  ...
  for (unsigned int ix = 0; ix < binX_; ix++) {
    for (unsigned int iy = 0; iy < binY_; iy++) {
      ...
      printf(" bin %3d, x = %6.2f, y = %6.2f: yval = %9.5f, width = %6.3f\n",
             i,
             0.5 * (axisX_[ix] + axisX_[ix + 1]),
             0.5 * (axisY_[iy] + axisY_[iy + 1]),
             (this->values_)[i],
             GetBinWidthX(ix) * GetBinWidthY(iy));
    }
  }
  printf("\n");
}
...
template <typename T, typename U>
FastHisto3D_t<T, U>::FastHisto3D_t(const TH3 &hist, bool normX, bool normY, bool normZ)
...
template <typename T, typename U>
FastHisto3D_t<T, U>::FastHisto3D_t(const FastHisto3D_t &other)
    : FastTemplate_t<T>(other),
      axisX_(other.axisX_),
      axisY_(other.axisY_),
      axisZ_(other.axisZ_),
...
template <typename T, typename U>
T FastHisto3D_t<T, U>::IntegralWidth(
    int xbinmin, int xbinmax, int ybinmin, int ybinmax, int zbinmin, int zbinmax) const {
  DefaultAccumulator<T> total = 0;
  const unsigned int binX_ = GetNbinsX();
  const unsigned int binY_ = GetNbinsY();
  ...
  for (unsigned int ix = 0; ix < binX_; ix++) {
    ...
    for (unsigned int iy = 0; iy < binY_; iy++) {
      ...
      for (unsigned int iz = 0; iz < binZ_; iz++) {
        if (zbinmin >= 0 && (int)iz < zbinmin)
          continue;
        if (zbinmax >= 0 && (int)iz > zbinmax)
          continue;

        total +=
            (this->values_)[(ix * binY_ + iy) * binZ_ + iz] * GetBinWidthX(ix) * GetBinWidthY(iy) * GetBinWidthZ(iz);
      }
    }
  }
  return total.sum();
    ...
    for (unsigned int iy = 0; iy < binY_; iy++) {
      for (unsigned int iz = 0; iz < binZ_; iz++) {
        ...
        printf(" bin %3d, x = %6.2f, y = %6.2f, z = %6.2f : zval = %9.5f, width = %6.3f\n",
               ...
               (this->values_)[i],
               GetBinWidthX(ix) * GetBinWidthY(iy) * GetBinWidthZ(iz));
      }
    }
  }
  printf("\n");
