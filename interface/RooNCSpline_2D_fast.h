...
protected:
  ...
  std::vector<std::vector<std::vector<std::vector<T>>>>
      coefficients;  // [ix][A_x,B_x,C_x,D_x][iy][A_x_y,B_x_y,C_x_y,D_x_y]

public:
  RooNCSpline_2D_fast();
  RooNCSpline_2D_fast(const char* name, const char* title);
  RooNCSpline_2D_fast(const char* name,
                      const char* title,
                      RooAbsReal& inXVar,
                      RooAbsReal& inYVar,
...
protected:
  ...
  virtual std::vector<std::vector<T>> getCoefficientsPerY(
      const std::vector<T>& kappaX,
      const TMatrix_t& xAinv,
      const Int_t& ybin,
      RooNCSplineCore::BoundaryCondition const& bcBegin,
      RooNCSplineCore::BoundaryCondition const& bcEnd,
      const Int_t xbin) const;  // xbin can be -1, which means push all of them
