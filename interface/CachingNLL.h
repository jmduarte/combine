...
namespace cacheutils {
  ...
  template <typename PdfT, typename VPdfT> class OptimizedCachingPdfT : public CachingPdf {
  public:
    OptimizedCachingPdfT(RooAbsReal *pdf, const RooArgSet *obs) : CachingPdf(pdf, obs), vpdf_(0) {}
    OptimizedCachingPdfT(const OptimizedCachingPdfT &other) : CachingPdf(other), vpdf_(0) {}
  ...
  public:
    CachingAddNLL(
        const char *name, const char *title, RooAbsPdf *pdf, RooAbsData *data, bool includeZeroWeights = false);
