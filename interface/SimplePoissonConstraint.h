...
public:
  ...
  SimplePoissonConstraint(
      const char* name, const char* title, RooAbsReal& _x, RooAbsReal& _mean, bool noRounding = false)
      : RooPoisson(name, title, _x, _mean, noRounding) {
    init();
  }
  SimplePoissonConstraint(const SimplePoissonConstraint& other, const char* name = 0) : RooPoisson(other, name) {
    init();
  }
