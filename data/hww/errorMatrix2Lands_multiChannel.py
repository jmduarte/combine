...
parser.add_option(
    "-g", "--gamma", dest="gamma", default=False, action="store_true", help="Use gammas for statistical uncertainties"
)
parser.add_option(
    "--gmM",
    dest="gmM",
...
parser.add_option(
    "-L", "--label-syst", dest="labelSyst", default=False, action="store_true", help="Give names to systematics"
)
...
if options.optimize:
    for mh in data.keys():
        ...
        for p in range(nproc):
            if hasrate[p] == 0:
                print("For mH = ", mh, " process ", processnames[p], " does not contribute.")
    ...
    for (inuis, N) in enumerate(D["nuis"]):
        ...
        if options.gamma and isStat:
            ...
            fout.write(
                name
                + "  gmN %7d " % realN
                + (" ".join([("%7.5f" % X if X else "   0   ") for X in alphas]))
                + "     "
                + D["nuisname"][inuis]
                + "\n"
            )
