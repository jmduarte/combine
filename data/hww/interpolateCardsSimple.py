...
argv.remove("-b-")
parser = OptionParser(
    usage="usage: %prog [options] [mass1 mass2] mass3 > datacard3.txt \nrun with --help to get list of options"
)
...
os.system(
    ...
    % (options.ddir, mass1, options.jets, options.ddir, mass, options.jets, options.postfix)
)
...
if DC1.processes != DC2.processes:
    raise RuntimeError(
        "The two datacards have different processes: %s has %s, %s has %s"
        % (file1, DC1.processes, file2, DC2.processes)
    )
            ...
            if b in errline:
                if p in errline[b]:
                    if options.log and pdf == "gmN" and errline[b][p] != 0:
                        errline[b][p] = exp(
                            alpha * log(systlines[name][2][b][p]) + beta * log(systlines2[name][2][b][p])
                        )
                    else:
...
for name in sysnamesSorted:
    ...
    for b, p, s in keyline:
        try:
            ...
            systline.append("-")
    xfile.write(
        " ".join([hfmt % ("%-28s   %s  %s" % (name, pdf, " ".join(pdfargs))), "  ".join([cfmt % x for x in systline])])
        + "\n"
    )
