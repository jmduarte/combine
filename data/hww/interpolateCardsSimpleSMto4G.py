...
argv.remove("-b-")
parser = OptionParser(
    usage="usage: %prog [options] [refmass1] mass3 > datacard3.txt \nrun with --help to get list of options"
)
...
if len(args) == 1:
    ...
    for m in refmasses[1:]:
        if abs(mass1 - mass) > abs(m - mass) or (
            abs(mass1 - mass) == abs(m - mass) and abs(mass1 - 164) < abs(m - 164)
        ):
            mass1 = m
else:
    mass1 = int(args[0])
...
for X in ["hwwof_0j_shape", "hwwof_1j_shape", "hwwsf_0j_shape", "hwwsf_1j_shape", "hww_2j_cut"]:
    ...
    if "shape" in X:
        XS = X.replace("_shape", "")
        os.system(
            "cp %s/%d/%s.input.root  %s/%g/SM4_%s%s.input.root"
            % (options.ddir, mass1, XS, options.ddir, mass, XS, options.postfix)
        )
    ...
    for name in sysnamesSorted:
        ...
        xfile.write(
            " ".join(
                [hfmt % ("%-28s   %s  %s" % (name, pdf, " ".join(pdfargs))), "  ".join([cfmt % x for x in systline])]
            )
            + "\n"
        )
