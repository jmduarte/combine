...
double minimizeFloatR(int Nobs, double &r, double &S, double S_In, double tolerance = 0.001) {
  ...
  double ru = r + 0.5 * r_step;
  if (ru > r_max)
    ru = r_max;
  double rd = r - 0.5 * r_step;
  if (rd < 0)
    rd = 0;
...
void toyByHand(double r0 = 1.0, int toys = 500, int seed = 1) {
  ...
  std::cout << "Log-normal uncertainty on " << (uncertainty_on_b ? "background" : "signal") << ", kappa: " << kappa
            << std::endl;
...
int main(int argc, char **argv) {
  toyByHand(atof(argc > 1 ? argv[1] : "1.0"), atoi(argc > 2 ? argv[2] : "500"), atoi(argc > 3 ? argv[3] : "1"));
  return 0;
}
