...
void importSet(TString setName, RooWorkspace *comb, const RooArgSet *atlas, const RooArgSet *cms) {
  ...
  while ((arg = (RooAbsArg *)it->Next()) != 0) {
    ...
    if (name == "LUMI" || name == "XS_GG" || name == "LUMI_mean" || name == "XS_GG_mean")
      continue;
    RooAbsArg *renamed = comb->var(name + "_atlas");
    if (renamed == 0) {
      std::cerr << "Error: " << name << " of type " << arg->ClassName() << " was not imported from atlas." << std::endl;
      continue;
    }
    set.add(*renamed);
  }
  ...
  while ((arg = (RooAbsArg *)it->Next()) != 0) {
    ...
    if (name == "theta_Lumi" || name == "theta_Higgs_XS" || name == "thetaIn_Lumi" || name == "thetaIn_Higgs_XS") {
      ...
      if (renamed == 0) {
        std::cerr << "Error: " << name << " of type " << arg->ClassName() << " was not imported from cms." << std::endl;
        continue;
      }
      set.add(*renamed);
    } else {
...
void mkModelObsPdf(RooWorkspace *w, TString model = "s", TString inputpdf = "model_") {
  ...
  RooProdPdf *modelObs_s =
      new RooProdPdf("modelObs_" + model, "Part of " + inputpdf + model + " that depends on observables", factors);
  w->import(*modelObs_s);
...
void mkCombModel(int mass = 140) {
  ...
  w->import(*mC->GetPdf(),
            RenameAllNodes("cms"),
            RenameAllVariablesExcept("cms", "r,theta_Lumi,thetaIn_Lumi,theta_Higgs_XS,thetaIn_Higgs_XS"));
  w->Print("V");
  ...
  w->factory(
      TString::Format("PROD::_naive_model_s(%s_atlas,%s_cms)", mA->GetPdf()->GetName(), mC->GetPdf()->GetName()));
  ...
  if (useReplace)
    model_s->SetName("model_s");  // this is directly the S+B model
  else
    model_s->SetName("modelDup_s");  // this is S+B model with duplicate constraints
  w->import(*model_s);
  ...
  RooConstVar *zorro = new RooConstVar("__zero__", "Zero", 0);
  w->import(*zorro);
