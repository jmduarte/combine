...
void setConstPars(RooAbsPdf *pdf, RooAbsData *data, bool setConst) {
  ...
  for (TObject *a = iter->Next(); a != 0; a = iter->Next()) {
    if (!a->InheritsFrom("RooRealVar"))
      continue;  // dynamic_cast in CINT doesn't work well
    RooRealVar *rrv = dynamic_cast<RooRealVar *>(a);
    if (rrv) {
      rrv->setConstant(setConst);
    }
  }
  delete iter;
  delete params;
...
void reparamCrop(double low = 130, double high = 150, double veryLow = 100, double veryHigh = 180, int ncat = 4) {
  ...
  for (int c = 0; c < ncat; ++c) {
    ...
    wAll->import(*ws[c]->pdf("MggBkg" + postfix),
                 RenameAllVariablesExcept("CMS", "mass"),
                 RenameAllNodes("CMS"),
                 RenameVariable("mass", "CMS_mgg"));
    wAll->import(*ws[c]->pdf("MggSig" + postfix),
                 RenameAllVariablesExcept("CMS", "mass"),
                 RenameAllNodes("CMS"),
                 RenameVariable("mass", "CMS_mgg"));
    ...
    sframe->Draw();
    c1->Print(TString::Format("fit_sig_cat%d.png", c));
    ...
    RooAddPdf bgModel("bgModel",
                      "bgModel",
                      RooArgList(*wAll->pdf("MggBkg" + postfix + "_CMS")),
                      RooArgList(*wAll->var("nB" + postfix)));
    ...
    frame->Draw();
    c1->Print(TString::Format("fit_cat%d.png", c));
  }
  //wAll->Print("V");
  ...
  wAll->factory(
      "sum::CMS_hgg_sig_m0_cat1(mgg_sig_m0_cat1_CMS, CMS_hgg_sig_m0_absShiftEBEB, CMS_hgg_sig_m0_absShiftBadR9)");
  wAll->factory("sum::CMS_hgg_sig_m0_cat2(mgg_sig_m0_cat2_CMS, CMS_hgg_sig_m0_absShiftEEEX)");
  wAll->factory(
      "sum::CMS_hgg_sig_m0_cat3(mgg_sig_m0_cat3_CMS, CMS_hgg_sig_m0_absShiftEEEX, CMS_hgg_sig_m0_absShiftBadR9)");
  ...
  wAll->factory(
      "prod::CMS_hgg_sig_sigma_cat1(mgg_sig_sigma_cat1_CMS, CMS_hgg_sig_sigmaScaleEBEB, CMS_hgg_sig_sigmaScaleBadR9)");
  wAll->factory("prod::CMS_hgg_sig_sigma_cat2(mgg_sig_sigma_cat2_CMS, CMS_hgg_sig_sigmaScaleEEEX)");
  wAll->factory(
      "prod::CMS_hgg_sig_sigma_cat3(mgg_sig_sigma_cat3_CMS, CMS_hgg_sig_sigmaScaleEEEX, CMS_hgg_sig_sigmaScaleBadR9)");
  wAll->factory("prod::CMS_hgg_sig_gsigma_cat0(mgg_sig_gsigma_cat0_CMS, CMS_hgg_sig_sigmaScaleEBEB)");
  wAll->factory(
      "prod::CMS_hgg_sig_gsigma_cat1(mgg_sig_gsigma_cat1_CMS, CMS_hgg_sig_sigmaScaleEBEB, "
      "CMS_hgg_sig_sigmaScaleBadR9)");
  wAll->factory("prod::CMS_hgg_sig_gsigma_cat2(mgg_sig_gsigma_cat2_CMS, CMS_hgg_sig_sigmaScaleEEEX)");
  wAll->factory(
      "prod::CMS_hgg_sig_gsigma_cat3(mgg_sig_gsigma_cat3_CMS, CMS_hgg_sig_sigmaScaleEEEX, "
      "CMS_hgg_sig_sigmaScaleBadR9)");

  for (int c = 0; c < ncat; ++c) {
    wAll->factory(TString::Format("CMS_hgg_bkg_slope_cat%d[0]", c));
    wAll->var(TString::Format("CMS_hgg_bkg_slope_cat%d", c))
        ->setVal(wAll->var(TString::Format("mgg_bkg_slope_cat%d_CMS", c))->getVal());
    wAll->var(TString::Format("CMS_hgg_bkg_slope_cat%d", c))
        ->setError(wAll->var(TString::Format("mgg_bkg_slope_cat%d_CMS", c))->getError());
  }

  // (3) do reparametrization of signal
  for (int c = 0; c < ncat; ++c) {
    wAll->factory(TString::Format("EDIT::CMS_hgg_sig_cat%d(MggSig_cat%d_CMS,", c, c) +
    ...
    wAll->factory(TString::Format("EDIT::CMS_hgg_bkg_cat%d(MggBkg_cat%d_CMS,", c, c) +
  ...
  for (int c = 0; c < ncat; ++c) {
    ...
    RooAddPdf bgModel("bgModel",
                      "bgModel",
                      *wAll->pdf("CMS_hgg_sig" + postfix),
                      *wAll->pdf("CMS_hgg_bkg" + postfix),
                      *wAll->var("sFrac"));
    ...
    frame->Draw();
    c1->Print(TString::Format("fit_data_cat%d.png", c));
  }
  ...
  std::cout << "=================================================================================================="
            << std::endl;
  std::cout << "=== LINES TO PUT IN THE DATACARD ================================================================="
            << std::endl;
  std::cout << "=================================================================================================="
            << std::endl;
  ...
  for (int c = 0; c < ncat; ++c) {
    TString postfix = TString::Format("_cat%d", c);
    printf("%4.2f      %5.1f      ", wAll->var("nS" + postfix)->getVal(), wAll->var("nB" + postfix)->getVal());
  }
  std::cout << std::endl;
  ...
  for (int c = 0; c < ncat; ++c) {
    ...
    for (int k = 0; k < ncat; ++k) {
      if (k == c) {
        printf("-         %5.3f      ",
               1.0 + wAll->var("nB" + postfix)->getError() / wAll->var("nB" + postfix)->getVal());
      } else {
        printf("-          -         ");
      }
    }
