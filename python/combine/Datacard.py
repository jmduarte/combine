...
MB.doModel()
    ...
    def path_to_shape(self, bin, proc, resolve=True):
        ...
        """
        return (
            self.barcode(bin, proc, 1).replace("$CHANNEL", bin).replace("$PROCESS", proc)
            if resolve
            else self.barcode(bin, proc, 1)
        )
    ...
    def path_to_uncert(self, bin, proc, resolve=True):
        ...
        """
        return (
            self.barcode(bin, proc, 2).replace("$CHANNEL", bin).replace("$PROCESS", proc)
            if resolve
            else self.barcode(bin, proc, 2)
        )
    ...
    def getAllVariables(self):
        ...
        """
        allVars = tuple(
            [syst[0] for syst in self.systs]
            + list(self.flatParamNuisances.keys())
            + list(self.extArgs.keys())
            + self.discretes
        )
