...
class TwoHiggsBase(PhysicsModel):
    ...
    def getYieldScale(self, bin, process):
        ...
        if self.DC.isSignal[process]:
            raise RuntimeError(
                "Found a signal process '%s' that is not among the supported ones: %s" % (process, self.modes)
            )
        return 1
    ...
    def setPhysicsOptionsBase(self, physOptions):
        for po in physOptions:
            if po.startswith("higgsMassSM="):
                self.mHSM = float(
                    (po.replace("higgsMassSM=", "").split(","))[0]
                )  # without index, this will try to case list as a float !
            if po == "mHAsPOI":
            ...
            if po.startswith("higgsMassRangeSM="):
                ...
                if len(self.mHSMRange) != 2:
                    ...
                    raise RuntimeError(
                        "Extrema for SM Higgs mass range defined with inverterd order. Second must be larger the first"
                    )
...
class TwoHiggsCvCf(TwoHiggsBase):
    ...
    def setup(self):
        self.decayScaling = {"hgg": "hgg", "hZg": "hZg", "hww": "hvv", "hzz": "hvv", "hbb": "hff", "htt": "hff"}
        self.productionScaling = {"ggH": "CF", "ttH": "CF", "qqH": "CV", "WH": "CV", "ZH": "CV", "VH": "CV"}
        ...
        self.modelBuilder.factory_(
            "sum::2HCvCf_Gscal_tot(2HCvCf_Gscal_sumf, 2HCvCf_Gscal_sumv, 2HCvCf_Gscal_gg, 2HCvCf_Gscal_Zg)"
        )
