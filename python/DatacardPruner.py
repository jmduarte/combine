            ...
            for (s_proc, s_bin, s_path, s_shape, s_syst) in shapes:
                if valid(u_bin, s_bin, bin_excepts):
                    if valid(u_proc, s_proc, proc_excepts):
                        ...
                        value = root_file.Get(
                            s_shape.replace("$CHANNEL", u_bin).replace("$PROCESS", u_proc).replace("$MASS", self.mass)
                        )
                        upper = root_file.Get(
                            s_syst.replace("$CHANNEL", u_bin)
                            .replace("$PROCESS", u_proc)
                            .replace("$MASS", self.mass)
                            .replace("$SYSTEMATIC", u_unc)
                            + "Up"
                        )
                        lower = root_file.Get(
                            s_syst.replace("$CHANNEL", u_bin)
                        ...
                        for i in range(value.GetNbinsX()):
                            if value.GetBinContent(i + 1):
                                unc_bin = float(u_value) * (
                                    upper.GetBinContent(i + 1) / value.GetBinContent(i + 1) - 1.0
                                )
                                if unc_max < unc_bin:
                                    unc_max = unc_bin
                                unc_bin = float(u_value) * (
                                    1.0 - lower.GetBinContent(i + 1) / value.GetBinContent(i + 1)
                                )
                                if unc_max < unc_bin:
                                    unc_max = unc_bin
                        root_file.Close()
            ...
            if len(words) > 1:
                if "shape" in words[1] or "lnN" in words[1]:
                    ...
                    if MANIPULATION == "COMMENT":
                        ...
                        print(
                            "Warning: MANIPULATION:", MANIPULATION, "unknown. Possible values are: COMMENT, UNCOMMENT."
                        )
            output.write(line)
        file.close()
        output.close()
