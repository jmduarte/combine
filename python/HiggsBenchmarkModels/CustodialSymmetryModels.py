...
class LambdaWZHiggs(SMLikeHiggsModel):
    ...
    def setup(self):
        ...
        self.productionScaling = {"ggH": "kf", "ttH": "kf", "WH": "kW", "ZH": "kZ"}
        ...
        self.modelBuilder.factory_(
            "sum::lambdaWZ_Gscal_tot(lambdaWZ_Gscal_Z, lambdaWZ_Gscal_W, lambdaWZ_Gscal_fermions, lambdaWZ_Gscal_gg, lambdaWZ_Gscal_Zg)"
        )
        ...
        try:
            ...
            if production == "VH":
                print(
                    ...
                    "The best I can do is to scale [%(production)s, %(decay)s, %(energy)s] with the decay BR only but this is wrong..."
                    % locals()
                )
...
class CzwHiggs(SMLikeHiggsModel):
    ...
    def setup(self):
        ...
        for e in ["7TeV", "8TeV"]:
            ...
            self.modelBuilder.factory_(
                'expr::Czw_XSscal_qqH_%s("(@0 + @1*@2) / (1.0 + @2) ", Cw, Cz, RqqH_%s)' % (e, e)
            )
            ...
            self.modelBuilder.factory_(
                'expr::Czw_XSscal_VH_%s("(@0*@1 + @2*@3) / (@1 + @3) ", Cw, SM_XS_WH_%s, Cz, SM_XS_ZH_%s)' % (e, e, e)
            )
    ...
    def getHiggsSignalYieldScale(self, production, decay, energy):
        ...
        if self.modelBuilder.out.function(name) == None:
            if production in ["ggH", "ttH"]:
                ...
                self.modelBuilder.factory_(
                    'expr::%s("@0 * @1", Czw_XSscal_%s_%s, Czw_BRscal_%s)' % (name, production, energy, decay)
                )
        return name
