...
class Anomalous_Interference_JHU_rw(PhysicsModelBase_NiceSubclasses):
    ...
    def getPOIList(self):
        ...
        if not self.dontmakea1ai:
            ...
            self.modelBuilder.doVar(
                'expr::ai("{aisign}(@0>0 ? 1 : -1) * sqrt(abs(@0)*{sigma1_HZZ}/{sigmai_HZZ})", CMS_zz4l_fai1)'.format(
                    **myxsecs
                )
            )

        self.modelBuilder.factory_(
            'expr::smCoupling_VBF("@0*@3*@1**2 - {aisign}@0*@1*@2*@3*sqrt({sigmai_VBF}/{sigma1_VBF})", {muV},a1,ai,muTT)'.format(
                **myxsecs
            )
        )
        self.modelBuilder.factory_(
            'expr::smCoupling_ZH("@0*@3*@1**2 - {aisign}@0*@1*@2*@3*sqrt({sigmai_ZH}/{sigma1_ZH})", {muV},a1,ai,muTT)'.format(
                **myxsecs
            )
        )
        self.modelBuilder.factory_(
            'expr::smCoupling_WH("@0*@3*@1**2 - {aisign}@0*@1*@2*@3*sqrt({sigmai_WH}/{sigma1_WH})", {muV},a1,ai,muTT)'.format(
                **myxsecs
            )
        )
        ...
        self.modelBuilder.factory_(
            'expr::intCoupling_VBF("{aisign}@0*@1*@2*@3*sqrt({sigmai_VBF}/{sigma1_VBF})*{sigmaint_VBF}/{sigma1_VBF}", {muV},a1,ai,muTT)'.format(
                **myxsecs
            )
        )
        self.modelBuilder.factory_(
            'expr::intCoupling_ZH("{aisign}@0*@1*@2*@3*sqrt({sigmai_ZH}/{sigma1_ZH})*{sigmaint_ZH}/{sigma1_ZH}", {muV},a1,ai,muTT)'.format(
                **myxsecs
            )
        )
        self.modelBuilder.factory_(
            'expr::intCoupling_WH("{aisign}@0*@1*@2*@3*sqrt({sigmai_WH}/{sigma1_WH})*{sigmaint_WH}/{sigma1_WH}", {muV},a1,ai,muTT)'.format(
                **myxsecs
            )
        )
    ...
    def getYieldScale(self, bin, process):
        ...
        for anomalouscoupling, pureBSM in six.iteritems(pureBSMs):
            if anomalouscoupling != self.anomalouscoupling and (
                process.endswith(pureBSM) or process.endswith(pureBSM + "f05ph0")
            ):
                raise ValueError(
                    "Can't have " + process + " for " + self.anomalouscoupling + " (it's for " + anomalouscoupling + ")"
                )
    ...
    def processPhysicsOptions(self, physOptions):
        ...
        for po in physOptions:
            if po in ("fa3", "fa2", "fL1", "fL1Zg"):
                if self.anomalouscoupling is not None:
                    raise ValueError(
                        "Multiple anomalous couplings provided as physics options ({}, {})".format(
        ...
        if self.anomalouscoupling is None:
            raise ValueError(
                "Have to provide an anomalous coupling as a physics option.  Choices: fa3, fa2, fL1, fL1Zg"
            )
