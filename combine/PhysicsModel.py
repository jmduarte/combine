...
class MultiSignalModelBase(PhysicsModelBase_NiceSubclasses):
    ...
    def setPhysicsOptions(self, physOptions):
        for po in physOptions[:]:
            if po.startswith(
                "turnoff="
            ):  # shorthand:  turnoff=process1,process2,process3 --> map=.*/(process1|process2|process3):0
    ...
    def doParametersOfInterest(self):
        ...
        if self.rMode.startswith("fixed,"):
            ...
            if self.rMode == "poi":
                ...
                raise RuntimeError(
                    "FloatingHiggsMass: the signal strength must be set to 'poi'(default), 'nuisance' or 'fixed,<value>'"
                )
        if self.modelBuilder.out.var("MH"):
            self.modelBuilder.out.var("MH").setRange(float(self.mHRange[0]), float(self.mHRange[1]))
            self.modelBuilder.out.var("MH").setConstant(False)
        ...
        for po in physOptions:
            ...
            if po.startswith("higgsMassRange="):
                ...
                if len(self.mHRange) != 2:
                    ...
                    raise RuntimeError(
                        "Higgs mass range: Extrema for Higgs mass range defined with inverterd order. Second must be larger the first"
                    )
