...
class LHCHCGBaseModel(SMLikeHiggsModel):
    ...
    def dobbH(self):
        ...
        self.modelBuilder.out.function("CMS_bbH_scaler_7TeV").addAsymmLogNormal(
            1.0 / 114.5, 1.106, self.modelBuilder.out.var("QCDscale_bbH")
        )
        self.modelBuilder.out.function("CMS_bbH_scaler_8TeV").addAsymmLogNormal(
            1.0 / 114.8, 1.103, self.modelBuilder.out.var("QCDscale_bbH")
        )
        self.modelBuilder.out.function("CMS_bbH_scaler_7TeV").addLogNormal(
            1.061, self.modelBuilder.out.var(self.bbH_pdf)
        )
        self.modelBuilder.out.function("CMS_bbH_scaler_8TeV").addLogNormal(
            1.062, self.modelBuilder.out.var(self.bbH_pdf)
        )
                ...
                for E in 7, 8:
                    ...
                    if D in self.add_bbH and P == "ggH":
                        ...
                        self.modelBuilder.factory_(
                            'expr::ggH_bbH_sum_%s_%dTeV("@1*@2+@0*@3*@4*@5",%s,%s,%s)' % (D, E, b2g, ggs, bbs)
                        )
...
class XSBRratios(LHCHCGBaseModel):
    ...
    def doParametersOfInterest(self):
        ...
        self.POIs = (
            "mu_XS_ggF_x_BR_%s,mu_XS_VBF_r_XS_ggF,mu_XS_ttH_r_XS_ggF,mu_XS_WH_r_XS_ggF,mu_XS_ZH_r_XS_ggF"
            % self.denominator
        )
...
class Kappas(LHCHCGBaseModel):
    ...
    def setup(self):
        ...
        self.modelBuilder.factory_(
            'expr::c7_Gscal_Z("@0*@0*@1*@2", kappa_Z, SM_BR_hzz, HiggsDecayWidth_UncertaintyScaling_hzz)'
        )
        self.modelBuilder.factory_(
            'expr::c7_Gscal_W("@0*@0*@1*@2", kappa_W, SM_BR_hww, HiggsDecayWidth_UncertaintyScaling_hww)'
        )
        self.modelBuilder.factory_(
        ...
        self.modelBuilder.factory_(
            'expr::c7_Gscal_top("@0*@0 * @1*@2", kappa_t, SM_BR_hcc, HiggsDecayWidth_UncertaintyScaling_hcc)'
        )
        self.modelBuilder.factory_(
        ...
        self.modelBuilder.factory_(
            "sum::c7_SMBRs(%s)"
            % (",".join("SM_BR_" + X for X in "hzz hww htt hmm hcc hbb hss hgluglu hgg hzg".split()))
        )
        ...
        self.modelBuilder.factory_(
            'expr::c7_BRscal_hww("@0*@0*@2/@1", kappa_W, c7_Gscal_tot, HiggsDecayWidth_UncertaintyScaling_hww)'
        )
        self.modelBuilder.factory_(
            'expr::c7_BRscal_hzz("@0*@0*@2/@1", kappa_Z, c7_Gscal_tot, HiggsDecayWidth_UncertaintyScaling_hzz)'
        )
        self.modelBuilder.factory_(
        ...
        self.modelBuilder.factory_(
            'expr::c7_BRscal_hcc("@0*@0*@2/@1", kappa_t, c7_Gscal_tot, HiggsDecayWidth_UncertaintyScaling_hcc)'
        )
        self.modelBuilder.factory_(
    ...
    def getHiggsSignalYieldScale(self, production, decay, energy):
        ...
        if production == "ggH":
            if decay in self.add_bbH:
                ...
                b2gs = "CMS_bbH_scaler_%s" % energy
                self.modelBuilder.factory_(
                    'expr::%s("@0*@0*@1*@1*(1+@2*@3*@4*@4*@5*@5)",kappa_gZ,%s,%s,%s,lambda_bZ,lambda_Zg)'
                    % (name, dscale, b2g, b2gs)
                )
            else:
                self.modelBuilder.factory_('expr::%s("@0*@0*   @1*@1",kappa_gZ,%s)' % (name, dscale))
        else:
            self.modelBuilder.factory_(
                'expr::%s("@0*@0*@1*@2*@2",kappa_gZ,PW_XSscal_%s_%s,%s)' % (name, production, energy, dscale)
            )
        if self.doBRU:
            name = name.replace("_noBRU", "")
            if decay == "hzz":
                self.modelBuilder.factory_(
                    "prod::%s(%s_noBRU, HiggsDecayWidth_UncertaintyScaling_%s)" % (name, name, "hzz")
                )
            else:
                self.modelBuilder.factory_(
...
class KappaVKappaF(LHCHCGBaseModel):
    ...
    def setup(self):
        ...
        for ds in ["WW", "ZZ", "gamgam", "bb", "tautau", "mumu", "inv"]:
            ...
            ## partial witdhs, normalized to the SM one
            self.modelBuilder.factory_(
                'expr::c7_Gscal_Z_%s("@0*@0*@1*@2", kVkV_%s, SM_BR_hzz, HiggsDecayWidth_UncertaintyScaling_hzz)'
                % (ds, ds)
            )
            self.modelBuilder.factory_(
                'expr::c7_Gscal_W_%s("@0*@0*@1*@2", kVkV_%s, SM_BR_hww, HiggsDecayWidth_UncertaintyScaling_hww)'
                % (ds, ds)
            )
            self.modelBuilder.factory_(
            ...
            self.modelBuilder.factory_(
                'expr::c7_Gscal_top_%s("@0*@0 * @1*@2", kFkF_%s, SM_BR_hcc, HiggsDecayWidth_UncertaintyScaling_hcc)'
                % (ds, ds)
            )
            self.modelBuilder.factory_(
                'expr::c7_Gscal_bottom_%s("@0*@0 * (@1*@3+@2)", kFkF_%s, SM_BR_hbb, SM_BR_hss, HiggsDecayWidth_UncertaintyScaling_hbb)'
                % (ds, ds)
            )
            self.modelBuilder.factory_(
                'expr::c7_Gscal_gluon_%s("  @0*@0*@1*@2", kFkF_%s, SM_BR_hgluglu, HiggsDecayWidth_UncertaintyScaling_hgluglu)'
                % (ds, ds)
            )
            self.modelBuilder.factory_(
        ...
        if self.modelBuilder.out.function(name) == None:
            ...
            if production == "ggH" and (decay in self.add_bbH) and energy in ["7TeV", "8TeV", "13TeV", "14TeV"]:
                ...
                self.modelBuilder.factory_(
                    ...
                    % (name, XSscal[0], XSscal[1], CMS_to_LHCHCG_DecSimple[decay], b2g, b2gs, BRscal)
                )
            else:
                self.modelBuilder.factory_('expr::%s("%s*@1", %s, c7_BRscal_%s)' % (name, XSscal[0], XSscal[1], BRscal))
            print("[LHC-HCG Kappas]", name, production, decay, energy, ": ", end=" ")
    ...
    def getHiggsSignalYieldScale(self, production, decay, energy):
        ...
        if self.modelBuilder.out.function(name) == None:
            ...
            if production in ["tHq", "tHW", "ttH"]:
                self.modelBuilder.factory_(
                    'expr::%s("%s*@1*@2", %s, c7_BRscal_%s, r)' % (name, XSscal[0], XSscal[1], BRscal)
                )
    ...
    def setup(self):
        ...
        for E in "7TeV", "8TeV", "13TeV", "14TeV":
            for P in "ggH", "tHq", "tHW", "ggZH":
                self.modelBuilder.factory_(
                    'expr::PW_XSscal_%s_%s("@0*@1*@1*@2*@2*@3*@3",Scaling_%s_%s,kappa_qq, kappa_uu, kappa_VV)'
                    % (P, E, P, E)
                )
            for P in "qqH", "WH", "ZH":
                self.modelBuilder.factory_(
                    'expr::PW_XSscal_%s_%s("@0*@0*@1*@1*@2*@2*@3*@3*@4*@4", kappa_qq, lambda_Vq, kappa_uu, lambda_Vu, kappa_VV)'
                    % (P, E)
                )
            self.modelBuilder.factory_(
                'expr::PW_XSscal_ttH_%s("@0*@0*@1*@1*@2*@2*@3*@3",kappa_qq,kappa_uu,kappa_VV,lambda_FV)' % E
            )
            self.modelBuilder.factory_(
                'expr::PW_XSscal_bbH_%s("@0*@0*@1*@1*@2*@2*@3*@3*@4*@4",kappa_qq,kappa_uu,lambda_du,kappa_VV,lambda_FV)'
                % E
            )
        self.decayMap_ = {
            "hww": "C_V",
        ...
        if production == "ggH":
            if decay in self.add_bbH:
                ...
                if decay in ["hgg", "hgluglu", "hzg"]:
                    self.modelBuilder.factory_(
                        'expr::%s("@0*@1*(1+@2*@3*@4)",PW_XSscal_ggH_%s,%s,%s,%s,PW_XSscal_bbH_%s)'
                        % (name, energy, dscale, b2g, b2gs, energy)
                    )
                else:
                    self.modelBuilder.factory_(
                        'expr::%s("@0*@1*@1*(1+@2*@3*@4)",PW_XSscal_ggH_%s,%s,%s,%s,PW_XSscal_bbH_%s)'
                        % (name, energy, dscale, b2g, b2gs, energy)
                    )
            else:
                if decay in ["hgg", "hgluglu", "hzg"]:
            ...
            if decay in ["hgg", "hgluglu", "hzg"]:
                ...
                self.modelBuilder.factory_(
                    'expr::%s("@0*@1*@1",PW_XSscal_%s_%s,%s)' % (name, production, energy, dscale)
                )
        if self.doBRU:
            name = name.replace("_noBRU", "")
            if decay == "hzz":
    ...
    def setup(self):
        ...
        for P in ALL_HIGGS_PROD:
            ...
            for D in SM_HIGG_DECAYS:
                for E in 7, 8:
                    terms = []
                    if P in ["ggH", "bbH", "ttH", "tHq", "tHW"] and CMS_to_LHCHCG_DecSimple[D] in [
                        "WW",
                        "ZZ",
                        "gamgam",
                    ]:
...
class CommonMatrixModel(LHCHCGBaseModel):
    ...
    def doParametersOfInterest(self):
        ...
        for D in ["gamgam", "WW", "ZZ", "tautau", "bb"]:
            ...
            if (
                D in self.fixDecays
            ):  # If there are any missing ggH measurements, create the mus here and a POI out of it.
                print(
                ...
                for E in 7, 8:
                    ...
                    if D in self.add_bbH and P == "ggH":
                        ...
                        self.modelBuilder.factory_(
                            ...
                            % (CMS_to_LHCHCG_DecSimple[D], E, 1.0, b2g, bbs, CMS_to_LHCHCG_DecSimple[D])
                        )
                        terms += [
