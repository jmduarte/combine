...
def quadratureAdd(pdf, val1, val2, context=None):
    ...
    if pdf in ["lnN", "lnU"]:
        if log(val1) * log(val2) < 0:
            raise RuntimeError(
                "Can't add in quadrature nuisances of pdf %s with values %s, %s that go in different directions (at %s)"
                % (pdf, val1, val2, context)
            )
...
def doAddNuisance(datacard, args):
    if len(args) < 5:
        raise RuntimeError(
            "Missing arguments: the syntax is: nuisance edit add process channel name pdf value [ options ]"
        )
    (process, channel, name, pdf, value) = args[:5]
    if process != "*":
        cprocess = re.compile(process)
    if channel != "*":
        cchannel = re.compile(channel.replace("+", "\+"))
    opts = args[5:]
    found = False
                    ...
                    if value in [0.0, 1.0]:
                        ...
                        if "addq" in opts:
                            errline[b][p] = quadratureAdd(
                                pdf, errline[b][p], value, context="nuisance edit add, args = %s" % args
                            )
...
def doRenameNuisance(datacard, args):
    if len(args) == 2:  # newname oldname
        ...
        for lsyst, nofloat, pdf0, args0, errline0 in datacard.systs[:]:
            ...
            if lsyst == oldname:  # found the nuisance
                ...
                if pdf0 != "param":
                    raise RuntimeError(
                        "Missing arguments: the syntax is: nuisance edit rename process channel oldname newname"
                    )
                for lsyst2, nofloat2, pdf02, args02, errline02 in datacard.systs[:]:
                    if lsyst2 == newname:
                        if pdf02 != "param":
                            if (args0[0]) not in ["0.0", "0.", "0"] or (args0[1]) not in ["1.0", "1.", "1"]:
                                raise RuntimeError(
                                    "Can't rename nuisance %s with Gaussian pdf G(%s,%s) to name %s which already exists with G(0,1) constraint!"
                                    % (lsyst, args0[0], args0[1], lsyst2)
                                )
                        else:
                            if (args0[0]) != (args02[0]) or float(args0[1]) != float(args02[1]):
                                raise RuntimeError(
                                    "Can't rename nuisance %s with Gaussian pdf G(%s,%s) to name %s which already exists with Gaussian pdf G(%s,%s) constraint!"
                                    % (lsyst, args0[0], args0[1], lsyst2, args02[0], args02[1])
                                )
                break
        if nuisanceID:
            ...
            raise RuntimeError("No nuisance parameter found with name %s" % oldname)
        return
    ...
    for lsyst, nofloat, pdf0, args0, errline0 in datacard.systs[:]:
        ...
        if lsystnew != lsyst:
            if pdf0 == "param":
                raise RuntimeError(
                    "Incorrect syntax. Cannot specify process and channel for %s with pdf %s. Use 'nuisance edit rename oldname newname'"
                    % (lsyst, pdf0)
                )
            found = False
            ...
            for lsyst2, nofloat2, pdf2, args2, errline2b in datacard.systs:
                if lsyst2 == lsystnew:
                    found = True
                    errline2 = errline2b
                    if pdf2 != pdf0 and pdf2 not in ["lnN"]:
                        raise RuntimeError(
                            "Can't rename nuisance %s with pdf %s to name %s which already exists as %s"
                            % (lsyst, pdf0, lsystnew, pdf2)
                        )
            if not found:
            ...
            for b in errline0.keys():
                if channel == "*" or cchannel.match(b):
                    ...
                    for p in datacard.exp[b].keys():
                        if process == "*" or cprocess.match(p):
                            ...
                            if p in errline0[b] and errline2[b][p] not in [0.0, 1.0]:
                                if "addq" in opts:
                                    errline2[b][p] = quadratureAdd(
                                        pdf0,
                                        errline0[b][p],
                                        errline2[b][p],
                                        context="nuisance edit rename, args = %s" % args,
                                    )
                                elif "overwrite" in opts:
                                    errline2[b][p] = errline0[b][p]
                                else:
                                    raise RuntimeError(
                                        "Can't rename nuisance with args = %s for bin %s, process %s: found existing non-null value %s, and no option 'addq' or 'overwrite' given"
                                        % (args, b, p, errline2[b][p])
                                    )
                            else:
                                errline2[b][p] = errline0[b][p]
                            errline0[b][p] = 0
                    if channel != "*" and not foundProc:
                        if "ifexists" not in opts:
                            raise RuntimeError(
                                "Error: nuisance edit rename %s found nothing in channel %s" % (args, channel)
                            )
                        else:
                            sys.stderr.write(
                                "Warning: nuisance edit rename %s found nothing in channel %s\n" % (args, channel)
                            )
    if not foundProc and channel != "*":
        if "ifexists" not in opts:
            raise RuntimeError(
                "Error: no process found matching nuisance edit rename with args = %s (and option 'ifexist' not specified)\n"
                % args
            )
        else:
...
def doMergeNuisance(datacard, args):
    if len(args) < 4:
        raise RuntimeError(
            "Missing arguments: the syntax is: nuisance edit merge process channel name1 name2 [ options ]"
        )
...
def doSplitNuisance(datacard, args):
    if len(args) < 7:
        raise RuntimeError(
            "Missing arguments: the syntax is: nuisance edit split process channel oldname newname1 newname2 value1 value2"
        )
...
def doFreezeNuisance(datacard, args):
    if len(args) < 1:
        raise RuntimeError(
            "Missing arguments: the syntax is: nuisance edit freeze name [ifexists] (name can be a pattern)"
        )
    ...
    for lsyst in (
        list(datacard.flatParamNuisances.keys())
        + list(datacard.rateParamsOrder)
        + datacard.discretes
        + list(datacard.extArgs.keys())
    ):
