        ...
        for name, vals in six.iteritems(self.msbar):
            ...
            if name in ("W", "Z"):
                ...
                self.modelBuilder.factory_(
                    'expr::C%(name)s("@0 * TMath::Power(@3,2*@2) / TMath::Power(@1,1+2*@2)", SM_VEV, M, eps, M%(name)s_MSbar)'
                    % locals()
                )
            else:
                #                # Ellis cf == v (mf^e/M^(1 + e))
                self.modelBuilder.factory_(
                    'expr::C%(name)s("@0 * TMath::Power(@3,@2) / TMath::Power(@1,1+@2)", SM_VEV, M, eps, M%(name)s_MSbar)'
                    % locals()
                )

        self.productionScaling = {"ttH": "Ctop", "WH": "CW", "ZH": "CZ"}
