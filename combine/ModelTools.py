        ...
        for rp in self.DC.extArgs.keys():
            ...
            if ":" in argv:
                ...
                if (fin, wsn) in open_files:
                    ...
                    if not wstmp.arg(rp):
                        raise RuntimeError(
                            "No parameter '%s' found for extArg in workspace %s from file %s" % (rp, wsn, fin)
                        )
    ...
    def doRateParams(self):
            ...
            for rk in range(len(self.DC.rateParams[rp])):
                ...
                if (fin, wsn) in open_files:
                    ...
                    if not wstmp.arg(argu):
                        raise RuntimeError(
                            "No parameter '%s' found for rateParam in workspace %s from file %s" % (argu, wsn, fin)
                        )
                    ...
                    try:
                        ...
                        raise RuntimeError(
                            "No File '%s' found for rateParam, or workspace '%s' not in file " % (fin, wsn)
                        )
        ...
        for rp in self.DC.rateParams.keys():
            for rk in range(len(self.DC.rateParams[rp])):
                ...
                if self.out.arg(argu):
                    continue
                if not reduce(lambda x, y: x * y, [self.out.arg(a) != None for a in argv.split(",")], 1):
        ...
        while True:
            ...
            if len(tbc) == len(toBeCreated):
                ...
                raise RuntimeError(
                    "Cannot produce following rateParams (dependent parameters not found!) %s"
                    % (",".join([t[0] for t in toBeCreated]))
                )
        ...
        for (n, nofloat, pdf, args, errline) in self.DC.systs:
            ...
            if pdf == "lnN" or (pdf.startswith("shape") and pdf != "shapeU"):
                ...
                if self.options.noOptimizePdf or is_func_scaled:
                    ...
                    self.doObj(
                        "%s_Pdf" % n, "SimpleGaussianConstraint", "%s%s, %s_In[0,%s], %s" % (n, r_exp, n, r, sig), True
                    )
                    # Use existing constraint since it could be a param
                ...
                for c in errline.values():  # list channels
                    for v in c.values():  # list effects in each channel
                        if v != 0:
                            if val != 0 and v != val:
                                raise RuntimeError(
                                    "Error: line %s contains two different uncertainties %g, %g, which is not supported for gmM"
                                    % (n, v, val)
                                )
                            val = v
                if val == 0:
                ...
                if pdf == "dFD":
                    self.doObj(
                        "%s_Pdf" % n,
                        ROOFIT_EXPR_PDF,
                        "'1/(2*(1+exp(%f*((@0-@1)-1)))*(1+exp(-%f*((@0-@1)+1))))', %s[0,%s], %s_In[0,%s]"
                    ...
                    self.doObj(
                        "%s_Pdf" % n,
                        ROOFIT_EXPR_PDF,
                        "'1/(2*(1+exp(%f*(@0-1)))*(1+exp(-%f*(@0+1))))', %s[0,%s], %s_In[0,%s]"
                        % (args[0], args[0], n, r, n, r),
                    )
                globalobs.append("%s_In" % n)
                if self.options.bin:
                ...
                if "/" in args[1]:
                        ...
                        if self.out.var(n):
                            ...
                            if self.out.var(n).getMin() != bounds[0] or self.out.var(n).getMax() != bounds[1]:
                                print(
                                    ...
                                    % (n, bounds[0], bounds[1], self.out.var(n).getMin(), self.out.var(n).getMax())
                                )
                    ...
                    self.doObj(
                        ...
                        "BifurGauss",
                        "%s, %s_In[%s,%g,%g], %s, %s"
                        % (n, n, args[0], self.out.var(n).getMin(), self.out.var(n).getMax(), sigmaStrL, sigmaStrR),
                        True,
                    )
                    self.out.var("%s_In" % n).setConstant(True)
                    if is_func_scaled:
                        self.doExp(
                            "%s_BoundHi" % n, "%g+%g*@0" % (mean, self.out.var(n).getMax() - mean), "%s" % (func_scaler)
                        )
                        self.doExp(
                            "%s_BoundLo" % n, "%g-%g*@0" % (mean, mean - self.out.var(n).getMin()), "%s" % (func_scaler)
                        )
                    ...
                    if self.options.noOptimizePdf or is_func_scaled:
                        self.doObj(
                            ...
                            "Gaussian",
                            "%s, %s_In[%s,%g,%g], %s"
                            % (n, n, args[0], self.out.var(n).getMin(), self.out.var(n).getMax(), sigmaStr),
                            True,
                        )
                    else:
                        self.doObj(
        ...
        for groupName, nuisanceNames in six.iteritems(self.DC.groups):
            for nuisanceName in nuisanceNames:
                if nuisanceName not in existingNuisanceNames:
                    raise RuntimeError(
                        'Nuisance group "%(groupName)s" refers to nuisance "%(nuisanceName)s" but it does not exist. Perhaps you misspelled it.'
                        % locals()
                    )
                ...
                for groupName in groupNames:
                    try:
                        ...
                        try:
                            ...
                            raise RuntimeError(
                                'Nuisance group "%(groupName)s" refers to nuisance but it is not an independant parameter.'
                                % locals()
                            )
                ...
                if "%sAND%s" % (b, p) in list(self.DC.rateParams.keys()):
                    for rk in range(len(self.DC.rateParams["%sAND%s" % (b, p)])):
                        ...
                        if self.out.arg(argu):
                            ...
                            raise RuntimeError(
                                "No rate parameter found %s, are you sure you defined it correctly in the datacard?"
                                % (argu)
                            )
                selfNormRate = 1.0
                for (n, nofloat, pdf, args, errline) in self.DC.systs:
                    ...
                    if pdf == "lnN" or pdf == "lnU":
                        ...
                        factors.append(n)
                        if abs(errline[b][p] * args[0] - self.DC.exp[b][p]) > max(
                            0.05 * max(self.DC.exp[b][p], 1), errline[b][p]
                        ):
                            raise RuntimeError(
    ...
    def doIndividualModels(self):
        ...
        for b in self.DC.bins:
            self.doObj(
                ...
                "sum",
                ", ".join(
                    ["n_exp_bin%s_proc_%s" % (b, p) for p in self.DC.exp[b].keys() if self.DC.isSignal[p] == False]
                ),
            )
            self.doObj(
                "n_exp_bin%s" % b, "sum", ", ".join(["n_exp_bin%s_proc_%s" % (b, p) for p in self.DC.exp[b].keys()])
            )
