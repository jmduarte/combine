...
"(@0*@0)*@2  + (@1*@1)*@3 + (@0*@1)*@4",\
 ...
 %(prefix)sGamma_tt, %(prefix)sGamma_bb, %(prefix)sGamma_tb\
            ...
            for qty, column in six.iteritems(structure):
                ...
                self.textToSpline(
                    rooName,
                    os.path.join(self.coupPath, fileFor["hgg" if what.startswith("hgg") else "hzg"]),
                    ycol=column,
                )
            scalingName = "Scaling_" + what
            #            print 'Building '+scalingName
 ...
 %(prefix)sGamma_tl, %(prefix)sGamma_bl, %(prefix)sGamma_lW\
            ...
            for sqrts in ("7TeV", "8TeV", "13TeV", "14TeV"):
                for qty, column in six.iteritems(structure):
                    rooName = prefix + qty + "_" + sqrts
                    self.textToSpline(
                        rooName, os.path.join(self.coupPath, "ggZH_%(sqrts)s.txt" % locals()), ycol=column
                    )
                scalingName = "Scaling_" + what + "_" + sqrts
                coeffSum = (
                    'expr::coeff_sum_%(scalingName)s( "@0+@1+@2+@3+@4+@5",\
                %(prefix)sc_kt2_%(sqrts)s, %(prefix)sc_kb2_%(sqrts)s, %(prefix)sc_kZ2_%(sqrts)s, %(prefix)sc_ktkb_%(sqrts)s, %(prefix)sc_ktkZ_%(sqrts)s, %(prefix)sc_kbkZ_%(sqrts)s)'
                    % locals()
                )
                self.modelBuilder.factory_(coeffSum)

                rooExpr = (
                    'expr::%(scalingName)s( "( (@0*@0)*(@3)  + (@1*@1)*(@4) + (@2*@2)*(@5) + (@0*@1)*(@6)  + (@0*@2)*(@7) + (@1*@2)*(@8) )/@9",\
                %(Ctop)s, %(Cb)s, %(CZ)s, %(prefix)sc_kt2_%(sqrts)s, %(prefix)sc_kb2_%(sqrts)s, %(prefix)sc_kZ2_%(sqrts)s, %(prefix)sc_ktkb_%(sqrts)s, %(prefix)sc_ktkZ_%(sqrts)s, %(prefix)sc_kbkZ_%(sqrts)s, coeff_sum_%(scalingName)s)'
                    % locals()
                )
                self.modelBuilder.factory_(rooExpr)
            ...
            for sqrts in ("7TeV", "8TeV", "13TeV", "14TeV"):
                ...
                rooExpr = (
                    ...
                    + '( "( (@0*@0)*(%g)  + (@1*@1)*(%g) + (@0*@1)*(%g) )/%g"'
                    % tuple((coeffs[sqrts] + [sum(coeffs[sqrts])]))
                    + ", %(Ctop)s, %(CW)s)" % locals()
                )
                self.modelBuilder.factory_(rooExpr)
    ...
    def textToSpline(self, name, filename, xvar="MH", ycol=1, xcol=0, skipRows=1, algo="CSPLINE"):
        ...
        spline = ROOT.RooSpline1D(
            name, "file %s, x=%d, y=%d" % (filename, xcol, ycol), xv, len(x), array("d", x), array("d", y), algo
        )
