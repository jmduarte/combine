...
void RooSimultaneousOpt::addExtraConstraint(const RooAbsPdf &pdf) {
  if (!_extraConstraints.contains(pdf))
    _extraConstraints.add(pdf);
}

void RooSimultaneousOpt::addExtraConstraints(const RooAbsCollection &pdfs) {
  if (_extraConstraints.getSize() == 0) {
    ...
    for (RooAbsArg *a = (RooAbsArg *)iter.Next(); a != 0; a = (RooAbsArg *)iter.Next()) {
      if (!_extraConstraints.contains(*a))
        _extraConstraints.add(*a);
    }
  }
}

void RooSimultaneousOpt::addChannelMasks(const RooArgList &args) {
  if (args.getSize() != indexCat().numTypes()) {
    std::cerr << "combine/RooSimultaneousOpt: number of channel masks must equal number of channels\n";
    return;
  }
