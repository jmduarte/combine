...
void DebugProposal::Propose(RooArgSet &xPrime, RooArgSet &x) {
  ...
  if (tries_ > 0) {
    std::cout << "DebugProposal::Propose: x'" << std::endl;
    xPrime.Print("V");
    std::cout << "DebugProposal::Propose: x " << std::endl;
    x.Print("V");
    if (nll_.get()) {
...
Double_t DebugProposal::GetProposalDensity(RooArgSet &x1, RooArgSet &x2) {
  if (tries_ > 0) {
    std::cout << "DebugProposal::GetProposalDensity: x1" << std::endl;
    x1.Print("V");
    std::cout << "DebugProposal::GetProposalDensity: x2" << std::endl;
    x2.Print("V");
  }
  Double_t ret = prop_->GetProposalDensity(x1, x2);
  if (tries_ > 0) {
    std::cout << "DebugProposal::GetProposalDensity: return " << ret << std::endl;
  }
  return ret;
}
...
Bool_t DebugProposal::IsSymmetric(RooArgSet &x1, RooArgSet &x2) {
  if (tries_ > 0) {
    std::cout << "DebugProposal::IsSymmetric: x1" << std::endl;
    x1.Print("V");
    std::cout << "DebugProposal::IsSymmetric: x2" << std::endl;
    x2.Print("V");
  }
  Bool_t ret = prop_->IsSymmetric(x1, x2);
  if (tries_ > 0) {
    std::cout << "DebugProposal::IsSymmetric: return " << ret << std::endl;
  }
  return ret;
}
