...
void DebugProposal::Propose(RooArgSet &xPrime, RooArgSet &x) {
  ...
  if (tries_ > 0) {
    std::cout << "combine/DebugProposal::Propose: x'" << std::endl;
    xPrime.Print("V");
    std::cout << "combine/DebugProposal::Propose: x " << std::endl;
    x.Print("V");
    if (nll_.get()) {
      ...
      std::cout << "combine/DebugProposal::Propose: NLL(x)  = " << nllX << std::endl;
      std::cout << "combine/DebugProposal::Propose: NLL(x') = " << nllXP << std::endl;
      std::cout << "combine/DebugProposal::Propose: uncorrected acc prob. = " << exp(nllX - nllXP) << std::endl;
      std::cout << "combine/DebugProposal::Propose: tracing NLL(x')" << std::endl;
      RooStats::SetParameters(&xPrime, &params_);
      tracePdf(nll_.get());
      std::cout << "combine/DebugProposal::Propose: tracing NLL(x)" << std::endl;
      RooStats::SetParameters(&x, &params_);
...
// point x2
Double_t DebugProposal::GetProposalDensity(RooArgSet &x1, RooArgSet &x2) {
  if (tries_ > 0) {
    std::cout << "combine/DebugProposal::GetProposalDensity: x1" << std::endl;
    x1.Print("V");
    std::cout << "combine/DebugProposal::GetProposalDensity: x2" << std::endl;
    x2.Print("V");
  }
  Double_t ret = prop_->GetProposalDensity(x1, x2);
  if (tries_ > 0) {
    std::cout << "combine/DebugProposal::GetProposalDensity: return " << ret << std::endl;
  }
  return ret;
}
...
Bool_t DebugProposal::IsSymmetric(RooArgSet &x1, RooArgSet &x2) {
  if (tries_ > 0) {
    std::cout << "combine/DebugProposal::IsSymmetric: x1" << std::endl;
    x1.Print("V");
    std::cout << "combine/DebugProposal::IsSymmetric: x2" << std::endl;
    x2.Print("V");
  }
  Bool_t ret = prop_->IsSymmetric(x1, x2);
  if (tries_ > 0) {
    std::cout << "combine/DebugProposal::IsSymmetric: return " << ret << std::endl;
  }
  return ret;
}
...
void DebugProposal::tracePdf(RooAbsReal *pdf) {
  ...
  for (int i = 0, n = deps.getSize(); i < n; ++i) {
    ...
    if (typeid(*rar) == typeid(RooRealVar) || rar->isConstant())
      continue;
    if (rar != 0) {
