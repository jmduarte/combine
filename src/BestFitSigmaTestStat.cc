...
BestFitSigmaTestStat::BestFitSigmaTestStat(
    const RooArgSet &observables, RooAbsPdf &pdf, const RooArgSet *nuisances, const RooArgSet &params, int verbosity)
    : pdf_(&pdf), verbosity_(verbosity) {
...
Double_t BestFitSigmaTestStat::Evaluate(RooAbsData &data, RooArgSet & /*nullPOI*/) {
  ...
  *params_ = snap_;
  ...
  std::cout << "Doing a fit for with " << r->GetName() << " in range [ " << r->getMin() << " , " << r->getMax() << "]"
            << std::endl;
