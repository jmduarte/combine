...
void RooMorphingPdf::Init() const {
  ...
  if (masses_.size() != unsigned(pdfs_.getSize())) {
    throw std::runtime_error(
        "combine/RooMorphingPdf: Number of mass points differs from the number of "
        "stored pdfs!");
  }
  for (unsigned i = 0; i < masses_.size(); ++i) {
    ...
    if (!tpdf)
      throw std::runtime_error(
          "combine/RooMorphingPdf: Supplied pdf is not of type "
          "FastVerticalInterpHistPdf2");
    tpdf->evaluate();
    if (tpdf->cache().size() != unsigned(morph_axis_.GetNbins())) {
      throw std::runtime_error(
          "combine/RooMorphingPdf: Component pdfs must have the same number of bins as "
