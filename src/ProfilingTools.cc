...
PerfCounter &PerfCounter::get(const char *name) { return perfCounters_[name]; }

void PerfCounter::printAll() {
  for (boost::unordered_map<const char *, PerfCounter>::const_iterator it = perfCounters_.begin(),
                                                                       ed = perfCounters_.end();
       it != ed;
       ++it) {
...
namespace runtimedef {
  ...
  int get(const std::string &name) { return definesByString_[name]; }
  void set(const std::string &name, int value) { definesByString_[name] = value; }
}  // namespace runtimedef
