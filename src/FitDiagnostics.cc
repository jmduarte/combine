...
FitDiagnostics::FitDiagnostics()
  ...
  nll_sb_ = -1;
  overallBins_ = 0;
  overallNorms_ = 0, overallNuis_ = 0, overallCons_ = 0;
}
...
void FitDiagnostics::applyOptions(const boost::program_options::variables_map &vm) {
  ...
  if (justFit_) {
    ...
    saveNormalizations_ = false;
    reuseParams_ = false, skipBOnlyFit_ = true;
  }
}

bool FitDiagnostics::runSpecific(RooWorkspace *w,
  ...
  if (reuseParams_ && minos_ != "none") {
    std::cout << "Cannot reuse b-only fit params when running minos. Parameters will be reset when running S+B fit"
              << std::endl;
    reuseParams_ = false;
  }
  ...
  if (!justFit_ && nuis && globalObs) {
    ...
    if (makePlots_ && currentToy_ < 1) {
      std::vector<RooPlot *> plots = utils::makePlots(
          *mc_s->GetPdf(), data, signalPdfNames_.c_str(), backgroundPdfNames_.c_str(), rebinFactor_, res_prefit);
  ...
  if (!customStartingPoint_)
    r->setVal(0.0);
  else
    std::cout << "customStartingPoint set to true, Background only fit will corrsepond to " << r->GetName() << " = "
              << r->getVal() << std::endl;
  r->setConstant(true);
  ...
  if (res_b) {
    ...
    if (!robustHesse_ && res_b->covQual() < 3) {
      if (!saveWithUncertainties_) {
        std::cerr << "[WARNING]: Unable to determine uncertainties on all fit parameters in b-only fit. Have a look at "
                     "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                     "nonstandard#fit-parameter-uncertainties for more information."
                  << std::endl;
        Logger::instance().log(
            std::string(
                "[WARNING]: Unable to determine uncertainties on all fit parameters in b-only fit. Have a look at "
                "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                "nonstandard#fit-parameter-uncertainties for more information."),
            Logger::kLogLevelError,
            __func__);
      } else if (ignoreCovWarning_) {
        std::cerr << "[WARNING]: Unable to determine uncertainties on all fit parameters in b-only fit. Caution: by "
                     "passing --ignoreCovWarning the shapes and uncertainties will be stored as configured via the "
                     "command line despite this issue. Have a look at "
                     "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                     "nonstandard#fit-parameter-uncertainties for more information."
                  << std::endl;
        Logger::instance().log(
            std::string("[WARNING]: Unable to determine uncertainties on all fit parameters in b-only fit. Caution: by "
                        "passing --ignoreCovWarning the shapes and uncertainties will be stored as configured via the "
                        "command line despite this issue. Have a look at "
                        "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                        "nonstandard#fit-parameter-uncertainties for more information."),
        ...
        std::cerr << "[WARNING]: Unable to determine uncertainties on all fit parameters in b-only fit. The option "
                     "--saveWithUncertainties will be ignored as it would lead to incorrect results. Have a look at "
                     "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                     "nonstandard#fit-parameter-uncertainties for more information."
                  << std::endl;
        Logger::instance().log(
            std::string("[WARNING]: Unable to determine uncertainties on all fit parameters in b-only fit. The option "
                        "--saveWithUncertainties will be ignored as it would lead to incorrect results. Have a look at "
                        "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                        "nonstandard#fit-parameter-uncertainties for more information."),
            Logger::kLogLevelError,
    ...
    if (makePlots_ && currentToy_ < 1) {
      std::vector<RooPlot *> plots = utils::makePlots(
          *mc_b->GetPdf(), data, signalPdfNames_.c_str(), backgroundPdfNames_.c_str(), rebinFactor_, res_b);
      for (std::vector<RooPlot *>::iterator it = plots.begin(), ed = plots.end(); it != ed; ++it) {
        c1->cd();
        (*it)->Draw();
    ...
    if (saveNormalizations_) {
      ...
      getNormalizations(
          mc_s->GetPdf(), *mc_s->GetObservables(), *norms, sampler, currentToy_ < 1 ? fitOut.get() : 0, "_fit_b", data);
      delete norms;
    }
  ...
  if (res_s) {
    ...
    if (fitOut.get()) {
      ...
      saveWithUncertainties_ =
          saveWithUncertsRequested_;  //Reset saveWithUncertainties flag to original value in case it has been set to false due to covariance matrix issues in the b-only fit.
      if (!robustHesse_ && res_s->covQual() < 3) {
        if (!saveWithUncertainties_) {
          std::cerr << "[WARNING]: Unable to determine uncertainties on all fit parameters in s+b fit. Have a look at "
                       "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                       "nonstandard#fit-parameter-uncertainties for more information."
                    << std::endl;
          Logger::instance().log(
              std::string(
                  "[WARNING]: Unable to determine uncertainties on all fit parameters in s+b fit. Have a look at "
                  "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                  "nonstandard#fit-parameter-uncertainties for more information."),
              Logger::kLogLevelError,
              __func__);
        } else if (ignoreCovWarning_) {
          std::cerr << "[WARNING]: Unable to determine uncertainties on all fit parameters in s+b fit. Caution: by "
                       "passing --ignoreCovWarning the shapes and uncertainties will be stored as configured via the "
                       "command line despite this issue. Have a look at "
                       "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                       "nonstandard#fit-parameter-uncertainties for more information."
                    << std::endl;
          Logger::instance().log(
              std::string("[WARNING]: Unable to determine uncertainties on all fit parameters in s+b fit. Caution: by "
                          "passing --ignoreCovWarning the shapes and uncertainties will be stored as configured via "
                          "the command line despite this issue. Have a look at "
                          "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                          "nonstandard#fit-parameter-uncertainties for more information."),
              Logger::kLogLevelError,
              __func__);
          ...
          saveWithUncertainties_ = false;
          std::cerr << "[WARNING]: Unable to determine uncertainties on all fit parameters in s+b fit. The option "
                       "--saveWithUncertainties will be ignored as it would lead to incorrect results. Have a look at "
                       "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                       "nonstandard#fit-parameter-uncertainties for more information."
                    << std::endl;
          Logger::instance().log(
              std::string(
                  "[WARNING]: Unable to determine uncertainties on all fit parameters in s+b fit. The option "
                  "--saveWithUncertainties will be ignored as it would lead to incorrect results. Have a look at "
                  "https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/wiki/"
                  "nonstandard#fit-parameter-uncertainties for more information."),
              Logger::kLogLevelError,
    ...
    if (makePlots_ && currentToy_ < 1) {
      std::vector<RooPlot *> plots = utils::makePlots(
          *mc_s->GetPdf(), data, signalPdfNames_.c_str(), backgroundPdfNames_.c_str(), rebinFactor_, res_s);
      for (std::vector<RooPlot *>::iterator it = plots.begin(), ed = plots.end(); it != ed; ++it) {
        c1->cd();
        (*it)->Draw();
    ...
    if (saveNormalizations_) {
      ...
      getNormalizations(
          mc_s->GetPdf(), *mc_s->GetObservables(), *norms, sampler, currentToy_ < 1 ? fitOut.get() : 0, "_fit_s", data);
      delete norms;
    }
  ...
  if (res_s) {
    ...
    if (rf) {
      ...
      limit = bestFitVal - loErr;
      limitErr = 0;
      if (!noErrors_)
        Combine::commitPoint(/*expected=*/true, /*quantile=*/0.16);
      limit = bestFitVal + hiErr;
      limitErr = 0;
      if (!noErrors_)
        Combine::commitPoint(/*expected=*/true, /*quantile=*/0.84);
      if (do95_ && rf->hasRange("err95") && !noErrors_) {
        limit = rf->getMax("err95");
        Combine::commitPoint(/*expected=*/true, /*quantile=*/0.975);
        limit = rf->getMin("err95");
        Combine::commitPoint(/*expected=*/true, /*quantile=*/0.025);
      }

      limit = bestFitVal;
      limitErr = maxError;
      std::cout << "\n --- FitDiagnostics ---" << std::endl;
      std::cout << "Best fit " << r->GetName() << ": " << rf->getVal() << "  " << -loErr << "/+" << +hiErr
                << "  (68% CL)" << std::endl;
      if (do95_) {
        std::cout << "         " << r->GetName() << ": " << rf->getVal() << "  " << -loErr95 << "/+" << +hiErr95
                  << "  (95% CL)" << std::endl;
      }
    } else {
      std::cout << "\n --- FitDiagnostics ---" << std::endl;
...
void FitDiagnostics::getNormalizationsSimple(RooAbsPdf *pdf, const RooArgSet &obs, RooArgSet &out) {
  ...
  std::map<std::string, ShapeAndNorm>::const_iterator bg = snm.begin(), ed = snm.end(), pair;
  int i;
  for (pair = bg, i = 0; pair != ed; ++pair, ++i) {
    RooRealVar *val =
        new RooRealVar((oldNormNames_ ? pair->first : pair->second.channel + "/" + pair->second.process).c_str(),
                       "",
                       pair->second.norm->getVal());
    // val->setError(sumx2[i]);
    out.addOwned(*val);
  }
  return;
}
void FitDiagnostics::getShapesAndNorms(RooAbsPdf *pdf,
                                       const RooArgSet &obs,
                                       std::map<std::string, ShapeAndNorm> &out,
  ...
  if (sim != 0) {
    ...
    for (int i = 0, n = cat.numBins((const char *)0); i < n; ++i) {
      ...
      if (pdfi)
        getShapesAndNorms(pdfi, obs, out, cat.getLabel());
    }
    return;
  }
  ...
  if (prod != 0) {
    ...
    for (int i = 0, n = list.getSize(); i < n; ++i) {
      ...
      if (pdfi->dependsOn(obs))
        getShapesAndNorms(pdfi, obs, out, channel);
    }
    return;
  }
  ...
  if (add != 0) {
    ...
    for (int i = 0, n = clist.getSize(); i < n; ++i) {
      ...
      if (coeffName.find(filterString_) == std::string::npos)
        continue;
      ShapeAndNorm &ns = out[coeffName];
      ns.norm = coeff;
      ns.pdf = (RooAbsPdf *)plist.at(i);
      ns.channel = (coeff->getStringAttribute("combine.channel") ? coeff->getStringAttribute("combine.channel")
                                                                 : channel.c_str());
      ns.process = (coeff->getStringAttribute("combine.process") ? coeff->getStringAttribute("combine.process")
                                                                 : ns.norm->GetName());
      ns.signal = (coeff->getStringAttribute("combine.process") ? coeff->getAttribute("combine.signal")
                                                                : (strstr(ns.norm->GetName(), "shapeSig") != 0));
...
void FitDiagnostics::getNormalizations(RooAbsPdf *pdf,
                                       const RooArgSet &obs,
                                       RooArgSet &out,
  ...
  if (shapeDir) {
    for (IT it = snm.begin(), ed = snm.end(); it != ed; ++it) {
      ...
      if (sub == 0) {
        ...
        RooDataHist *dataCut =
            (RooDataHist *)data.reduce(RooFit::Cut(TString("CMS_channel==CMS_channel::" + it->second.channel)));
  ...
  for (pair = bg, i = 0; pair != ed; ++pair, ++i) {
    ...
    if (fOut != 0 && saveShapes_ && pair->second.obs.getSize() == 1) {
      ...
      TH1 *hist =
          pair->second.pdf->createHistogram("", *x, pair->second.isfunc ? RooFit::Extended(false) : RooCmdArg::none());
      ...
      if (htot == 0) {
        ...
        TH2F *htot2covar =
            new TH2F("total_covar", "Covariance signal+background", bins[i], 0, bins[i], bins[i], 0, bins[i]);
  ...
  if (totalBins == 0)
    totalBins =
        1;  // cover the case, where there is only a counting experiment without a "fake" shape, which is essentially just 1 bin.

  //Total covariance
  TH2D *totOverall2Covar =
      new TH2D("overall_total_covar", "Covariance signal+background", totalBins, 0, totalBins, totalBins, 0, totalBins);
    ...
    for (int t = 0; t < ntoys; ++t) {
      ...
      for (pair = bg, i = 0; pair != ed; ++pair, ++i) {
        ...
        if (saveShapes_ && pair->second.obs.getSize() == 1) {
          ...
          RooRealVar *x = (RooRealVar *)pair->second.obs.at(0);
          std::unique_ptr<TH1> hist(pair->second.pdf->createHistogram(
              pair->second.pdf->GetName(), *x, pair->second.isfunc ? RooFit::Extended(false) : RooCmdArg::none()));
  ...
  for (pair = bg, i = 0; pair != ed; ++pair, ++i) {
    RooRealVar *val = new RooRealVar(
        (oldNormNames_ ? pair->first : pair->second.channel + "/" + pair->second.process).c_str(), "", vals[i]);
    val->setError(sumx2[i]);
    out.addOwned(*val);
    if (shapes[i])
...
void FitDiagnostics::setShapesFitResultTrees(std::map<std::string, ShapeAndNorm> &snm, double *vals) {
  ...
  for (pair = bg, i = 0; pair != ed; ++pair, ++i) {
    if (pair->second.obs.getSize() == 1) {
      RooRealVar *x = (RooRealVar *)pair->second.obs.at(0);
      TH1 *hist = pair->second.pdf->createHistogram(
          Form("%d", iBinOverall), *x, pair->second.isfunc ? RooFit::Extended(false) : RooCmdArg::none());
      hist->Scale(pair->second.norm->getVal() / hist->Integral("width"));
...
void FitDiagnostics::createFitResultTrees(const RooStats::ModelConfig &mc, bool withSys) {
  ...
  for (pair = bg, i = 0; pair != ed; ++pair, ++i) {
    ...
    if (pair->second.obs.getSize() == 1) {
      TH1 *hist = pair->second.pdf->createHistogram(
          Form("%d", totalBins), *x, pair->second.isfunc ? RooFit::Extended(false) : RooCmdArg::none());
  ...
  for (pair = bg, i = 0; pair != ed; ++pair, ++i) {
    if (pair->second.obs.getSize() == 1) {
      RooRealVar *x = (RooRealVar *)pair->second.obs.at(0);
      TH1 *hist = pair->second.pdf->createHistogram(
          Form("%d", count), *x, pair->second.isfunc ? RooFit::Extended(false) : RooCmdArg::none());
...
FitDiagnostics::ToySampler::ToySampler(RooAbsPdf *pdf, const RooArgSet *nuisances) : pdf_(pdf), data_(0) {
  nuisances->snapshot(snapshot_);
}

FitDiagnostics::ToySampler::~ToySampler() { delete data_; }
...
const RooAbsCollection &FitDiagnostics::ToySampler::get(int itoy) { return *data_->get(itoy); }

const RooAbsCollection &FitDiagnostics::ToySampler::centralValues() { return snapshot_; }
