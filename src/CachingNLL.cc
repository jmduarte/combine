...
#include "combine/CMSHistV.h"
#include "combine/CMSHistFunc.h"
#include "combine/CMSHistErrorPropagator.h"
#include "combine/CMSHistFuncWrapper.h"
#include "combine/VectorizedGaussian.h"
#include "combine/VectorizedCB.h"
#include "combine/VectorizedSimplePdfs.h"
#include "combine/VectorizedHistFactoryPdfs.h"
#include "combine/CachingMultiPdf.h"
#include "combine/RooCheapProduct.h"
#include "combine/Accumulators.h"
...
namespace cacheutils {
  ...
  public:
    ...
    ReminderSum(const ReminderSum &other, const char *name = 0)
        : RooAbsReal(other, name), list_("deps", this, other.list_), terms_(other.terms_) {}
    ~ReminderSum() {}
...
namespace {
  template <unsigned int> void tracePoint(const RooAbsCollection &point) {
    ...
    if (&point != lastPoint) {
      ...
      for (RooAbsArg *a = (RooAbsArg *)(iter.Next()); a != 0; a = (RooAbsArg *)(iter.Next())) {
        ...
        if (!rrv)
          continue;
        if (values[i] != rrv->getVal())
          std::cout << a->GetName() << " " << values[i] << " => " << rrv->getVal() << "    ";
        values[i++] = rrv->getVal();
      }
      std::cout << std::endl;
    }
...
namespace {
  unsigned long CachingSimNLLEvalCount = 0;
}
#endif

cacheutils::ArgSetChecker::ArgSetChecker(const RooAbsCollection &set) {
  std::unique_ptr<TIterator> iter(set.createIterator());
  for (RooAbsArg *a = dynamic_cast<RooAbsArg *>(iter->Next()); a != 0; a = dynamic_cast<RooAbsArg *>(iter->Next())) {
    RooRealVar *rrv = dynamic_cast<RooRealVar *>(a);
...
cacheutils::ValuesCache::ValuesCache(const RooAbsCollection &params, int size)
    : size_(1), maxSize_(size), directMode_(false) {
...
std::pair<std::vector<Double_t> *, bool> cacheutils::ValuesCache::get() {
  ...
  if (!good)
    items[found]->checker.changed(true);  // store new values in cache sentry
...
cacheutils::CachingPdf::CachingPdf(RooAbsReal *pdf, const RooArgSet *obs)
    : obs_(obs),
      pdfOriginal_(pdf),
      pdfPieces_(),
      pdf_(runtimedef::get("CACHINGPDF_NOCHEAPCLONE") ? utils::fullCloneFunc(pdfOriginal_, pdfPieces_)
...
void cacheutils::CachingPdf::newData_(const RooAbsData &data) {
  lastData_ = &data;
...
void cacheutils::CachingPdf::realFill_(const RooAbsData &data, std::vector<Double_t> &vals) {
#ifdef DEBUG_CACHE
...
void cacheutils::OptimizedCachingPdfT<PdfT, VPdfT>::newData_(const RooAbsData &data) {
  lastData_ = &data;
  pdf_->optimizeCacheMode(*data.get());
  pdf_->attachDataSet(data);
  const_cast<RooAbsData *>(lastData_)->setDirtyProp(false);
...
template <typename PdfT, typename VPdfT>
void cacheutils::OptimizedCachingPdfT<PdfT, VPdfT>::realFill_(const RooAbsData &data, std::vector<Double_t> &vals) {
  vpdf_->fill(vals);
}

cacheutils::ReminderSum::ReminderSum(const char *name, const char *title, const RooArgList &sumSet)
    : RooAbsReal(name, title), list_("deps", "", this) {
...
cacheutils::CachingAddNLL::CachingAddNLL(
    const char *name, const char *title, RooAbsPdf *pdf, RooAbsData *data, bool includeZeroWeights)
  ...
  if (pdf == 0)
    throw std::invalid_argument(std::string("Pdf passed to ") + name + " is null");
  setData(*data);
  setup_();
  propagateData();
...
cacheutils::CachingAddNLL::~CachingAddNLL() {
  for (int i = 0, n = integrals_.size(); i < n; ++i)
    delete integrals_[i];
  integrals_.clear();
}

cacheutils::CachingAddNLL *cacheutils::CachingAddNLL::clone(const char *name) const {
...
void cacheutils::CachingAddNLL::addPdfs_(RooAddPdf *addpdf, bool recursive, const RooArgList &basecoeffs) {
  ...
  if (npdf == addpdf->coefList().getSize()) {
    ...
    prods_.push_back(
        new ReminderSum((std::string("reminder_of_") + addpdf->GetName()).c_str(), "", addpdf->coefList()));
...
cacheutils::CachingPdfBase *cacheutils::makeCachingPdf(RooAbsReal *pdf, const RooArgSet *obs) {
  ...
  if (histNll && typeid(*pdf) == typeid(FastVerticalInterpHistPdf)) {
    ...
    if (verb) {
      std::cout << "I don't have an optimized implementation for " << pdf->ClassName() << " (" << pdf->GetName() << ")"
                << std::endl;
    }
    return new CachingPdf(pdf, obs);
  }
...
void cacheutils::CachingAddNLL::setup_() {
  ...
  if ((addpdf = dynamic_cast<RooAddPdf *>(pdf_)) != 0) {
    ...
    const RooArgSet *obs = data_->get();
    isRooRealSum_ = true;
    basicIntegrals_ = canBasicIntegrals_;
    ...
    for (int i = 0; i < npdf; ++i) {
      ...
      if (tryfactor && ((prodi = dynamic_cast<RooProduct *>(funci)) != 0)) {
        ...
        if (newcoeffs.getSize() > 1) {
          if (cheapprod)
            prods_.push_back(new RooCheapProduct("", "", newcoeffs, runtimedef::get("ADDNLL_ROOREALSUM_PRUNECONST")));
          else
...
void cacheutils::CachingAddNLL::setIncludeZeroWeights(bool includeZeroWeights) {
  includeZeroWeights_ = includeZeroWeights;
...
Double_t cacheutils::CachingAddNLL::evaluate() const {
  ...
  if (!multiNll && !multiPdfs_.empty()) {
    for (std::vector<std::pair<const RooMultiPdf *, CachingPdfBase *>>::iterator itp = multiPdfs_.begin(),
                                                                                 edp = multiPdfs_.end();
         itp != edp;
         ++itp) {
      bool hasChangedPdf = itp->first->checkIndexDirty();
      if (hasChangedPdf)
        itp->second->setDataDirty();
    }
  }
  ...
  for (; itc != edc; ++itp, ++itc) {
    ...
    if (basicIntegrals_) {
      double integral = (binWidths_.size() > 1) ? vectorized::dot_product(pdfvals.size(), &pdfvals[0], &binWidths_[0])
        ...
        if (refintegral > 0) {
          if (std::abs((integral - refintegral) / refintegral) > 1e-5) {
            printf("integrals don't match: %+10.6f  %+10.6f  %10.7f %s\n",
                   refintegral,
                   integral,
                   refintegral ? std::abs((integral - refintegral) / refintegral) : 0,
                   itp->pdf()->GetName());
    ...
    if (!isnormal(*its) || *its <= 0) {
      if ((weights_[its - bgs] == 0) && (*its == 0)) {
        ...
        *its = 1.0;  // arbitrary number, to avoid log(0)
        ...
        static int nwarn = 0;
        if (++nwarn < 100) {
          std::cout << "WARNING: underflow to " << *its << " in " << pdf_->GetName() << " for zero-entry bin "
                    << its - bgs << std::endl;
        }
      ...
      if (gentleNegativePenalty_ && abs(weights_[its - bgs]) < 1e-2) {
        std::cout << "WARNING: gentle underflow to " << *its << " in " << pdf_->GetName() << " for bin " << its - bgs
                  << ", weight " << weights_[its - bgs] << std::endl;
        *its = 1.0;  // skip the log
        ret -= 25;   // add a penalty (negative since we flip 'ret' afterwards)
        continue;
      }
      std::cout << "WARNING: underflow to " << *its << " in " << pdf_->GetName() << " for bin " << its - bgs
                << ", weight " << weights_[its - bgs] << std::endl;
      if (!CachingSimNLL::noDeepLEE_)
        logEvalError("Number of events is negative or error");
      else
        CachingSimNLL::hasError_ = true;
      if (fastExit_) {
        std::cout << "FASTEXIT from " << pdf_->GetName() << std::endl;
        return 9e9;
      } else
        *its = 1;
    }
...
void cacheutils::CachingAddNLL::clearConstantZeroPoint() {
    ...
    if (obs->getSize() == 1 && xvar != 0 && xvar->numBins() == data_->numEntries()) {
      ...
      for (unsigned int ibin = 0, nbin = binWidths_.size(); ibin < nbin; ++ibin) {
        ...
        if (std::abs(bc - dc) > 1e-5 * binWidths_[ibin]) {
          printf(
              "channel %s, for observable %s, bin %d mismatch: binning %+8.5f ( data %+8.5f , diff %+7.8f of width "
              "%8.5f\n",
              pdf_->GetName(),
              xvar->GetName(),
              ibin,
...
RooArgSet *cacheutils::CachingAddNLL::getObservables(const RooArgSet *depList, Bool_t valueOnly) const {
  return new RooArgSet();
}

RooArgSet *cacheutils::CachingAddNLL::getParameters(const RooArgSet *depList, Bool_t stripDisconnected) const {
...
cacheutils::CachingSimNLL::CachingSimNLL(RooSimultaneous *pdf, RooAbsData *data, const RooArgSet *nuis)
    : pdfOriginal_(pdf), dataOriginal_(data), nuis_(nuis), params_("params", "parameters", this) {
  setup_();
}
...
cacheutils::CachingSimNLL *cacheutils::CachingSimNLL::clone(const char *name) const {
...
cacheutils::CachingSimNLL::~CachingSimNLL() {
  ...
  for (std::vector<SimpleGaussianConstraint *>::iterator it = constrainPdfsFast_.begin(), ed = constrainPdfsFast_.end();
       it != ed;
       ++it, ++ito) {
    if (*ito) {
      delete *it;
  ...
  for (std::vector<SimplePoissonConstraint *>::iterator it = constrainPdfsFastPoisson_.begin(),
                                                        ed = constrainPdfsFastPoisson_.end();
       it != ed;
       ++it, ++ito) {
    if (*ito) {
      delete *it;
...
void cacheutils::CachingSimNLL::setup_() {
  ...
  factorizedPdf_.reset(
      dynamic_cast<RooSimultaneous *>(utils::factorizePdf(*dataOriginal_->get(), *pdfclone, constraints)));
  ...
  for (int ib = 0, nb = pdfs_.size(); ib < nb; ++ib) {
    ...
    if (pdf != 0) {
      ...
      bool includeZeroWeights =
          (runtimedef::get("ADDNLL_ROOREALSUM_BASICINT") && runtimedef::get("ADDNLL_ROOREALSUM_KEEPZEROS") &&
           (dynamic_cast<RooRealSumPdf *>(pdf) != 0));
...
Double_t cacheutils::CachingSimNLL::evaluate() const {
  ...
  if (!constrainPdfs_.empty() || !constrainPdfsFast_.empty()) {
    ...
    for (std::vector<RooAbsPdf *>::const_iterator it = constrainPdfs_.begin(), ed = constrainPdfs_.end(); it != ed;
         ++it, ++itz) {
      ...
      if (!isnormal(pdfval) || pdfval <= 0) {
        ...
        Logger::instance().log(
            ...
            Logger::kLogLevelInfo,
            __func__);
        if (gentleNegativePenalty_) {
          ret += 25;
          continue;
        }
        if (!noDeepLEE_)
          logEvalError(
              (std::string("Constraint pdf ") + (*it)->GetName() + " evaluated to zero, negative or error").c_str());
        pdfval = 1e-9;
      }
      ret2 += (log(pdfval) + *itz);
    }
    ...
    for (std::vector<SimpleGaussianConstraint *>::const_iterator it = constrainPdfsFast_.begin(),
                                                                 ed = constrainPdfsFast_.end();
         it != ed;
         ++it, ++itz) {
    ...
    for (std::vector<SimplePoissonConstraint *>::const_iterator it = constrainPdfsFastPoisson_.begin(),
                                                                ed = constrainPdfsFastPoisson_.end();
         it != ed;
         ++it, ++itz) {
      double logpdfval = (*it)->getLogValFast();
  ...
  static unsigned long _trace_ = 0;
  _trace_++;
  if (_trace_ % 10 == 0) {
    putchar('.');
    fflush(stdout);
...
void cacheutils::CachingSimNLL::setData(const RooAbsData &data) {
  ...
  if (!(RooCategory *)data.get()->find("CMS_channel")) {
    throw std::logic_error(
        "Error: no category in dataset. You should try to recreate your datacard as a Fake shape -- combineCards.py "
        "mycard.txt -S > myshapecard.txt OR rerun with option --forceRecreateNLL");
    assert(0);
  }
...
void cacheutils::CachingSimNLL::splitWithWeights(const RooAbsData &data,
  ...
  if (cat == 0)
    throw std::logic_error("Error: no category");
  ...
  obs.remove(*cat, true, true);
  RooRealVar weight("_weight_", "", 1);
  RooArgSet obsplus(obs);
  obsplus.add(weight);
  if (nb != int(datasets_.size()))
    throw std::logic_error("Number of categories changed");  // this can happen due to bugs in RooDataSet
  std::vector<int> includeZeroWeights(nb, 0);
  bool includeZeroWeightsAny = false;
...
RooArgSet *cacheutils::CachingSimNLL::getObservables(const RooArgSet *depList, Bool_t valueOnly) const {
  return new RooArgSet();
}

RooArgSet *cacheutils::CachingSimNLL::getParameters(const RooArgSet *depList, Bool_t stripDisconnected) const {
  return new RooArgSet(params_);
}
