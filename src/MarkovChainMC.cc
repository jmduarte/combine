...
#include "combine/JacknifeQuantile.h"
...
int MarkovChainMC::runOnce(RooWorkspace *w,
  ...
  if (cropNSigmas_ > 0) {
    ...
    for (int i = 0, n = fpf.getSize(); i < n; ++i) {
      ...
      if (std::string(r->GetName()) == fv->GetName())
        continue;
  ...
  if (proposalType_ != UniformP) {
    ...
    if (proposalHelperUniformFraction_ > 0)
      ph.SetUniformFraction(proposalHelperUniformFraction_);
  }

  std::unique_ptr<DebugProposal> pdfDebugProp(
      debugProposal_ > 0 ? new DebugProposal(pdfProp, mc_s->GetPdf(), &data, debugProposal_) : 0);
  ...
  MCMCInterval *newInterval = new MCMCInterval(TString("MCMCIntervalCloned_") + TString(mc.GetName()),
                                               RooArgSet(*mc_s->GetParametersOfInterest()),
                                               *clonedChain);
...
void MarkovChainMC::limitAndError(double &limit, double &limitErr, const std::vector<double> &limitsIn) const {
  ...
  if (adaptiveTruncation_ && num >= 10) {
    ...
    while (start < end && limits[start] < min)
      ++start;
    while (end > start && limits[end] > max)
      --end;
    num = end - start + 1;
    ...
    limit /= num;
    for (int k = start; k <= end; k++)
      limitErr += (limits[k] - limit) * (limits[k] - limit);
...
RooStats::MarkovChain *MarkovChainMC::mergeChains(const RooArgSet &poi, const std::vector<double> &limits) const {
  std::vector<double> limitsSorted(limits);
  ...
  TIter iter(&chains_);
  int index = 0;
  for (RooStats::MarkovChain *other = (RooStats::MarkovChain *)iter.Next(); other != 0;
       other = (RooStats::MarkovChain *)iter.Next(), ++index) {
    if (limits[index] < lmin || limits[index] > lmax)
      continue;
    int burninSteps =
...
void MarkovChainMC::readChains(const RooArgSet &poi, std::vector<double> &limits) {
  ...
  TKey *k;
  while ((k = (TKey *)next()) != 0) {
    if (TString(k->GetName()).Index(prefix) != 0)
      continue;
...
void MarkovChainMC::limitFromChain(
    double &limit, double &limitErr, const RooArgSet &poi, RooStats::MarkovChain &chain, int burnInSteps) {
  if (burnInSteps < 0) {
    if (adaptiveBurnIn_)
      burnInSteps = guessBurnInSteps(chain);
    else
      burnInSteps = max<int>(burnInSteps_, chain.Size() * burnInFraction_);
  }
...
RooStats::MarkovChain *MarkovChainMC::slimChain(const RooArgSet &poi, const RooStats::MarkovChain &chain) const {
  ...
  for (int i = 0, n = chain.Size(); i < n; ++i) {
    ...
    if (i)
      ret->AddFast(entry, nll, weight);
    else
      ret->Add(entry, nll, weight);
  }
  return ret;
}
...
int MarkovChainMC::stationarityTest(const RooStats::MarkovChain &chain, const RooArgSet &poi, int nchunks) const {
  ...
  for (int c = 0; c < nchunks; ++c) {
    for (int c2 = 0; c2 < nchunks; ++c2) {
      if (c2 != c)
        dists.push_back(fabs(mean[c] - mean[c2]));
    }
