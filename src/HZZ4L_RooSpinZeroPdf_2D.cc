...
ClassImp(HZZ4L_RooSpinZeroPdf_2D)
    ...
    HZZ4L_RooSpinZeroPdf_2D::HZZ4L_RooSpinZeroPdf_2D(const char* name,
                                                     const char* title,
                                                     RooAbsReal& _kd,
                                                     RooAbsReal& _kdint,
                                                     RooAbsReal& _ksmd,
...

{
  ...
  while ((func = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "ERROR: :HZZ4L_RooSpinZeroPdf_2D(" << GetName() << ") funcficient " << func->GetName()
...
Double_t HZZ4L_RooSpinZeroPdf_2D::evaluate() const {
  ...
  value = (1. - fabs(fai1) - fabs(fai2)) * T1 + fabs(fai1) * T2 + fabs(fai2) * T3 +
  ...
  if ((1. - fabs(fai1) - fabs(fai2)) < 0 || value <= 0.)
    return 1.0e-15;

  return value;
}

Int_t HZZ4L_RooSpinZeroPdf_2D::getAnalyticalIntegral(RooArgSet& allVars,
                                                     RooArgSet& analVars,
                                                     const char* /*rangeName*/) const {
  if (matchArgs(allVars, analVars, RooArgSet(*kd.absArg(), *kdint.absArg(), *ksmd.absArg())))
    return 4;
...
Double_t HZZ4L_RooSpinZeroPdf_2D::analyticalIntegral(Int_t code, const char* rangeName) const {
  switch (code) {
    ...
    case 4: {
      ...
      double integral =
          (1. - fabs(fai1) - fabs(fai2)) * Int_T1 + fabs(fai1) * Int_T2 + fabs(fai2) * Int_T3 +
      ...
      if ((1 - fabs(fai1) - fabs(fai2)) < 0 || integral <= 0.)
        integral = 1.0e-10;
