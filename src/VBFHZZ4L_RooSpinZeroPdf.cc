...
ClassImp(VBFHZZ4L_RooSpinZeroPdf)
    ...
    VBFHZZ4L_RooSpinZeroPdf::VBFHZZ4L_RooSpinZeroPdf(const char* name,
                                                     const char* title,
                                                     RooAbsReal& _kd,
                                                     RooAbsReal& _kdint,
                                                     RooAbsReal& _ksmd,
  ...
  while ((func = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "ERROR: :VBFHZZ4L_RooSpinZeroPdf(" << GetName() << ") funcficient " << func->GetName()
...
Double_t VBFHZZ4L_RooSpinZeroPdf::evaluate() const {
  ...
  value = (a1 * a1 * a1 * a1 * T1 + a1 * a1 * a1 * ai * T2 + a1 * a1 * ai * ai * T3 + a1 * ai * ai * ai * T4 +
           ai * ai * ai * ai * T5);
...
Int_t VBFHZZ4L_RooSpinZeroPdf::getAnalyticalIntegral(RooArgSet& allVars,
                                                     RooArgSet& analVars,
...
Double_t VBFHZZ4L_RooSpinZeroPdf::analyticalIntegral(Int_t code, const char* rangeName) const {
  switch (code) {
    case 4: {
      ...
      double integral = (a1 * a1 * a1 * a1 * Int_T1 + a1 * a1 * a1 * ai * Int_T2 + a1 * a1 * ai * ai * Int_T3 +
                         a1 * ai * ai * ai * Int_T4 + ai * ai * ai * ai * Int_T5);
