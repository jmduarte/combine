...
namespace {
  ...
  public:
    RooProductWithAccessors(const RooProduct &other) : RooProduct(other) {}
...
void utils::printRDH(RooAbsData *data) {
  ...
  for (size_t j = 0; j < nv; ++j) {
    printf("%16.16s  ", varnames[j].c_str());
  }
  for (size_t j = 0; j < nc; ++j) {
    printf("%16.16s  ", catnames[j].c_str());
  }
  printf("  weight\n");
  for (int i = 0, nb = data->numEntries(); i < nb; ++i) {
    ...
    printf("%4d  ", i);
    for (size_t j = 0; j < nv; ++j) {
      printf("%16g  ", bin->getRealValue(varnames[j].c_str()));
    }
    for (size_t j = 0; j < nc; ++j) {
      printf("%16.16s  ", bin->getCatLabel(catnames[j].c_str()));
    }
    printf("%12.7f\n", data->weight());
  }
}
...
void utils::printRAD(const RooAbsData *d) {
  if (d->InheritsFrom("RooDataHist") || d->numEntries() != 1)
...
RooAbsPdf *utils::factorizePdf(const RooArgSet &observables, RooAbsPdf &pdf, RooArgList &constraints) {
  ...
  if (id == typeid(RooProdPdf)) {
    ...
    if (!needNew && newFactors.getSize() > 1) {
    ...
    if (id == typeid(RooSimultaneousOpt)) {
      ...
      if (o.extraConstraints().getSize() > 0)
        needNew = true;
    ...
    if (needNew) {
      if (id == typeid(RooSimultaneousOpt)) {
        ret = new RooSimultaneousOpt(
            TString::Format("%s_obsOnly", pdf.GetName()), "", (RooAbsCategoryLValue &)sim->indexCat());
      } else {
        ret = new RooSimultaneous(
            TString::Format("%s_obsOnly", pdf.GetName()), "", (RooAbsCategoryLValue &)sim->indexCat());
      }
      for (int ic = 0, nc = nbins; ic < nc; ++ic) {
        cat->setBin(ic);
...
void utils::factorizePdf(
    RooStats::ModelConfig &model, RooAbsPdf &pdf, RooArgList &obsTerms, RooArgList &constraints, bool debug) {
  return factorizePdf(*model.GetObservables(), pdf, obsTerms, constraints, debug);
}
void utils::factorizePdf(
    const RooArgSet &observables, RooAbsPdf &pdf, RooArgList &obsTerms, RooArgList &constraints, bool debug) {
  assert(&pdf);
  const std::type_info &id = typeid(pdf);
  if (id == typeid(RooProdPdf)) {
    RooProdPdf *prod = dynamic_cast<RooProdPdf *>(&pdf);
...
RooArgList utils::factors(const RooProduct &prod) { return ::RooProductWithAccessors(prod).realTerms(); }
void utils::factorizeFunc(const RooArgSet &observables,
  ...
  if (id == typeid(RooProduct)) {
    ...
    if (!obsTerms.contains(func) || keepDuplicate)
      obsTerms.add(func);
  } else {
    if ((!constraints.contains(func) && (!func.getAttribute("ignoreConstraint"))) || keepDuplicate)
...
RooAbsReal *utils::fullCloneFunc(const RooAbsReal *pdf, const RooArgSet &obs, RooArgSet &holder, bool cloneLeafNodes) {
  ...
  for (RooAbsArg *a = iter.next(); a != 0; a = iter.next()) {
    if (a == pdf)
      toClone.add(*a);
    else if (a->dependsOn(obs))
      toClone.add(*a);
  }
  unsigned int nobsitems = toClone.getSize();
  toClone.snapshot(holder, cloneLeafNodes);
  if (runtimedef::get("fullCloneFunc_VERBOSE"))
    std::cout << "For PDF " << pdf->GetName() << ", cloned " << nobsitems << "/" << nitems << " items" << std::endl;
  // Find the top level FUNC in the snapshot list
  return (RooAbsReal *)holder.find(pdf->GetName());
}
...
void utils::getClients(const RooAbsCollection &values, const RooAbsCollection &allObjects, RooAbsCollection &clients) {
  ...
  for (RooAbsArg *v = (RooAbsArg *)iterVal->Next(); v != 0; v = (RooAbsArg *)iterVal->Next()) {
    if (typeid(*v) != typeid(RooRealVar) && typeid(*v) != typeid(RooCategory))
      continue;
    ...
    for (RooAbsArg *a = (RooAbsArg *)clientIter->Next(); a != 0; a = (RooAbsArg *)clientIter->Next()) {
      if (allObjects.containsInstance(*a) && !clients.containsInstance(*a))
        clients.add(*a);
    }
  }
}
...
bool utils::checkModel(const RooStats::ModelConfig &model, bool throwOnFail) {
  ...
  if (model.GetParametersOfInterest() == 0) {
    ...
    for (RooAbsArg *a = (RooAbsArg *)iter->Next(); a != 0; a = (RooAbsArg *)iter->Next()) {
      ...
      if (!v) {
        ok = false;
        errors << "ERROR: parameter of interest " << a->GetName() << " is a " << a->ClassName()
               << " and not a RooRealVar\n";
        continue;
      }
      if (v->isConstant()) {
        ok = false;
        errors << "ERROR: parameter of interest " << a->GetName() << " is constant\n";
        continue;
      }
      if (!pdf->dependsOn(*v)) {
        ok = false;
  ...
  if (model.GetNuisanceParameters() != 0) {
    ...
    for (RooAbsArg *a = (RooAbsArg *)iter->Next(); a != 0; a = (RooAbsArg *)iter->Next()) {
      ...
      if (!v) {
        ok = false;
        errors << "ERROR: nuisance parameter " << a->GetName() << " is a " << a->ClassName()
               << " and not a RooRealVar\n";
        continue;
      }
      if (v->isConstant()) {
      ...
      if (!pdf->dependsOn(*v)) {
        errors << "WARNING: pdf does not depend on nuisance parameter " << a->GetName() << "\n";
        continue;
      }
      allowedToFloat.add(*v);
    }
  ...
  if (model.GetGlobalObservables() != 0) {
    ...
    for (RooAbsArg *a = (RooAbsArg *)iter->Next(); a != 0; a = (RooAbsArg *)iter->Next()) {
      ...
      if (!v) {
        ok = false;
        errors << "ERROR: global observable " << a->GetName() << " is a " << a->ClassName()
               << " and not a RooRealVar\n";
        continue;
      }
      if (!v->isConstant()) {
  ...
  for (RooAbsArg *a = (RooAbsArg *)iter->Next(); a != 0; a = (RooAbsArg *)iter->Next()) {
    if (a->getAttribute("flatParam") && a->isConstant()) {
      ok = false;
      errors << "ERROR: parameter " << a->GetName() << " is declared as flatParam but is constant.\n";
    }
    ...
    if (a->getAttribute("flatParam")) {
      ...
      if (rrv->getVal() > rrv->getMax() || rrv->getVal() < rrv->getMin()) {
        ok = false;
        errors << "ERROR: flatParam " << rrv->GetName() << " has a value " << rrv->getVal()
    ...
    if (v != 0 && !v->isConstant() && (v->getVal() < v->getMin() || v->getVal() > v->getMax())) {
      ok = false;
      errors << "ERROR: parameter " << a->GetName() << " has value " << v->getVal() << " outside range [ "
             << v->getMin() << " , " << v->getMax() << " ]\n";
    }
  }
  iter.reset();
  ...
  if (model.GetNuisanceParameters() != 0) {
    RooArgList constraints;
    std::unique_ptr<RooAbsPdf> factorizedPdf(
        utils::factorizePdf(*model.GetObservables(), *model.GetPdf(), constraints));
    if (factorizedPdf.get() == 0 || factorizedPdf.get() == model.GetPdf()) {
      factorizedPdf.release();
      ok = false;
      errors << "ERROR: have nuisance parameters, but can't factorize the pdf\n";
    }
...
RooSimultaneous *utils::rebuildSimPdf(const RooArgSet &observables, RooSimultaneous *sim) {
  ...
  RooSimultaneous *ret =
      new RooSimultaneous(TString::Format("%s_reloaded", sim->GetName()), "", (RooAbsCategoryLValue &)sim->indexCat());
  for (int ic = 0, nc = nbins; ic < nc; ++ic) {
    cat->setBin(ic);
    RooAbsPdf *newpdf = (RooAbsPdf *)factorizedPdfs[ic];
    if (newpdf) {
      if (constraints.getSize() > 0) {
        RooArgList allfactors(constraints);
        allfactors.add(*newpdf);
...
void utils::copyAttributes(const RooAbsArg &from, RooAbsArg &to) {
  ...
  if (!strattribs.empty()) {
    for (std::map<std::string, std::string>::const_iterator it = strattribs.begin(), ed = strattribs.end(); it != ed;
         ++it)
  ...
  if (id == typeid(RooSimultaneous) || id == typeid(RooSimultaneousOpt)) {
    ...
    for (RooAbsData *ds = (RooAbsData *)next(); ds != 0; ds = (RooAbsData *)next()) {
      ...
      for (RooAbsArg *a = 0; (a = (RooAbsArg *)obs_iter->Next()) != 0;) {
        ...
        if (nbins == 0)
          nbins = 100;
        if (nbins / rebinFactor > 6)
          nbins = ceil(nbins / rebinFactor);
        ...
        if (fitRes)
          pdfi->plotOn(ret.back(),
                       RooFit::Normalization(pdfi->expectedEvents(RooArgSet(*x)), RooAbsReal::NumEvent),
                       RooFit::VisualizeError(*fitRes, 1),
                       RooFit::FillColor(kOrange));
        if (signalSel && strlen(signalSel))
          pdfi->plotOn(ret.back(),
                       RooFit::LineColor(209),
                       RooFit::Components(signalSel),
                       RooFit::Normalization(pdfi->expectedEvents(RooArgSet(*x)), RooAbsReal::NumEvent));
        if (signalSel && strlen(signalSel))
          pdfi->plotOn(ret.back(), RooFit::LineColor(209), RooFit::Components(signalSel));
        if (backgroundSel && strlen(backgroundSel))
          pdfi->plotOn(ret.back(),
                       RooFit::LineColor(206),
                       RooFit::Components(backgroundSel),
                       RooFit::Normalization(pdfi->expectedEvents(RooArgSet(*x)), RooAbsReal::NumEvent));
        std::cout << "[utils::makePlots] Number of events for pdf in " << ret.back()->GetName() << ", pdf "
                  << pdfi->GetName() << " = " << pdfi->expectedEvents(RooArgSet(*x)) << std::endl;
    ...
    for (RooAbsArg *a = 0; (a = (RooAbsArg *)obs_iter->Next()) != 0;) {
      ...
      if (x != 0) {
        ...
        if (fitRes)
          pdf.plotOn(ret.back(),
                     RooFit::Normalization(pdf.expectedEvents(RooArgSet(*x)), RooAbsReal::NumEvent),
                     RooFit::VisualizeError(*fitRes, 1),
                     RooFit::FillColor(kOrange));
        if (signalSel && strlen(signalSel))
          pdf.plotOn(ret.back(),
        ...
        if (backgroundSel && strlen(backgroundSel))
          pdf.plotOn(ret.back(),
                     ...
                     RooFit::Normalization(pdf.expectedEvents(RooArgSet(*x)), RooAbsReal::NumEvent));
        std::cout << "[utils::makePlots] Number of events for pdf in " << ret.back()->GetName() << ", pdf "
                  << pdf.GetName() << " = " << pdf.expectedEvents(RooArgSet(*x)) << std::endl;
        pdf.plotOn(ret.back(),
                   RooFit::Normalization(pdf.expectedEvents(RooArgSet(*x)), RooAbsReal::NumEvent),
                   RooFit::VisualizeError(*fitRes, 1));
        data.plotOn(ret.back(), RooFit::DataError(RooAbsData::Poisson));
...
void utils::CheapValueSnapshot::readFrom(const RooAbsCollection &src) {
  ...
  RooLinkedListIter iter = src.iterator();
  int i = 0;
...
void utils::CheapValueSnapshot::writeTo(const RooAbsCollection &src) const {
  if (&src == src_) {
    ...
    RooLinkedListIter iter = src_->iterator();
    int i = 0;
    for (RooAbsArg *a = (RooAbsArg *)iter.Next(); a != 0; a = (RooAbsArg *)iter.Next(), ++i) {
      RooAbsArg *a2 = src.find(a->GetName());
      if (a2 == 0)
        continue;
...
void utils::setModelParameters(const std::string &setPhysicsModelParameterExpression, const RooArgSet &params) {
  ...
  for (UInt_t p = 0; p < SetParameterExpressionList.size(); ++p) {
    ...
    // check for regex syntax: rgx{regex}
    else if (boost::starts_with(SetParameterExpression[0], "rgx{") &&
             boost::ends_with(SetParameterExpression[0], "}")) {
      ...
      for (RooAbsArg *tmp = (RooAbsArg *)iter->Next(); tmp != 0; tmp = (RooAbsArg *)iter->Next()) {
        ...
        if (isrvar) {
          ...
          if (std::regex_match(target, match, rgx)) {
            ...
            cout << "Set Default Value of Parameter " << target << " To : " << PhysicsParameterValue << "\n";
          ...
          if (std::regex_match(target, match, rgx)) {
            ...
            cout << "Set Default Index of Parameter " << target << " To : " << PhysicsParameterValue << "\n";
      ...
      if (tmp) {
        ...
        if (isrvar) {
          ...
          RooCategory *tmpCategory = dynamic_cast<RooCategory *>(tmp);
          int PhysicsParameterValue = atoi(SetParameterExpression[1].c_str());
          cout << "Set Default Index of Parameter " << SetParameterExpression[0] << " To : " << PhysicsParameterValue
               << "\n";
          tmpCategory->setIndex(PhysicsParameterValue);
        }
      } else {
...
void utils::setModelParameterRanges(const std::string &setPhysicsModelParameterRangeExpression,
  ...
  for (UInt_t p = 0; p < SetParameterRangeExpressionList.size(); ++p) {
    ...
    if (SetParameterRangeExpression.size() != 3) {
      std::cout << "Error parsing physics model parameter expression : " << SetParameterRangeExpressionList[p] << endl;
               ...
               (SetParameterRangeExpression[2] == "inf" || SetParameterRangeExpression[2] == "+inf")) {
      RooRealVar *tmpParameter = (RooRealVar *)params.find(SetParameterRangeExpression[0].c_str());
      if (tmpParameter) {
        cout << "Leave Parameter " << SetParameterRangeExpression[0] << " freely floating, with no range\n";
      ...
      // check for regex syntax: rgx{regex}
      if (boost::starts_with(SetParameterRangeExpression[0], "rgx{") &&
          boost::ends_with(SetParameterRangeExpression[0], "}")) {
        ...
        for (RooAbsArg *a = (RooAbsArg *)iter->Next(); a != 0; a = (RooAbsArg *)iter->Next()) {
          ...
          if (std::regex_match(target, match, rgx)) {
            if (tmpParameter->isConstant())
              continue;
            cout << "Set Range of Parameter " << target << " To : (" << PhysicsParameterRangeLow << ","
                 << PhysicsParameterRangeHigh << ")\n";
        ...
        RooRealVar *tmpParameter = (RooRealVar *)params.find(SetParameterRangeExpression[0].c_str());
        if (tmpParameter) {
          cout << "Set Range of Parameter " << SetParameterRangeExpression[0] << " To : (" << PhysicsParameterRangeLow
               << "," << PhysicsParameterRangeHigh << ")\n";
          tmpParameter->setRange(PhysicsParameterRangeLow, PhysicsParameterRangeHigh);
        } else {
          std::cout << "Warning: Did not find a parameter with name " << SetParameterRangeExpression[0] << endl;
...
void utils::createSnapshotFromString(const std::string expression,
                                     const RooArgSet &allvars,
                                     RooArgSet &output,
                                     const char *context) {
  if (expression.find("=") == std::string::npos) {
    if (allvars.getSize() != 1)
      throw std::invalid_argument(std::string("Error: the argument to ") + context +
                                  " is a single value, but there are multiple variables to choose from");
    ...
    if (errno != 0)
      std::invalid_argument(std::string("Error: the argument to ") + context + " is not a valid number.");
  } else {
    ...
    do {
      ...
      if (eqidx == std::string::npos || (colidx2 != std::string::npos && colidx2 < eqidx)) {
        throw std::invalid_argument(std::string("Error: the argument to ") + context +
                                    " is not in the forms 'value' or 'name1=value1,name2=value2,...'\n");
      }
      std::string poiName = expression.substr(colidx, eqidx - colidx);
      std::string poiVal =
          expression.substr(eqidx + 1, (colidx2 == std::string::npos ? std::string::npos : colidx2 - eqidx - 1));
      RooAbsArg *poi = allvars.find(poiName.c_str());
      if (poi == 0)
        throw std::invalid_argument(std::string("Error: unknown parameter '") + poiName + "' passed to " + context +
                                    ".");
      output.addClone(*poi);
      errno = 0;
      output.setRealValue(poi->GetName(), strtod(poiVal.c_str(), NULL));
      if (errno != 0)
        throw std::invalid_argument(std::string("Error: invalid value '") + poiVal + "' for parameter '" + poiName +
                                    "' passed to " + context + ".");
      colidx = colidx2 + 1;
...
void utils::reorderCombinations(std::vector<std::vector<int> > &vec,
