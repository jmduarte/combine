...
//_____________________________________________________________________________
RooStepBernstein::RooStepBernstein(
    const char* name, const char* title, RooAbsReal& x, RooAbsReal& step_thresh, const RooArgList& coefList)
...
ClassImp(RooGaussStepBernstein)
    ...
    namespace {
  double zeroth(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2.0);
    const double erf_parm_a = (a + m - x) / (root2 * s);
    const double erf_parm_b = (b + m - x) / (root2 * s);

    return (sqrt(M_PI / 2.0) * s * (erf(erf_parm_b) - erf(erf_parm_a)));
  }
  double zeroth_integral(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2.0);
    ...
    return rootpiover2 * s *
           (gaus_a * s * root2overpi - gaus_b * s * root2overpi + (a + m - x) * erf_a - (b + m - x) * erf_b);
  }
  double first(const double x, const double m, const double s, const double a, const double b) {
    ...
    const double gaus_a = exp(-0.5 * (a + m - x) * (a + m - x) / (s * s));
    const double gaus_b = exp(-0.5 * (b + m - x) * (b + m - x) / (s * s));
    return (s * s * (gaus_b - gaus_a) + rootpiover2 * s * (a + m - x) * (erf(erf_parm_b) - erf(erf_parm_a))) / (a - b);
  }
  double first_integral(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2.0);
    const double root2overpi = sqrt(2.0 / M_PI);
    const double rootpiover2 = sqrt(M_PI / 2.0);
    const double gaus_a = exp(-0.5 * (a + m - x) * (a + m - x) / (s * s));
    ...
    return rootpiover2 * s *
           (root2overpi * s * (gaus_a * (a + m - x) + gaus_b * (-2 * a + b - m + x)) +
            (s * s + (a + m - x) * (a + m - x)) * erf_a - (s * s + (2 * a - b + m - x) * (b + m - x)) * erf_b) /
           (2 * (a - b));
  }
  double second(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2);
    ...
    const double gaus_b = exp(-0.5 * (b + m - x) * (b + m - x) / (s * s));
    return s *
           (gaus_b * s * (2 * a - b + m - x) - gaus_a * s * (a + m - x) +
            rootpiover2 * (s * s + (a + m - x) * (a + m - x)) * (erf_b - erf_a)) /
           amb;
  }
  double second_integral(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2);
    ...
    const double poly4 = (b3 + 3 * a * (-b2 + s2 + mmx2) + (3 * s2 + mmx2) * mmx + 3 * a2 * (b + m - x));

    return s * (gaus_a * s * poly1 - gaus_b * s * poly2 + rootpiover2 * (poly3 * erf_a - poly4 * erf_b)) / amb;
  }
  double third(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2);
    ...
    const double apmmx = (a + m - x);
    const double apmmx2 = apmmx * apmmx;
    const double poly1 = (3 * a * a + b * b + 2 * s2 + (m - x) * (m - x) + b * (x - m) - 3 * a * (b - m + x));
    const double poly2 = (2 * s2 + apmmx2);
    const double poly3 = (3 * s2 + apmmx2) * apmmx;
    return s * (gaus_b * s * poly1 - gaus_a * s * poly2 + rootpiover2 * poly3 * (erf_b - erf_a)) / amb;
  }
  double third_integral(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2);
    ...
    const double poly2 = (-4 * a3 + b3 + b * (3 * s2 + mmx2) - (5 * s2 + mmx2) * mmx - b2 * mmx + 6 * a2 * (b - m + x) -
                          4 * a * (b2 + 2 * s2 + mmx2 - b * mmx));
    const double poly3 =
        (a4 + 4 * a3 * m + 6 * a2 * m2 + 4 * a * m3 + m4 + 6 * a2 * s2 + 12 * a * m * s2 + 6 * m2 * s2 + 3 * s4 -
         4 * apm * (apm2 + 3 * s2) * x + 6 * (apm2 + s2) * x2 - 4 * apm * x3 + x4);
    const double poly4 =
        (-b4 + m4 + 6 * m2 * s2 + 3 * s4 + 4 * a * (b3 + (3 * s2 + mmx2) * mmx) + 6 * a2 * (-b2 + s2 + mmx2) +
         4 * a3 * (b + m - x) - 4 * m * (m2 + 3 * s2) * x + 6 * (m2 + s2) * x2 - 4 * m * x3 + x4);
    return s * (gaus_a * s * poly1 + gaus_b * s * poly2 + rootpiover2 * (poly3 * erf_a - poly4 * erf_b)) / amb;
  }
  double fourth(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2);
    ...
    const double poly1 = (-4 * a3 + b3 + b * (3 * s2 + mmx2) - (5 * s2 + mmx2) * mmx - b2 * mmx + 6 * a2 * (b - m + x) -
                          4 * a * (b2 + 2 * s2 + mmx2 - b * mmx));
  ...
  double fourth_integral(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2);
    ...
    const double poly2 =
        (5 * a4 + b4 + b2 * (4 * s2 + mmx2) + (s2 + mmx2) * (8 * s2 + mmx2) - b * (7 * s2 + mmx2) * mmx -
         b3 * (mmx)-10 * a3 * (b - m + x) + 10 * a2 * (b2 + 2 * s2 + mmx2 - b * mmx) -
         5 * a * (b3 + b * (3 * s2 + mmx2) - (5 * s2 + mmx2) * mmx - b2 * mmx));
    const double poly3 =
        (apmmx * (a4 + 4 * a3 * m + 6 * a2 * m2 + 4 * a * m3 + m4 + 10 * a2 * s2 + 20 * a * m * s2 + 10 * m2 * s2 +
                  15 * s4 - 4 * apm * (apm2 + 5 * s2) * x + 2 * (3 * apm2 + 5 * s2) * x2 - 4 * apm * x3 + x4));
  ...
  double fifth(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2);
    ...
    const double poly1 =
        (5 * a4 + b4 + b2 * (4 * s2 + mmx2) + (s2 + mmx2) * (8 * s2 + mmx2) - b * (7 * s2 + mmx2) * mmx - b3 * mmx -
         10 * a3 * (b - m + x) + 10 * a2 * (b2 + 2 * s2 + mmx2 - b * mmx) -
         5 * a * (b3 + b * (3 * s2 + mmx2) - (5 * s2 + mmx2) * mmx - b2 * mmx));
  ...
  double fifth_integral(const double x, const double m, const double s, const double a, const double b) {
    const double root2 = sqrt(2);
    ...
    const double poly2 =
        (-6 * a5 + b5 + b3 * (5 * s2 + mmx2) - b2 * (9 * s2 + mmx2) * mmx - (3 * s2 + mmx2) * (11 * s2 + mmx2) * mmx -
         b4 * mmx + 15 * a4 * (b - m + x) +
         b * (m4 + 12 * m2 * s2 + 15 * s4 - 4 * m * (m2 + 6 * s2) * x + 6 * (m2 + 2 * s2) * x2 - 4 * m * x3 + x4) -
         20 * a3 * (b2 + 2 * s2 + mmx2 - b * mmx) +
         15 * a2 * (b3 + b * (3 * s2 + mmx2) - (5 * s2 + mmx2) * mmx - b2 * mmx) -
         6 * a * (b4 + b2 * (4 * s2 + mmx2) + (s2 + mmx2) * (8 * s2 + mmx2) - b * (7 * s2 + mmx2) * mmx - b3 * mmx));
    const double poly3 = (a6 + 6 * a5 * m + 15 * a4 * m2 + 20 * a3 * m3 + 15 * a2 * m4 + 6 * a * m5 + m6 +
                          15 * a4 * s2 + 60 * a3 * m * s2 + 90 * a2 * m2 * s2 + 60 * a * m3 * s2 + 15 * m4 * s2 +
                          45 * a2 * s4 + 90 * a * m * s4 + 45 * m2 * s4 + 15 * s6 -
                          6 * apm * (apm4 + 10 * apm2 * s2 + 15 * s4) * x + 15 * (apm4 + 6 * apm2 * s2 + 3 * s4) * x2 -
                          20 * apm * (apm2 + 3 * s2) * x3 + 15 * (apm2 + s2) * x4 - 6 * apm * x5 + x6);
    const double poly4 =
        (6 * a5 * b - 15 * a4 * b2 + 20 * a3 * b3 - 15 * a2 * b4 + 6 * a * b5 - b6 + 6 * a5 * m + 15 * a4 * m2 +
  ...
  double sixth(const double x, const double m, const double s) {
    ...
    const double poly1 = ((3.0 * s2 + mmx2) * (11.0 * s2 + mmx2) * (m - x));
    const double poly2 =
        (m6 + 15.0 * m4 * s2 + 45.0 * m2 * s4 + 15.0 * s6 - 6.0 * m * (m4 + 10.0 * m2 * s2 + 15.0 * s4) * x +
         15.0 * (m4 + 6.0 * m2 * s2 + 3.0 * s4) * x2 - 20.0 * m * (m2 + 3.0 * s2) * x3 + 15.0 * (m2 + s2) * x4 -
         6.0 * m * x5 + x6);
    const double poly3 = (m6 + 15.0 * s6 - 6.0 * m5 * x + 45.0 * s4 * x2 + 15.0 * s2 * x4 + x6 + 15.0 * m4 * (s2 + x2) -
                          20.0 * m3 * (3.0 * s2 * x + x3) + 15 * m2 * (3.0 * s4 + 6.0 * s2 * x2 + x4) -
                          6.0 * m * (15.0 * s4 * x + 10.0 * s2 * x3 + x5));
    return (s * gaus *
            (-s * poly1 + gaus * rootpiover2 * (m - x) * poly2 - gaus * rootpiover2 * poly3 * erf(erf_parm)));
  }

  double poly_conv(const double x, const double mean, const double sigma, const double a, const double b, const int i) {
    switch (i) {
      case 0:
  ...
  double poly_conv_integral(
      const double x, const double mean, const double sigma, const double a, const double b, const int i) {
    switch (i) {
      case 0:
...
RooGaussStepBernstein::RooGaussStepBernstein(const char* name,
  ...
  while ((coef = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(coef)) {
      cout << "RooGaussStepBernstein::ctor(" << GetName() << ") ERROR: coefficient " << coef->GetName()
           << " is not of type RooAbsReal" << endl;
      assert(0);
    }
    _coefList.add(*coef);
...
Double_t RooGaussStepBernstein::evaluate() const {
  ...
  for (Int_t i = 0; i <= degree; ++i) {
    ...
    for (Int_t k = i; k <= degree; ++k) {
      beta += (pow(-1., k - i) * TMath::Binomial(degree, k) * TMath::Binomial(k, i) * poly_vals[k]);
    }
    beta *= coef->getVal();
    result += beta;
  }
...
Int_t RooGaussStepBernstein::getAnalyticalIntegral(RooArgSet& allVars,
  ...
  if (matchArgs(allVars, analVars, _x) && _coefList.getSize() <= 5)
    return 1;
...
//_____________________________________________________________________________
Double_t RooGaussStepBernstein::analyticalIntegral(Int_t code, const char* rangeName) const {
  assert(code == 1);
  ...
  for (int i = 0; i <= degree; ++i) {
    // we stretch the upper bound by 10% to remove the turn-off of
    // covolution
    integral_vals_lo[i] = poly_conv_integral(_x.min(rangeName), _mean, _sigma, _stepVal, _x.max() * 1.10, i);
    integral_vals_hi[i] = poly_conv_integral(_x.max(rangeName), _mean, _sigma, _stepVal, _x.max() * 1.10, i);
  }
  ...
  for (Int_t i = 0; i <= degree; ++i) {
    ...
    beta = 0.0;
    for (Int_t k = i; k <= degree; ++k) {
      beta += (pow(-1., k - i) * TMath::Binomial(degree, k) * TMath::Binomial(k, i) *
