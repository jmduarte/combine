  ...
  std::unique_ptr<TH1> safeCreateHist2D(RooAbsPdf *pdf, const RooRealVar &x, const RooRealVar &y, bool conditional) {
    if (!pdf->getAttribute("safeCreateHist2D:ok") && typeid(*pdf) == typeid(RooHistPdf)) {
      ...
      for (int i = 0; i < 2; ++i) {
        ...
        if (binning->numBins() != lvarg->numBins() || binning->binLow(0) != lvarg->getMin() ||
            binning->binHigh(binning->numBins() - 1) != lvarg->getMax()) {
          std::cout << "ERROR: inconsistent binning of RooDataHist " << dataHist.GetName() << ", var  "
                    << lvarg->GetName() << std::endl;
          std::cout << "  bins: " << binning->numBins() << " (binning) vs " << lvarg->numBins() << " (var)"
                    << std::endl;
          std::cout << "  min:  " << binning->binLow(0) << " (binning) vs " << lvarg->getMin() << " (var)" << std::endl;
          std::cout << "  max:  " << binning->binHigh(binning->numBins() - 1) << " (binning) vs " << lvarg->getMax()
                    << " (var)" << std::endl;
          ok = false;
        }
      }
      if (!ok) {
        ...
        if (x.numBins() != xbinning->numBins())
          assert(0);
        if (y.numBins() != ybinning->numBins())
          assert(0);
        ...
        TH2F *h2d = static_cast<TH2F *>(hist.get());
        h2d->SetDirectory(0);
        ...
        for (unsigned int id = 0, nd = dataHist.numEntries(); id < nd; ++id) {
          ...
          if (bx == 0 || bx > x.numBins() ||
              fabs(xaxis->GetBinCenter(bx) - xval) > 1e-4 * std::max(1.0, std::abs(xval))) {
            std::cout << "ERROR: dataset entry inconsistent with bin center along X" << std::endl;
            point->Print("V");
            assert(0);
          }
          if (by == 0 || by > y.numBins() ||
              fabs(yaxis->GetBinCenter(by) - yval) > 1e-4 * std::max(1.0, std::abs(yval))) {
...
VerticalInterpHistPdf::VerticalInterpHistPdf(const char *name,
  ...
  if (inFuncList.getSize() != 2 * inCoefList.getSize() + 1) {
    coutE(InputArguments) << "combine/VerticalInterpHistPdf::VerticalInterpHistPdf(" << GetName()
  ...
  while ((func = (RooAbsArg *)funcIter->Next())) {
    ...
    if (!pdf) {
      coutE(InputArguments) << "ERROR: VerticalInterpHistPdf::VerticalInterpHistPdf(" << GetName() << ") function  "
                            << func->GetName() << " is not of type RooAbsPdf" << std::endl;
      assert(0);
    }
    RooArgSet *params = pdf->getParameters(RooArgSet(x));
    if (params->getSize() > 0) {
      coutE(InputArguments) << "ERROR: VerticalInterpHistPdf::VerticalInterpHistPdf(" << GetName() << ") pdf  "
                            << func->GetName() << " has some parameters." << std::endl;
      assert(0);
    }
    delete params;
    _funcList.add(*func);
  ...
  while ((coef = (RooAbsArg *)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal *>(coef)) {
      coutE(InputArguments) << "ERROR: VerticalInterpHistPdf::VerticalInterpHistPdf(" << GetName() << ") coefficient "
...
void VerticalInterpHistPdf::syncComponent(int i) const {
  ...
  if (_cacheSingle[i]->Integral("width")) {
    _cacheSingle[i]->Scale(1.0 / _cacheSingle[i]->Integral("width"));
  }
  if (i > 0) {
...
void VerticalInterpHistPdf::setupCaches() const {
  ...
  _cacheSingle = new TH1 *[2 * ndim + 1];
  assert(_cacheSingle);
  _cacheSingleGood = new int[2 * ndim + 1];
  assert(_cacheSingleGood);
  for (int i = 0; i < 2 * ndim + 1; ++i) {
    _cacheSingle[i] = 0;
...
FastVerticalInterpHistPdfBase::FastVerticalInterpHistPdfBase(const FastVerticalInterpHistPdfBase &other,
    ...
    : RooAbsPdf(other, name),
      ...
      _morphParams(other._morphParams) {
  // Copy constructor
  _funcIter = _funcList.createIterator();
  _coefIter = _coefList.createIterator();
  _sentry.addVars(_coefList);
...
void FastVerticalInterpHistPdf2D::syncNominal() const {
  ...
  if (_conditional)
    _cacheNominal.NormalizeXSlices();
  else
    _cacheNominal.Normalize();
...
void FastVerticalInterpHistPdf::syncComponents(int dim) const {
  ...
  const RooRealVar &x = dynamic_cast<const RooRealVar &>(_x.arg());
  std::unique_ptr<TH1> histHi(pdfHi->createHistogram("", x));
  histHi->SetDirectory(0);
  std::unique_ptr<TH1> histLo(pdfLo->createHistogram("", x));
...
void FastVerticalInterpHistPdf2D::syncComponents(int dim) const {
  ...
  const RooRealVar &y = dynamic_cast<const RooRealVar &>(_y.arg());
#ifdef PATCH_FOR_HZZ_TEMPLATES
  std::unique_ptr<TH1> histHi(::safeCreateHist2D(pdfHi, x, y, _conditional));
  histHi->SetDirectory(0);
  std::unique_ptr<TH1> histLo(::safeCreateHist2D(pdfLo, x, y, _conditional));
  histLo->SetDirectory(0);
#else
  const RooCmdArg &cond = _conditional ? RooFit::ConditionalObservables(RooArgSet(x)) : RooCmdArg::none();
  std::unique_ptr<TH1> histHi(pdfHi->createHistogram("", x, RooFit::YVar(y), cond));
  histHi->SetDirectory(0);
  std::unique_ptr<TH1> histLo(pdfLo->createHistogram("", x, RooFit::YVar(y), cond));
  histLo->SetDirectory(0);
#endif
...
void FastVerticalInterpHistPdf3D::syncComponents(int dim) const {
  ...
  const RooRealVar &z = dynamic_cast<const RooRealVar &>(_z.arg());
  const RooCmdArg &cond = _conditional ? RooFit::ConditionalObservables(RooArgSet(x)) : RooCmdArg::none();
  std::unique_ptr<TH1> histHi(pdfHi->createHistogram("", x, RooFit::YVar(y), RooFit::ZVar(z), cond));
  histHi->SetDirectory(0);
  std::unique_ptr<TH1> histLo(pdfLo->createHistogram("", x, RooFit::YVar(y), RooFit::ZVar(z), cond));
  histLo->SetDirectory(0);
...
void FastVerticalInterpHistPdfBase::syncTotal(FastTemplate &cache,
                                              ...
                                              const FastTemplate &cacheNominalLog) const {
  TRACEME()
...
FastVerticalInterpHistPdfV::FastVerticalInterpHistPdfV(const FastVerticalInterpHistPdf &hpdf,
                                                       const RooAbsData &data,
                                                       bool includeZeroWeights)
    : hpdf_(hpdf), begin_(0), end_(0) {
  // check init
  if (hpdf._cache.size() == 0)
    hpdf.setupCaches();
  if (!hpdf._sentry.good() || !hpdf._init)
    hpdf.syncTotal();
  // find bins
  std::vector<int> bins;
...
void FastVerticalInterpHistPdfV::fill(std::vector<Double_t> &out) const {
  ...
  if (begin_ != end_) {
    ...
    for (auto b : blocks_)
      std::copy(
          &hpdf_._cache.GetBinContent(b.begin), (&hpdf_._cache.GetBinContent(b.end - 1)) + 1, out.begin() + b.index);
  } else {
...
//_____________________________________________________________________________
FastVerticalInterpHistPdf2Base::FastVerticalInterpHistPdf2Base(const char *name,
                                                               const char *title,
                                                               const RooArgSet &obs,
                                                               const TList &inFuncList,
                                                               const RooArgList &inCoefList,
                                                               Double_t smoothRegion,
                                                               Int_t smoothAlgo)
    : RooAbsPdf(name, title),
  ...
  if (inFuncList.GetSize() != 2 * inCoefList.getSize() + 1) {
    coutE(InputArguments) << "combine/VerticalInterpHistPdf::VerticalInterpHistPdf(" << GetName()
                          ...
                          << "while Nfunc= " << inFuncList.GetSize() << " and Ncoef= " << inCoefList.getSize()
                          << std::endl;
    assert(0);
  }
  ...
  while ((func = (RooAbsArg *)funcIter.Next())) {
    ...
    if (!pdf && !hist) {
      coutE(InputArguments) << "ERROR: VerticalInterpHistPdf::VerticalInterpHistPdf(" << GetName() << ") function  "
                            << func->GetName() << " is not of type TH1 or RooAbsPdf" << std::endl;
      assert(0);
    }
    if (pdf) {
      RooArgSet *params = pdf->getParameters(obs);
      if (params->getSize() > 0) {
        coutE(InputArguments) << "ERROR: VerticalInterpHistPdf::VerticalInterpHistPdf(" << GetName() << ") pdf  "
                              << func->GetName() << " (" << func->ClassName() << ") has some parameters." << std::endl;
        obs.Print("");
        params->Print("");
        assert(0);
      }
...
//_____________________________________________________________________________
FastVerticalInterpHistPdf2Base::FastVerticalInterpHistPdf2Base(const FastVerticalInterpHistPdf2Base &other,
                                                               const char *name)
    : RooAbsPdf(other, name),
      _coefList("coefList", this, other._coefList),
      _smoothRegion(other._smoothRegion),
...
//_____________________________________________________________________________
FastVerticalInterpHistPdf2Base::FastVerticalInterpHistPdf2Base(const FastVerticalInterpHistPdfBase &other,
                                                               const char *name)
    : RooAbsPdf(other, name),
      _coefList("coefList", this, other._coefList),
      _smoothRegion(other._smoothRegion),
...
void FastVerticalInterpHistPdf2Base::initBase() const {
  if (_initBase)
    return;
...
FastVerticalInterpHistPdf2::FastVerticalInterpHistPdf2(const char *name,
                                                       const char *title,
                                                       const RooRealVar &x,
                                                       const TList &funcList,
...
FastVerticalInterpHistPdf2D2::FastVerticalInterpHistPdf2D2(const char *name,
                                                           const char *title,
                                                           const RooRealVar &x,
                                                           const RooRealVar &y,
                                                           bool conditional,
...
Double_t FastVerticalInterpHistPdf2::evaluate() const {
  if (!_initBase)
    initBase();
  if (_cache.size() == 0)
    _cache = _cacheNominal;  // _cache is not persisted
  if (!_sentry.good())
    syncTotal();
  return _cache.GetAt(_x);
}
...
void FastVerticalInterpHistPdf2::initComponent(int dim, TObject *thi, TObject *tlo) {
  ...
  if (histHi && histLo) {
    ...
    const RooRealVar &x = dynamic_cast<const RooRealVar &>(_x.arg());
    histHi = pdfHi->createHistogram("", x);
    histHi->SetDirectory(0);
    histLo = pdfLo->createHistogram("", x);
    histLo->SetDirectory(0);
    hi = *histHi;
    lo = *histLo;
    delete histHi;
...
void FastVerticalInterpHistPdf2D2::initComponent(int dim, TObject *thi, TObject *tlo) {
  ...
  if (histHi && histLo) {
    ...
    const RooRealVar &y = dynamic_cast<const RooRealVar &>(_y.arg());
    const RooCmdArg &cond = _conditional ? RooFit::ConditionalObservables(RooArgSet(x)) : RooCmdArg::none();
    histHi = dynamic_cast<TH2 *>(pdfHi->createHistogram("", x, RooFit::YVar(y), cond));
    histHi->SetDirectory(0);
    histLo = dynamic_cast<TH2 *>(pdfLo->createHistogram("", x, RooFit::YVar(y), cond));
    histLo->SetDirectory(0);
    hi = FastHisto2D(*histHi, _conditional);
    lo = FastHisto2D(*histLo, _conditional);
    delete histHi;
...
void FastVerticalInterpHistPdf2Base::initMorph(Morph &out,
                                               const FastTemplate &nominal,
                                               FastTemplate &lo,
                                               FastTemplate &hi) const {
...
void FastVerticalInterpHistPdf2Base::syncTotal(FastTemplate &cache,
                                               const FastTemplate &cacheNominal,
                                               const FastTemplate &cacheNominalLog) const {
  /* === how the algorithm works, in theory ===
     * let  dhi = h_hi - h_nominal
...
Int_t FastVerticalInterpHistPdf2D2::getMaxVal(const RooArgSet &vars) const {
  ...
  switch (vars.getSize()) {
    case 1:
      if (vars.contains(_x.arg()))
        return 1;
      if (vars.contains(_y.arg()))
        return 2;
      break;
    case 2:
...
Double_t FastVerticalInterpHistPdf2D2::maxVal(int code) const {
  ...
  coutE(InputArguments) << "FastVerticalInterpHistPdf2D2::maxVal(" << GetName() << ") unsupported integration code "
                        << code << "\n"
                        << std::endl;
  assert(0);
}

FastVerticalInterpHistPdf2V::FastVerticalInterpHistPdf2V(const FastVerticalInterpHistPdf2 &hpdf,
                                                         const RooAbsData &data,
                                                         bool includeZeroWeights)
    : hpdf_(hpdf), begin_(0), end_(0) {
  // check init
  if (!hpdf._initBase)
    hpdf.initBase();
  if (hpdf._cache.size() == 0)
    hpdf._cache = hpdf._cacheNominal;
  if (!hpdf._sentry.good())
    hpdf.syncTotal();
  // find bins
  std::vector<int> bins;
  RooArgSet obs(hpdf._x.arg());
