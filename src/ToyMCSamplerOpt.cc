...
ToyMCSamplerOpt::ToyMCSamplerOpt(RooStats::TestStatistic &ts,
    ...
    : ToyMCSampler(ts, ntoys),
      ...
      nuisIndex_(-1),
      weightVar_(0) {
  if (!generateNuisances)
    fPriorNuisance = 0;  // set things straight from the beginning
}
...
ToyMCSamplerOpt::~ToyMCSamplerOpt() {
  delete weightVar_;
  for (std::map<RooAbsPdf *, toymcoptutils::SimPdfGenInfo *>::iterator it = genCache_.begin(), ed = genCache_.end();
       it != ed;
       ++it) {
    delete it->second;
  }
...
toymcoptutils::SinglePdfGenInfo::SinglePdfGenInfo(
    RooAbsPdf &pdf, const RooArgSet &observables, bool preferBinned, const RooDataSet *protoData, int forceEvents)
  ...
  if (pdf.canBeExtended()) {
    if (pdf.getAttribute("forceGenBinned"))
      mode_ = Binned;
    else if (pdf.getAttribute("forceGenPoisson"))
      mode_ = Poisson;
    else if (pdf.getAttribute("forceGenUnbinned"))
      mode_ = Unbinned;
  }
  ...
  delete obs;
  if (mode_ == Binned) {
    if (runtimedef::get("TMCSO_GenBinned"))
      mode_ = BinnedNoWorkaround;
    else if (runtimedef::get("TMCSO_GenBinnedWorkaround"))
      mode_ = Binned;
    else
      mode_ = Poisson;
...
RooAbsData *toymcoptutils::SinglePdfGenInfo::generate(const RooDataSet *protoData, int forceEvents) {
  ...
  switch (mode_) {
    case Unbinned:
      if (spec_ == 0)
        spec_ = protoData
                    ? pdf_->prepareMultiGen(observables_, RooFit::Extended(), RooFit::ProtoData(*protoData, true, true))
      ...
      break;
    case Binned: {  // aka generateBinnedWorkaround
    ...
    case BinnedNoWorkaround:
      ret = protoData
                ? pdf_->generateBinned(observables_, RooFit::Extended(), RooFit::ProtoData(*protoData, true, true))
...
RooDataSet *toymcoptutils::SinglePdfGenInfo::generatePseudoAsimov(RooRealVar *&weightVar,
  ...
  if (mode_ == Unbinned) {
    ...
    if (weightVar == 0)
      weightVar = new RooRealVar("_weight_", "", 1.0);
    RooArgSet obsPlusW(observables_);
...
RooDataSet *toymcoptutils::SinglePdfGenInfo::generateWithHisto(RooRealVar *&weightVar,
  ...
  if (observables_.getSize() > 3)
    throw std::invalid_argument(std::string("ERROR in SinglePdfGenInfo::generateWithHisto for ") + pdf_->GetName() +
  ...
  if (verbose > 2) {
    ...
    if (y)
      printf(", %d y-bins ", histoSpec_->GetNbinsY());
    if (z)
      printf(", %d z-bins ", histoSpec_->GetNbinsZ());
    printf("\n");
  }
  ...
  switch (obs.getSize()) {
    case 1:
      for (int i = 1, n = histoSpec_->GetNbinsX(); i <= n; ++i) {
        ...
        data->add(observables_,
                  weightScale * (asimov ? w * histoSpec_->GetBinContent(i)
                                        : RooRandom::randomGenerator()->Poisson(w * histoSpec_->GetBinContent(i))));
      }
      break;
    case 2: {
      ...
      for (int ix = 1, nx = h2.GetNbinsX(); ix <= nx; ++ix) {
        for (int iy = 1, ny = h2.GetNbinsY(); iy <= ny; ++iy) {
          ...
          data->add(observables_,
                    weightScale * (asimov ? w * h2.GetBinContent(ix, iy)
                                          : RooRandom::randomGenerator()->Poisson(w * h2.GetBinContent(ix, iy))));
        }
      }
    } break;
    case 3: {
      ...
      for (int ix = 1, nx = h3.GetNbinsX(); ix <= nx; ++ix) {
        for (int iy = 1, ny = h3.GetNbinsY(); iy <= ny; ++iy) {
          for (int iz = 1, nz = h3.GetNbinsZ(); iz <= nz; ++iz) {
            ...
            data->add(observables_,
                      weightScale * (asimov ? w * h3.GetBinContent(ix, iy, iz)
                                            : RooRandom::randomGenerator()->Poisson(w * h3.GetBinContent(ix, iy, iz))));
          }
        }
      }
    }
...
RooDataSet *toymcoptutils::SinglePdfGenInfo::generateCountingAsimov() {
  ...
  if (prod != 0) {
    ...
    throw std::logic_error("A counting model pdf must be either a RooProdPdf or a RooPoisson");
  RooDataSet *ret = new RooDataSet(TString::Format("%sData", pdf_->GetName()), "", obs);
  ret->add(obs);
  return ret;
}

void toymcoptutils::SinglePdfGenInfo::setToExpected(RooProdPdf &prod, RooArgSet &obs) {
  ...
  for (RooAbsArg *a = (RooAbsArg *)iter->Next(); a != 0; a = (RooAbsArg *)iter->Next()) {
    ...
    if ((pois = dynamic_cast<RooPoisson *>(a)) != 0) {
      ...
      if (subprod)
        setToExpected(*subprod, obs);
      else
        throw std::logic_error("Illegal term in counting model: depends on observables, but not Poisson or Product");
    }
  }
}

void toymcoptutils::SinglePdfGenInfo::setToExpected(RooPoisson &pois, RooArgSet &obs) {
  RooRealVar *myobs = 0;
  RooAbsReal *myexp = 0;
...
toymcoptutils::SimPdfGenInfo::SimPdfGenInfo(
    RooAbsPdf &pdf, const RooArgSet &observables, bool preferBinned, const RooDataSet *protoData, int forceEvents)
    : pdf_(&pdf), cat_(0), observables_(observables), copyData_(true) {
  ...
  if (simPdf) {
    ...
    for (int ic = 0; ic < nbins; ++ic) {
      ...
      if (pdfi == 0)
        throw std::logic_error(std::string("Unmapped category state: ") + cat_->getLabel());
...
toymcoptutils::SimPdfGenInfo::~SimPdfGenInfo() {
  ...
  for (std::map<std::string, RooAbsData *>::iterator it = datasetPieces_.begin(), ed = datasetPieces_.end(); it != ed;
       ++it) {
    delete it->second;
  }
...
RooAbsData *toymcoptutils::SimPdfGenInfo::generate(RooRealVar *&weightVar,
  ...
  if (cat_ != 0) {
    ...
    if (copyData_) {
      ...
      RooAbsArg::setDirtyInhibit(true);  // don't propagate dirty flags while filling histograms
      for (std::map<std::string, RooAbsData *>::iterator it = datasetPieces_.begin(), ed = datasetPieces_.end();
           it != ed;
           ++it) {
    ...
    ret = pdfs_[0]->generate(protoData, forceEvents);
...
RooAbsData *toymcoptutils::SimPdfGenInfo::generateEpsilon(RooRealVar *&weightVar) {
  RooAbsData *ret = 0;
  TString retName = TString::Format("%sData", pdf_->GetName());
  if (cat_ != 0) {
    //bool needsWeights = false;
    for (int i = 0, n = cat_->numBins((const char *)0); i < n; ++i) {
      if (pdfs_[i] == 0)
        continue;
      if (pdfs_[i]->mode() != SinglePdfGenInfo::Unbinned)
        continue;
      cat_->setBin(i);
      RooAbsData *&data = datasetPieces_[cat_->getLabel()];
      delete data;
    ...
    ret = pdfs_[0]->generateAsimov(weightVar, 1e-9);
  //std::cout << "Asimov dataset generated from sim pdf " << pdf_->GetName() << " (sumw? " << ret->sumEntries() << ")" << std::endl;
  //utils::printRAD(ret);
  return ret;
}
...
void toymcoptutils::SimPdfGenInfo::setCacheTemplates(bool cache) {
  for (std::vector<SinglePdfGenInfo *>::const_iterator it = pdfs_.begin(), ed = pdfs_.end(); it != ed; ++it) {
    if (*it)
      (*it)->setCacheTemplates(cache);
  }
}

void ToyMCSamplerOpt::SetPdf(RooAbsPdf &pdf) {
  //std::cout << "combine/ToyMCSamplerOpt::SetPdf called" << std::endl;
  //utils::printPdf(&pdf);
...
RooDataSet *ToyMCSamplerOpt::GetSamplingDistributionsSingleWorker(RooArgSet &paramPointIn) {
  //std::cout << "combine/ToyMCSamplerOpt::GetSamplingDistributionsSingleWorker called" << std::endl;
  //utils::printPdf(fPdf);
  ...
  for (Int_t i = 0; i < fMaxToys; ++i) {
    ...
    if (toysInTails >= fToysInTails && i + 1 > fNToys)
      break;
    ...
    if (i % 500 == 0 && i > 0) {
      ...
      if (fToysInTails)
        ooccoutP((TObject *)0, Generation) << " (tails: " << toysInTails << " / " << fToysInTails << ")" << std::endl;
      else
...
RooAbsData *ToyMCSamplerOpt::GenerateToyData(RooArgSet & /*nullPOI*/, double &weight) const {
  //std::cout << "combine/ToyMCSamplerOpt::GenerateToyData called" << std::endl;
  //utils::printPdf(fPdf);
  weight = 1;
...
RooAbsData *ToyMCSamplerOpt::Generate(RooAbsPdf &pdf,
                                      ...
                                      const RooDataSet *protoData,
                                      int forceEvents) const {
  //std::cout << "combine/ToyMCSamplerOpt::Generate called" << std::endl;
  //utils::printPdf(&pdf);
  ...
  if (info == 0) {
    ...
    if (!fPriorNuisance && importanceSnapshots_.empty())
      info->setCacheTemplates(true);
  }
