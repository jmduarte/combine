...
Int_t RooDoubleCB::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* range) const {
  ...
  RooRealVar* rrv_x = dynamic_cast<RooRealVar*>(x.absArg());
  if (!rrv_x && x.absArg())
    x.absArg()->leafNodeServerList(&deps_x, 0, true);
  RooRealVar* rrv_xp = dynamic_cast<RooRealVar*>(xp.absArg());
  if (!rrv_xp && xp.absArg())
    xp.absArg()->leafNodeServerList(&deps_xp, 0, true);
  RooRealVar* rrv_mean = dynamic_cast<RooRealVar*>(mean.absArg());
  if (!rrv_mean)
    mean.absArg()->leafNodeServerList(&deps_mean, 0, true);
  RooRealVar* rrv_width = dynamic_cast<RooRealVar*>(width.absArg());
  if (!rrv_width)
    width.absArg()->leafNodeServerList(&deps_width, 0, true);
  RooRealVar* rrv_alpha1 = dynamic_cast<RooRealVar*>(alpha1.absArg());
  if (!rrv_alpha1)
    alpha1.absArg()->leafNodeServerList(&deps_alpha1, 0, true);
  RooRealVar* rrv_alpha2 = dynamic_cast<RooRealVar*>(alpha2.absArg());
  if (!rrv_alpha2)
    alpha2.absArg()->leafNodeServerList(&deps_alpha2, 0, true);
  RooRealVar* rrv_n1 = dynamic_cast<RooRealVar*>(n1.absArg());
  if (!rrv_n1)
    n1.absArg()->leafNodeServerList(&deps_n1, 0, true);
  RooRealVar* rrv_n2 = dynamic_cast<RooRealVar*>(n2.absArg());
  if (!rrv_n2)
    n2.absArg()->leafNodeServerList(&deps_n2, 0, true);

  if (rrv_x) {
    if ((deps_xp.find(*rrv_x) == nullptr || rrv_xp) || (deps_mean.find(*rrv_x) == nullptr || rrv_mean) ||
        (deps_width.find(*rrv_x) == nullptr || rrv_width) || (deps_alpha1.find(*rrv_x) == nullptr || rrv_alpha1) ||
        (deps_alpha2.find(*rrv_x) == nullptr || rrv_alpha2) || (deps_n1.find(*rrv_x) == nullptr || rrv_n1) ||
  ...
  if (rrv_xp) {
    if ((deps_x.find(*rrv_xp) == nullptr || rrv_x) || (deps_mean.find(*rrv_xp) == nullptr || rrv_mean) ||
        (deps_width.find(*rrv_xp) == nullptr || rrv_width) || (deps_alpha1.find(*rrv_xp) == nullptr || rrv_alpha1) ||
        (deps_alpha2.find(*rrv_xp) == nullptr || rrv_alpha2) || (deps_n1.find(*rrv_xp) == nullptr || rrv_n1) ||
        (deps_n2.find(*rrv_xp) == nullptr || rrv_n2)) {
      if (matchArgs(allVars, analVars, xp))
        code += 2;
    }
  }
  code = code % 3;
  ...
  switch (code) {
    case 1: {
      ...
      if (central_low < central_high)
        central = rootPiBy2 * width *
                  (TMath::Erf((central_high - mean) / xscale) - TMath::Erf((central_low - mean) / xscale));
      ...
      if (left_low < left_high) {  //is the left tail in range?
        if (fabs(n1 - 1.) > 1.e-5)
          left = A1 / (-n1 + 1.) * width *
                 (pow(B1 - (left_low - mean) / width, -n1 + 1.) - pow(B1 - (left_high - mean) / width, -n1 + 1.));
        else
      ...
      if (right_low < right_high) {  //is the right tail in range?
        if (fabs(n2 - 1.) > 1.e-5)
          right = A2 / (-n2 + 1.) * width *
                  (pow(B2 + (right_high - mean) / width, -n2 + 1.) - pow(B2 + (right_low - mean) / width, -n2 + 1.));
        else
...
double RooRelBW::evaluate() const {
  return pow(x * x, n) / ((x * x - mean * mean) * (x * x - mean * mean) +
...
Triangle::Triangle(const Triangle& other, const char* name)
    : RooAbsPdf(other, name),
      m("m", this, other.m),
      start("start", this, other.start),
      turn("turn", this, other.turn),
      stop("stop", this, other.stop) {}
