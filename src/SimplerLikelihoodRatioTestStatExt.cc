...
SimplerLikelihoodRatioTestStatOpt::SimplerLikelihoodRatioTestStatOpt(const RooArgSet &obs,
  ...
  if (simPdfNull_)
    unrollSimPdf(simPdfNull_, simPdfComponentsNull_);
  if (simPdfAlt_)
    unrollSimPdf(simPdfAlt_, simPdfComponentsAlt_);
...
Double_t SimplerLikelihoodRatioTestStatOpt::Evaluate(RooAbsData &data, RooArgSet &nullPOI) {
  // get parameters, if not there already
  if (paramsNull_.get() == 0)
    paramsNull_.reset(pdfNull_->getParameters(data));
  if (paramsAlt_.get() == 0)
    paramsAlt_.reset(pdfAlt_->getParameters(data));
...
double SimplerLikelihoodRatioTestStatOpt::evalSimNLL(RooAbsData &data,
  ...
  for (i = 0, n = components.size(); i < n; ++i) {
    if (components[i])
      sum += components[i]->extendedTerm(UInt_t(sumw[i]), obs_);
  }
  return sum;
}
