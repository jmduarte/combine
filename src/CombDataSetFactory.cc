...
CombDataSetFactory::CombDataSetFactory(const RooArgSet &vars, RooCategory &cat) : vars_(vars), cat_(&cat), weight_(0) {
  if (vars_.contains(cat))
    vars_.remove(cat);
}

CombDataSetFactory::~CombDataSetFactory() {}

void CombDataSetFactory::addSetBin(const char *label, RooDataHist *set) { map_[label] = set; }

void CombDataSetFactory::addSetAny(const char *label, RooDataHist *set) {
  if (weight_ == 0)
    weight_ = new RooRealVar("_weight_", "", 1);
  RooDataSet *data =
      new RooDataSet(TString(set->GetName()) + "_unbin", "", RooArgSet(*set->get(), *weight_), "_weight_");
...
RooDataSet *CombDataSetFactory::doneUnbinned(const char *name, const char *title) {
  ...
  if (weight_) {
       ...
       //
        ...
        //utils::printRDH(ret);
#else
    RooArgSet varsPlusCat(vars_);
    varsPlusCat.add(*cat_);
    RooArgSet varsPlusWeight(varsPlusCat);
    varsPlusWeight.add(*weight_);
