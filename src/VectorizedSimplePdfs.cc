...
VectorizedExponential::VectorizedExponential(const RooExponential &pdf,
                                             const RooAbsData &data,
                                             bool includeZeroWeights) {
  RooArgSet obs(*data.get());
  std::unique_ptr<RooArgSet> params(pdf.getParameters(data));
  if (params->getSize() != 1)
    throw std::invalid_argument("Can't resolve which is the parameter of the exponential");
...
VectorizedPower::VectorizedPower(const RooPower &pdf, const RooAbsData &data, bool includeZeroWeights) {
  RooArgSet obs(*data.get());
  if (obs.find(pdf.base()) == 0)
    throw std::invalid_argument("Dataset does not depend on the base of the power");
  x_ = dynamic_cast<const RooRealVar *>(obs.first());
...
void VectorizedPower::fill(std::vector<Double_t> &out) const {
  ...
  Double_t norm;
  if (exponent == -1)
    norm = log(xmax) - log(xmin);
  else if (exponent == 0)
    norm = xmax - xmin;
  else
    norm = (std::pow(xmax, exponent + 1) - std::pow(xmin, exponent + 1)) / (exponent + 1);
  out.resize(xvals_.size());
