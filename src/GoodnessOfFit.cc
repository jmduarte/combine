...
GoodnessOfFit::GoodnessOfFit() : LimitAlgo("GoodnessOfFit specific options") {
  options_.add_options()("algorithm",
      ...
      "setParametersForFit",
      boost::program_options::value<std::string>(&setParametersForFit_)->default_value(""),
      "Set parameters values for the saturated model fitting step")(
      "setParametersForEval",
      boost::program_options::value<std::string>(&setParametersForEval_)->default_value(""),
      "Set parameter values for the saturated model NLL eval step")
      ...
      ("fixedSignalStrength",
       ...
       "Compute the goodness of fit for a fixed signal strength. If not specified, it's left floating")(
          "plots",
          "Make plots containing information of the computation of the Anderson-Darling or Kolmogorov-Smirnov test "
          "statistic");
}
...
bool GoodnessOfFit::run(RooWorkspace *w,
  ...
  if (algo_ == "AD" || algo_ == "KS") {
    ...
    if (algo_ == "AD")
      return runKSandAD(w, mc_s, mc_b, data, limit, limitErr, hint, 0);
    if (algo_ == "KS")
      return runKSandAD(w, mc_s, mc_b, data, limit, limitErr, hint, 1);
  }
...
bool GoodnessOfFit::runSaturatedModel(RooWorkspace *w,
  ...
  if (sim) {
    ...
    RooSimultaneous *satsim = (typeid(*sim) == typeid(RooSimultaneousOpt)) ? new RooSimultaneousOpt(satname, "", *cat)
                                                                           : new RooSimultaneous(satname, "", *cat);
    for (int ic = 0, nc = nbins; ic < nc; ++ic) {
      ...
      if (datai == 0)
        throw std::runtime_error(std::string("Error: missing dataset for category label ") + cat->getLabel());
      ...
      if (constraints.getSize() > 0) {
        RooArgList terms(constraints);
        terms.add(*saturatedPdfi);
  ...
  saturated.reset();
  for (int i = 0, n = tempData_.size(); i < n; ++i)
    delete tempData_[i];
  tempData_.clear();

  if (fabs(nll_nominal) > 1e10 || fabs(nll_saturated) > 1e10)
    return false;
...
bool GoodnessOfFit::runKSandAD(RooWorkspace *w,
                               RooStats::ModelConfig *mc_s,
                               RooStats::ModelConfig *mc_b,
                               RooAbsData &data,
                               double &limit,
  ...
  */
  ...
  if (binNames_.size()) {
    ...
    if (datasets->GetSize() != int(binNames_.size())) {
      throw std::runtime_error("Numbers of categories and datasets do not match");
    }
    ...
    RooRealVar *obs = dynamic_cast<RooRealVar *>(std::unique_ptr<RooArgSet>(pdf->getObservables(data))->first());
...
Double_t GoodnessOfFit::EvaluateADDistance(RooAbsPdf &pdf, RooAbsData &data, RooRealVar &observable, bool kolmo) {
  ...
  std::stable_sort(
      data_points.begin(), data_points.end(), [](double_pair i, double_pair j) { return i.first < j.first; });
  ...
  int bin = 0;
  for (std::vector<double_pair>::const_iterator d = data_points.begin(); d != data_points.end(); d++, ++bin) {
    ...
    observableval = observable.getBinning().binHigh(observable.getBinning().binNumber(d->first));
    ...
    if (kolmo) {
      ...
      if (distance > test_stat)
        test_stat = distance;
    } else {
      bin_prob = current_cdf_val - last_cdf_val;
      distance =
          s_data * pow((empirical_df - current_cdf_val), 2) / current_cdf_val / (1. - current_cdf_val) * bin_prob;
...
RooAbsPdf *GoodnessOfFit::makeSaturatedPdf(RooAbsData &data) {
  ...
  RooAddPdf *ret =
      new RooAddPdf(TString::Format("%s_saturated", data.GetName()), "", RooArgList(*hpdf), RooArgList(*norm));
