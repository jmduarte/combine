...
void QuantileCalculator::randomizePoints() { std::random_shuffle(points_.begin(), points_.end()); }
...
int QuantileCalculator::guessPartitions(int size, double quantile) {
  ...
  std::cout << "   .....  will split the " << size << " events in " << n << " subsets, smallnum is " << smallnum
            << std::endl;
  return n;
}

template <typename T> void QuantileCalculator::import(const std::vector<T> &values, const std::vector<T> &weights) {
  int n = values.size();
...
void QuantileCalculator::quantiles(double quantile, bool doJacknife) {
  ...
  for (int j = 0; j <= m; ++j) {
    ...
    for (int i = 0; i < n; ++i) {
      if ((points_[i].set == j) == doJacknife)
        continue;  // if jacknife, cut away just one piece, otherwise cut away everthing else
