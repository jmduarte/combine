...
ClassImp(RooChebyshevPDF)
    ...
    RooChebyshevPDF::RooChebyshevPDF(const char* name, const char* title, RooAbsReal& var, RooArgList& coefList)
    : RooAbsPdf(name, title), x("x", "x", this, var), coefs("coefs", "coefs", this) {
...
RooChebyshevPDF::RooChebyshevPDF(const RooChebyshevPDF& other, const char* name)
    : RooAbsPdf(other, name), x("x", this, other.x), coefs("coefs", this, other.coefs) {}
...
Double_t RooChebyshevPDF::ChebyshevP(Int_t order, Double_t v) {
  ...
  if (order == 2)
    return 2.0 * v * v - 1.0;
  if (order == 3)
    return 4.0 * v * v * v - 3.0 * v;
  if (order == 4)
    return 8.0 * v * v * v * v - 8.0 * v * v + 1.0;
  if (order == 5)
    return 16. * v * v * v * v * v - 20. * v * v * v + 5. * v;
  if (order == 6)
    return 32. * v * v * v * v * v * v - 48. * v * v * v * v + 18. * v * v - 1.0;
  if (order == 7)
    return 64. * v * v * v * v * v * v * v - 112. * v * v * v * v * v + 56. * v * v * v - 7. * v;
  if (order > 7)
    return 2.0 * v * ChebyshevP(order - 1, v) - ChebyshevP(order - 2, v);
  assert(order > -1);
  return 0.0;
}
...
Int_t RooChebyshevPDF::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const {
  if (matchArgs(allVars, analVars, x))
    return 1;
  return 0;
}

Double_t RooChebyshevPDF::analyticalIntegral(Int_t code, const char* rangeName) const {
  assert(code);
  RooAbsReal* coef;
  Int_t tord = 1;
  Double_t dv = (x.max() - x.min()) / 2.0;
  Double_t vmax = 2.0 / (x.max() - x.min()) * (x.max(rangeName) - (x.min() + (x.max() - x.min()) / 2.0));
  Double_t vmin = 2.0 / (x.max() - x.min()) * (x.min(rangeName) - (x.min() + (x.max() - x.min()) / 2.0));
  ...
  for (tord = 1; tord <= coefs.getSize(); ++tord) {
    ...
    if (tord == 1) {
      ...
      val += coef->getVal() *
             (tord * ChebyshevP(tord + 1, vmax) / (tord * tord - 1) - vmax * ChebyshevP(tord, vmax) / (tord - 1));
      val -= coef->getVal() *
             (tord * ChebyshevP(tord + 1, vmin) / (tord * tord - 1) - vmin * ChebyshevP(tord, vmin) / (tord - 1));
    }
  }
  return dv * val;
}
...
ClassImp(RooErfPdf)
    ...
    RooErfPdf::RooErfPdf(
        const char* name, const char* title, RooAbsReal& _x, RooAbsReal& _turnOn, RooAbsReal& _width, int _onOff)
...
Double_t RooErfPdf::analyticalIntegral(Int_t code, const char* rangeName) const {
  ...
  if (code == 1) {
    return 0.5 * (x.max(rangeName) - x.min(rangeName) +
                  onOff * (indefErfIntegral(x.max(rangeName)) - indefErfIntegral(x.min(rangeName))));
  }
  return 0;
}
...
ClassImp(RooPowerExpPdf)
    ...
    RooPowerExpPdf::RooPowerExpPdf(
        const char* name, const char* title, RooAbsReal& _x, RooAbsReal& _c, RooAbsReal& _power)
    : RooAbsPdf(name, title), x("x", "x", this, _x), c("c", "c", this, _c), power("power", "power", this, _power) {}

RooPowerExpPdf::RooPowerExpPdf(const RooPowerExpPdf& other, const char* name)
    : RooAbsPdf(other, name), x("x", this, other.x), c("c", this, other.c), power("power", this, other.power) {}
...
Double_t RooPowerExpPdf::analyticalIntegral(Int_t code, const char* rangeName) const {
  ...
  if (code == 1) {
    double maxTerm =
        -1 * TMath::Power(x.max(rangeName), power + 1) * ExpIntegralE(-1 * power, -1 * c * x.max(rangeName));
    double minTerm =
        -1 * TMath::Power(x.min(rangeName), power + 1) * ExpIntegralE(-1 * power, -1 * c * x.min(rangeName));
...
ClassImp(RooTH1DPdf)
    ...
    RooTH1DPdf::RooTH1DPdf(const char* name, const char* title, RooAbsReal& _x, TH1D& _hist, bool _interpolate)
    : RooAbsPdf(name, title), x("x", "x", this, _x), hist(_hist), interpolate(_interpolate) {
  hist.SetDirectory(0);
}

RooTH1DPdf::RooTH1DPdf(const RooTH1DPdf& other, const char* name)
    : RooAbsPdf(other, name), x("x", this, other.x), hist(other.hist), interpolate(other.interpolate) {
  hist.SetDirectory(0);
}
...
Double_t RooTH1DPdf::analyticalIntegral(Int_t code, const char* rangeName) const {
  ...
  if (code == 1) {
    return hist.Integral(hist.FindBin(x.min(rangeName)), hist.FindBin(x.max(rangeName)), "width");
  }
  return 0;
}
...
ClassImp(RooPowerLaw)
    ...
    RooPowerLaw::RooPowerLaw(const char* name, const char* title, RooAbsReal& _x, RooAbsReal& _power)
    : RooAbsPdf(name, title), x("x", "dependent", this, _x), power("power", "power", this, _power) {}

RooPowerLaw::RooPowerLaw(const RooPowerLaw& other, const char* name)
    : RooAbsPdf(other, name), x("x", this, other.x), power("power", this, other.power) {}

Int_t RooPowerLaw::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& anVars, const char* rangeName) const {
  if ((power == -1.0) && (x.min(rangeName) < 0.))
    return 0;
  if (matchArgs(allVars, anVars, x))
    return 1;
  return 0;
}
  ...
  switch (code) {
    case 1:
      ...
      if (power == -1.) {
        ...
        ret = (TMath::Power(x.max(rangeName), power + 1) - TMath::Power(x.min(rangeName), power + 1)) / (power + 1);
      }
      return ret;
      break;
  }
...
Double_t RooPowerLaw::evaluate() const { return TMath::Power(x, power); }

ClassImp(RooPowerFunction)

    RooPowerFunction::RooPowerFunction(const char* name, const char* title, RooAbsReal& _x, RooAbsReal& _r)
    : RooAbsReal(name, title), x("x", "x", this, _x), r("r", "r", this, _r) {}

RooPowerFunction::RooPowerFunction(const RooPowerFunction& other, const char* name)
    : RooAbsReal(other, name), x("x", this, other.x), r("r", this, other.r) {}
...
Int_t RooPowerFunction::getAnalyticalIntegral(RooArgSet& allVars,
                                              RooArgSet& analVars,
...
ClassImp(RooExpPoly)
    ...
    RooExpPoly::RooExpPoly(const char* name, const char* title, RooAbsReal& _x, RooArgList& _coefs)
    : RooAbsPdf(name, title), x("x", "x", this, _x), coefs("coefs", "coefs", this) {
...
Double_t RooExpPoly::analyticalIntegral(Int_t code, const char* rangeName) const {
  ...
  if (coefs.getSize() > 0) {
    ...
    if (coef1 != 0.)
      val = (TMath::Exp(coef1 * x.max(rangeName)) - TMath::Exp(coef1 * x.min(rangeName))) / coef1;
    if (coefs.getSize() > 1) {
      ...
      if (coef2 != 0) {
        ...
        DoubleComplex_t c1(coef1, 0);
        DoubleComplex_t c2(coef2, 0);
        ...
        //   // 	    << "erfimin: " << erfimin << '\n';
        // }
        double zval_max(erfimax * 0.5 * rootpi * TMath::Exp(-coef1 * coef1 / 4. / coef2) / absrootc2);
        double zval_min(erfimin * 0.5 * rootpi * TMath::Exp(-coef1 * coef1 / 4. / coef2) / absrootc2);
