...
ClassImp(HZZ4L_RooSpinZeroPdf_1D)
    ...
    HZZ4L_RooSpinZeroPdf_1D::HZZ4L_RooSpinZeroPdf_1D(const char* name,
                                                     const char* title,
                                                     RooAbsReal& _kd,
                                                     //RooAbsReal& _kdint,
                                                     //RooAbsReal& _ksmd,
...

{
  ...
  while ((func = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "ERROR: :HZZ4L_RooSpinZeroPdf_1D(" << GetName() << ") funcficient " << func->GetName()
...
Int_t HZZ4L_RooSpinZeroPdf_1D::getAnalyticalIntegral(RooArgSet& allVars,
                                                     RooArgSet& analVars,
                                                     const char* /*rangeName*/) const {
  //  if (matchArgs(allVars,analVars,RooArgSet(*kd.absArg(), *kdint.absArg(), *ksmd.absArg()))) return 4 ;
  if (matchArgs(allVars, analVars, kd))
    return 4;
