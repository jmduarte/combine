...
RooNCSpline_2D_fast::RooNCSpline_2D_fast(const char* name, const char* title)
...
RooNCSpline_2D_fast::RooNCSpline_2D_fast(const char* name,
                                         const char* title,
                                         RooAbsReal& inXVar,
                                         RooAbsReal& inYVar,
  ...
  if (npointsX() > 1 && npointsY() > 1) {
    ...
    if (det == 0.) {
      coutE(InputArguments) << "RooNCSpline_2D_fast::interpolateFcn: Matrix xA could not be inverted. Something is "
    ...
    if (det == 0.) {
      coutE(InputArguments) << "RooNCSpline_2D_fast::interpolateFcn: Matrix yA could not be inverted. Something is "
    ...
    for (unsigned int j = 0; j < npointsY(); j++) {
      vector<vector<RooNCSplineCore::T>> xcoefsAtYj =
          getCoefficientsPerY(kappaX, xAinv, j, bcBeginX, bcEndX, -1);  // [ix][Ax,Bx,Cx,Dx] at each y_j
      if (j == 0) {
      ...
      if (nxbins != (int)xcoefsAtYj.size() || npoldim != (int)xcoefsAtYj.at(0).size()) {
        coutE(InputArguments) << "RooNCSpline_2D_fast::interpolateFcn: nxbins!=(int)xcoefsAtYj.size() || "
                                 "npoldim!=(int)xcoefsAtYj.at(0).size()!"
                              << endl;
        assert(0);
      }
      for (int ix = 0; ix < nxbins; ix++) {
        for (int ipow = 0; ipow < npoldim; ipow++)
          coefsAlongY.at(ipow).at(ix).push_back(xcoefsAtYj.at(ix).at(ipow));
      }
    }
    ...
    for (int ix = 0; ix < nxbins; ix++) {
      ...
      for (int ic = 0; ic < npoldim; ic++) {
        vector<vector<RooNCSplineCore::T>> yCoefs = getCoefficientsAlongDirection(
            kappaY, yAinv, coefsAlongY.at(ic).at(ix), bcBeginY, bcEndY, -1);  // [iy][A,B,C,D]
...
RooNCSpline_2D_fast::RooNCSpline_2D_fast(const RooNCSpline_2D_fast& other, const char* name)
...
RooNCSplineCore::T RooNCSpline_2D_fast::interpolateFcn(Int_t code, const char* rangeName) const {
  ...
  if (verbosity == RooNCSplineCore::kVerbose) {
    cout << "RooNCSpline_2D_fast(" << GetName() << ")::interpolateFcn begin with code: " << code << endl;
  }
  ...
  if (code == 0 || code % 2 != 0) {  // Case to just compute the value at x
    if (!testRangeValidity(theXVar, 0))
      return 0;
    xbin = getWhichBin(theXVar, 0);
    tx = getTVar(kappaX, theXVar, xbin, 0);
  } else {  // Case to integrate along x
    RooNCSplineCore::T coordmin = theXVar.min(rangeName);
    cropValueForRange(coordmin, 0);
    RooNCSplineCore::T coordmax = theXVar.max(rangeName);
  ...
  if (code == 0 || code % 3 != 0) {  // Case to just compute the value at y
    if (!testRangeValidity(theYVar, 1))
      return 0;
    ybin = getWhichBin(theYVar, 1);
    ty = getTVar(kappaY, theYVar, ybin, 1);
  } else {  // Case to integrate along y
    RooNCSplineCore::T coordmin = theYVar.min(rangeName);
    cropValueForRange(coordmin, 1);
    RooNCSplineCore::T coordmax = theYVar.max(rangeName);
  ...
  for (int ix = 0; ix < (int)coefficients.size(); ix++) {
    if ((xbin >= 0 && ix != xbin) || (xbinmin >= 0 && xbinmax >= xbinmin && !(xbinmin <= ix && ix <= xbinmax)))
      continue;
    ...
    if (verbosity == RooNCSplineCore::kVerbose) {
      if (code == 0 || code % 2 != 0)
        cout << "Evaluating tx=" << txhigh << " in bin " << ix << endl;
      else
        cout << "Evaluating tx[" << txlow << ", " << txhigh << "] in bin " << ix << endl;
    }
    ...
    for (int ic = 0; ic < (int)coefficients.at(ix).size(); ic++) {
      ...
      for (int iy = 0; iy < (int)yCoefs.size(); iy++) {
        if ((ybin >= 0 && iy != ybin) || (ybinmin >= 0 && ybinmax >= ybinmin && !(ybinmin <= iy && iy <= ybinmax)))
          continue;
        ...
        if (verbosity == RooNCSplineCore::kVerbose) {
          if (code == 0 || code % 3 != 0)
            cout << "\tEvaluating ty=" << tyhigh << " in bin " << iy << endl;
          else
            cout << "\tEvaluating ty[" << tylow << ", " << tyhigh << "] in bin " << iy << endl;
        }
...
RooNCSplineCore::T RooNCSpline_2D_fast::getTVar(const vector<RooNCSplineCore::T>& kappas,
...
vector<vector<RooNCSplineCore::T>> RooNCSpline_2D_fast::getCoefficientsPerY(
  ...
  for (unsigned int bin = 0; bin < npointsX(); bin++) {
    fcnList.push_back(FcnList.at(ybin).at(bin));
  }
...
Double_t RooNCSpline_2D_fast::evaluate() const {
  ...
  if (useFloor && value < floorEval) {
    if (verbosity >= RooNCSplineCore::kError)
      coutE(Eval) << "RooNCSpline_2D_fast ERROR::RooNCSpline_2D_fast(" << GetName() << ") evaluation returned " << value
                  << " at (x, y) = (" << theXVar << ", " << theYVar << ")" << endl;
    value = floorEval;
  }
  if (verbosity == RooNCSplineCore::kVerbose) {
    cout << "RooNCSpline_2D_fast(" << GetName() << ")::evaluate = " << value << " at (x, y) = (" << theXVar << ", "
         << theYVar << ")" << endl;
  }
  return value;
}
Int_t RooNCSpline_2D_fast::getAnalyticalIntegral(RooArgSet& allVars,
                                                 RooArgSet& analVars,
...
Double_t RooNCSpline_2D_fast::analyticalIntegral(Int_t code, const char* rangeName) const {
  ...
  if (useFloor && value < floorInt) {
    if (verbosity >= RooNCSplineCore::kError)
      coutE(Integration) << "RooNCSpline_2D_fast ERROR::RooNCSpline_2D_fast(" << GetName() << ") integration returned "
                         << value << " for code = " << code << endl;
    value = floorInt;
  }
  if (verbosity == RooNCSplineCore::kVerbose) {
    cout << "RooNCSpline_2D_fast(" << GetName() << ")::analyticalIntegral = " << value << " for code = " << code
         << endl;
  }
  return value;
}
