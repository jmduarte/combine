...
cacheutils::CachingMultiPdf::CachingMultiPdf(const RooMultiPdf &pdf, const RooArgSet &obs) : pdf_(&pdf) {
...
const std::vector<Double_t> &cacheutils::CachingMultiPdf::eval(const RooAbsData &data) {
  ...
  for (unsigned int i = 0, n = ret.size(); i < n; ++i) {
    if (chk[i] > 1e-6)
      normratio = max(normratio, abs(ret[i] / chk[i] - 1));
  }
  printf("%-70s: normalization match %+13.10f, selected pdf %s\n",
         pdf_->GetName(),
         normratio,
         pdf_->getCurrentPdf()->ClassName());
  fflush(stdout);
...
cacheutils::CachingAddPdf::CachingAddPdf(const RooAddPdf &pdf, const RooArgSet &obs) : pdf_(&pdf) {
...
cacheutils::CachingProduct::CachingProduct(const RooProduct &pdf, const RooArgSet &obs) : pdf_(&pdf) {
