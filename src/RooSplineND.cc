...
RooSplineND::RooSplineND(const char *name,
                         const char *title,
                         RooArgList &vars,
                         TTree *tree,
                         const char *fName,
    ...
    : RooAbsReal(name, title), vars_("vars", "Variables", this) {
  rescaleAxis = rescale;
  ndim_ = vars.getSize();
  ...
  std::cout << "combine/RooSplineND -- Num Dimensions == " << ndim_ << std::endl;
  std::cout << "combine/RooSplineND -- Num Samples    == " << M_ << std::endl;
  ...
  for (int i = 0; i < nentries; i++) {
    if (!(keep_points->Contains(i)))
      continue;
    tree->GetEntry(i);
    //std::cout <<"Adding point " << i << ", ";
    for (int k = 0; k < ndim_; k++) {
      float cval = b_map[k];
      if (cval < r_map[k].first)
        r_map[k].first = cval;
      if (cval > r_map[k].second)
        r_map[k].second = cval;
...
RooSplineND::RooSplineND(const char *name,
                         ...
                         std::map<int, std::pair<double, double> > &rmap,
                         double wmean,
                         double wrms)
...
TObject *RooSplineND::clone(const char *newname) const {
  return new RooSplineND(
      newname, this->GetTitle(), vars_, ndim_, M_, eps_, rescaleAxis, w_, v_map, r_map, w_mean, w_rms);
}
//_____________________________________________________________________________
...
void RooSplineND::calculateWeights(std::vector<double> &f) {
  std::cout << "combine/RooSplineND -- Solving for Weights" << std::endl;
  if (M_ == 0) {
    w_mean = 0;
    w_rms = 1;
    return;
  ...
  std::cout << "combine/RooSplineND -- ........ Done" << std::endl;
...
double RooSplineND::getDistSquare(int i, int j) {
  ...
  for (int k = 0; k < ndim_; k++) {
    ...
    double dk;
    if (rescaleAxis)
      dk = axis_pts_ * (v_i - v_j) / (r_map[k].second - r_map[k].first);
    else
      dk = (v_i - v_j);
