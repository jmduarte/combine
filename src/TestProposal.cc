...
TestProposal::TestProposal(double divisor, const RooRealVar* alwaysStepMe)
    : RooStats::ProposalFunction(), divisor_(1. / divisor), poiDivisor_(divisor_), discreteModelIndicator_(0) {
...
TestProposal::TestProposal(double divisor, const RooArgList& alwaysStepMe)
  ...
  if (alwaysStepMe.getSize() > 1)
    poiDivisor_ /= sqrt(double(alwaysStepMe.getSize()));
}
...
void TestProposal::Propose(RooArgSet& xPrime, RooArgSet& x) {
  ...
  const RooRealVar* indicatorPrime =
      discreteModelIndicator_ ? (RooRealVar*)xPrime.find(discreteModelIndicator_->GetName()) : 0;
  for (int i = 0; (var = (RooRealVar*)it.Next()) != NULL; ++i) {
    if (i == j) {
      if (alwaysStepMe_.contains(*var))
        break;  // don't step twice
...
Bool_t TestProposal::IsSymmetric(RooArgSet& x1, RooArgSet& x2) { return true; }

// Return the probability of proposing the point x1 given the starting
// point x2
Double_t TestProposal::GetProposalDensity(RooArgSet& x1, RooArgSet& x2) {
