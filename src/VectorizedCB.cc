...
VectorizedCBShape::VectorizedCBShape(const RooCBShape& gaus, const RooAbsData& data, bool includeZeroWeights) {
  ...
  if (obs.getSize() != 1)
    throw std::invalid_argument("Multi-dimensional dataset?");
  ...
  if (obs.contains(w.mvar())) {
    ...
    throw std::invalid_argument(
        "CBShape observable is not m: if this is intended, set --X-rtd ADDNLL_CBNLL=0 to disable CBShape vectorization "
        "in NLL.");
  }
  m0_ = &w.m0var();
  n_ = &w.nvar();
    ...
    if (left_low < left_high) {  //is the left tail in range?
      ...
      if (fabs(n1 - 1.0) > 1.e-5) {
        ...
        if (xmax > (mean - alpha1 * width))
          ...
          left1 = std::pow(leftpow * (n1invalpha1 - alpha1 - thigh), 1. - n1);

        if (tmin < -1000.)
          left2 = 0.;
        else
          left2 = std::pow(leftpow * (n1invalpha1 - alpha1 - tmin), 1. - n1);
        ...
        left =
            A1 * width * (std::log(B1 - (left_low - mean) * invwidth) - std::log(B1 - (left_high - mean) * invwidth));
      }
    }
  }
...
void VectorizedCBShape::cbGauss(
    double* __restrict__ t, unsigned int n, double norm, double* __restrict__ out, double* __restrict__ work2) const {
  for (unsigned int i = 0; i < n; ++i) {
    work2[i] = -0.5 * t[i] * t[i];
  }
  if (hasFast()) {
