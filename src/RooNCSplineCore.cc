...
RooNCSplineCore::RooNCSplineCore(const char* name, const char* title)
...
RooNCSplineCore::RooNCSplineCore(const char* name,
                                 const char* title,
                                 RooAbsReal& inXVar,
...
RooNCSplineCore::RooNCSplineCore(const RooNCSplineCore& other, const char* name)
    : RooAbsReal(other, name),
      verbosity(other.verbosity),
      useFloor(other.useFloor),
      floorEval(other.floorEval),
      floorInt(other.floorInt),
      rangeXmin(other.rangeXmin),
      rangeXmax(other.rangeXmax),
...
void RooNCSplineCore::getBArray(const std::vector<RooNCSplineCore::T>& kappas,
                                const vector<RooNCSplineCore::T>& fcnList,
                                std::vector<RooNCSplineCore::T>& BArray,
                                BoundaryCondition const& bcBegin,
                                BoundaryCondition const& bcEnd) const {
  BArray.clear();
  int npoints = kappas.size();
  if (npoints != (int)fcnList.size()) {
    coutE(InputArguments) << "combine/RooNCSplineCore::getBArray: Dim(kappas)=" << npoints
                          << " != Dim(fcnList)=" << fcnList.size() << endl;
    assert(0);
  }
  if (bcBegin == bcQuadraticWithNullSlope || bcEnd == bcQuadraticWithNullSlope || bcBegin == bcQuadratic ||
      bcEnd == bcQuadratic) {
    int nthr = 1 + (bcBegin == bcQuadraticWithNullSlope || bcBegin == bcQuadratic ? 1 : 0) +
               (bcEnd == bcQuadraticWithNullSlope || bcEnd == bcQuadratic ? 1 : 0);
    if (npoints <= nthr) {
  ...
  if (npoints > 1) {
    ...
    switch (bcBegin) {
      case bcApproximatedSecondDerivative:
        bcval =
            (npoints < 3
                 ? 0
                 : ((fcnList.at(2) - fcnList.at(1)) * kappas.at(1) - (fcnList.at(1) - fcnList.at(0)) * kappas.at(0)) /
                       (0.5 / kappas.at(0) + 0.5 / kappas.at(1)));
      ...
      case bcQuadraticWithNullSlope:
        ...
        break;
      default:
        cerr << "combine/RooNCSplineCore::getBArray: bcBegin " << bcBegin << " is not implemented!" << endl;
        assert(0);
    }
    // Intermediate point constraint is always D0, D1 and D2 continuous
    for (int j = 1; j < npoints - 1; j++) {
      if (j == 1 && bcBegin == bcQuadraticWithNullSlope)
        continue;
      if (j == npoints - 2 && bcEnd == bcQuadraticWithNullSlope)
        continue;
    ...
    switch (bcEnd) {
      case bcApproximatedSecondDerivative:
        ecval = (npoints < 3 ? 0
                             : ((fcnList.at(npoints - 1) - fcnList.at(npoints - 2)) * kappas.at(npoints - 2) -
                                (fcnList.at(npoints - 2) - fcnList.at(npoints - 3)) * kappas.at(npoints - 3)) /
                                   (0.5 / kappas.at(npoints - 3) + 0.5 / kappas.at(npoints - 2)));
      case bcNaturalSpline:
        BArray.push_back(3. * (fcnList.at(npoints - 1) - fcnList.at(npoints - 2)) +
                         ecval / 2. / pow(kappas.at(npoints - 1), 2));
        break;
      case bcApproximatedSlope:
      ...
      case bcQuadraticWithNullSlope:
        ...
        break;
      default:
        cerr << "combine/RooNCSplineCore::getAArray: bcEnd " << bcEnd << " is not implemented!" << endl;
        assert(0);
    }
  } else if (npoints == 1)
    BArray.push_back(0);
}
void RooNCSplineCore::getAArray(const vector<RooNCSplineCore::T>& kappas,
                                vector<vector<RooNCSplineCore::T>>& AArray,
                                BoundaryCondition const& bcBegin,
                                BoundaryCondition const& bcEnd) const {
  AArray.clear();
    ...
    if (npoints == 1)
      ...
      switch (bcBegin) {
        ...
        case bcQuadratic:
          ...
          break;
        default:
          cerr << "combine/RooNCSplineCore::getAArray: bcBegin " << bcBegin << " is not implemented!" << endl;
          assert(0);
      }
    } else if (i == npoints - 1) {
      switch (bcEnd) {
        ...
        case bcApproximatedSecondDerivative:
          Ai[npoints - 2] = 1;
          Ai[npoints - 1] = 2. * kappas.at(npoints - 1) / kappas.at(npoints - 2);
          break;
        case bcClamped:
        case bcApproximatedSlope:
        case bcQuadraticWithNullSlope:
        ...
        case bcQuadratic:
          Ai[npoints - 2] = 1;
          Ai[npoints - 1] = kappas.at(npoints - 1) / kappas.at(npoints - 2);
          break;
        default:
          cerr << "combine/RooNCSplineCore::getAArray: bcEnd " << bcEnd << " is not implemented!" << endl;
          assert(0);
      ...
      if ((i == 1 && bcBegin == bcQuadraticWithNullSlope) || (i == npoints - 2 && bcEnd == bcQuadraticWithNullSlope))
        Ai[i] = 1;
      else {
...
vector<RooNCSplineCore::T> RooNCSplineCore::getCoefficients(const TVector_t& S,
                                                            const vector<RooNCSplineCore::T>& kappas,
                                                            const vector<RooNCSplineCore::T>& fcnList,
                                                            const Int_t& bin) const {
...
vector<vector<RooNCSplineCore::T>> RooNCSplineCore::getCoefficientsAlongDirection(
    const std::vector<RooNCSplineCore::T>& kappas,
    const TMatrix_t& Ainv,
    const vector<RooNCSplineCore::T>& fcnList,
    BoundaryCondition const& bcBegin,
    BoundaryCondition const& bcEnd,
    const Int_t pickBin) const {
...
RooNCSplineCore::T RooNCSplineCore::evalSplineSegment(const std::vector<RooNCSplineCore::T>& coefs,
                                                      const RooNCSplineCore::T& kappa,
                                                      const RooNCSplineCore::T& tup,
                                                      const RooNCSplineCore::T& tdn,
                                                      Bool_t doIntegrate) const {
  DefaultAccumulator<RooNCSplineCore::T> res;
  for (unsigned int ic = 0; ic < coefs.size(); ic++) {
    if (doIntegrate)
      res += coefs.at(ic) * (pow(tup, (int)(ic + 1)) - pow(tdn, (int)(ic + 1))) / ((RooNCSplineCore::T)(ic + 1));
    else
      res += coefs.at(ic) * pow(tup, (int)ic);
  }
  if (doIntegrate)
    res /= kappa;
  return res;
