...
VerticalInterpPdf::VerticalInterpPdf(const char* name,
  ...
  if (inFuncList.getSize() != 2 * inCoefList.getSize() + 1) {
    coutE(InputArguments) << "combine/VerticalInterpPdf::VerticalInterpPdf(" << GetName()
  ...
  while ((func = (RooAbsArg*)funcIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "ERROR: VerticalInterpPdf::VerticalInterpPdf(" << GetName() << ") function  "
                            << func->GetName() << " is not of type RooAbsReal" << std::endl;
      assert(0);
    }
    _funcList.add(*func);
  }
  ...
  while ((coef = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(coef)) {
      coutE(InputArguments) << "ERROR: VerticalInterpPdf::VerticalInterpPdf(" << GetName() << ") coefficient "
...
Bool_t VerticalInterpPdf::checkObservables(const RooArgSet* nset) const {
  ...
  // from observables
  if (_quadraticAlgo == -1)
    return false;  // multiplicative morphing. we don't care.
  ...
  while ((func = (RooAbsReal*)_funcIter->Next())) {
    if (func->observableOverlaps(nset, *coef)) {
      coutE(InputArguments) << "combine/VerticalInterpPdf::checkObservables(" << GetName() << "): ERROR: coefficient "
