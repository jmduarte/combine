#include "combine/RooNCSplineFactory_2D.h"

using namespace std;

RooNCSplineFactory_2D::RooNCSplineFactory_2D(RooAbsReal& XVar_,
                                             RooAbsReal& YVar_,
                                             TString appendName_,
    ...
    : appendName(appendName_),
      ...
      fcn(0),
      PDF(0) {}
RooNCSplineFactory_2D::~RooNCSplineFactory_2D() { destroyPDF(); }

void RooNCSplineFactory_2D::addUnique(std::vector<RooNCSplineCore::T>& list, RooNCSplineCore::T val) {
  for (unsigned int ip = 0; ip < list.size(); ip++) {
    if (list.at(ip) == val)
      return;
  }
  list.push_back(val);
}
const std::vector<splineTriplet_t> RooNCSplineFactory_2D::getPoints(const std::vector<RooNCSplineCore::T>& XList,
  ...
  if (nX * nY != n) {
    cerr << "combine/RooNCSplineFactory_2D::getPoints: nX=" << nX << " x nY=" << nY << " != nFcn=" << n << endl;
    assert(0);
  }

  std::vector<splineTriplet_t> pList;
  pList.reserve(n);
...
void RooNCSplineFactory_2D::destroyPDF() {
  delete PDF;
  PDF = 0;
  delete fcn;
  fcn = 0;
...
void RooNCSplineFactory_2D::initPDF(const std::vector<splineTriplet_t>& pList) {
  ...
  for (unsigned int iy = 0; iy < YList.size(); iy++) {
    vector<RooNCSplineCore::T> dum;
    dum.reserve(XList.size());
  ...
  fcn = new RooNCSpline_2D_fast(
      name.Data(), title.Data(), *XVar, *YVar, XList, YList, FcnList, bcBeginX, bcEndX, bcBeginY, bcEndY);
...
void RooNCSplineFactory_2D::setPoints(TTree* tree) {
  ...
  for (int ip = 0; ip < n; ip++) {
    tree->GetEntry(ip);
    pList.push_back(splineTriplet_t(x, y, fcn));
  }
  setPoints(pList);
}

void RooNCSplineFactory_2D::setEndConditions(RooNCSplineCore::BoundaryCondition const bcBegin,
