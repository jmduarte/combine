...
ClassImp(HZZ4L_RooCTauPdf_1D)
    ...
    HZZ4L_RooCTauPdf_1D::HZZ4L_RooCTauPdf_1D(const char* name,
                                             const char* title,
                                             RooAbsReal& _kd,
                                             RooAbsReal& _ctau,
  ...
  while ((func = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "ERROR: :HZZ4L_RooCTauPdf_1D(" << GetName() << ") funcficient " << func->GetName()
  ...
  for (int mp = 0; mp < (nbins_ctau > 101 ? 101 : nbins_ctau); mp++)
    Integral_T[mp] = dynamic_cast<const RooHistFunc*>(_coefList.at(mp))->analyticalIntegral(1000);
  for (int mp = nbins_ctau; mp < 101; mp++)
...
HZZ4L_RooCTauPdf_1D::HZZ4L_RooCTauPdf_1D(const HZZ4L_RooCTauPdf_1D& other, const char* name)
...
Int_t HZZ4L_RooCTauPdf_1D::getAnalyticalIntegral(RooArgSet& allVars,
                                                 RooArgSet& analVars,
