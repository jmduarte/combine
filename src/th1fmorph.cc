...
TH1_t *th1fmorph_(const char *chname,
  ...
  Int_t idx = 0;
  for (std::set<Double_t>::const_iterator bedge = bedgesn_tmp.begin(); bedge != bedgesn_tmp.end(); ++bedge) {
    bedgesn[idx] = (*bedge);
    ++idx;
  }
  ...
  if (wt1 < 0 || wt1 > 1. || wt2 < 0. || wt2 > 1. || fabs(1 - (wt1 + wt2)) > 1.0e-4) {
    cout << "Warning! th1fmorph: This is an extrapolation!! Weights are " << wt1 << " and " << wt2
         << " (sum=" << wt1 + wt2 << ")" << endl;
  }
  if (idebug >= 1)
    cout << "th1morph - Weights: " << wt1 << " " << wt2 << endl;

  if (idebug >= 1)
    cout << "New hist: " << nbn << " " << xminn << " " << xmaxn << endl;

  // Treatment for empty histograms: Return an empty histogram
  // with interpolated bins.

  if (hist1->GetSum() <= 0 || hist2->GetSum() <= 0) {
    cout << "Warning! th1morph detects an empty input histogram. Empty interpolated histogram returned: " << endl
         << "         " << chname << " - " << chtitle << endl;
  ...
  if (idebug >= 1)
    cout << "Input histogram content sums: " << hist1->GetSum() << " " << hist2->GetSum() << endl;
  // *
  ...
  if (idebug >= 1)
    cout << "Total histogram 1: " << total << endl;
  ...
  if (idebug >= 1)
    cout << "Total histogram 22: " << total << endl;
  for (Int_t i = 1; i < nb2 + 1; i++) {
  ...
  sigdisn[nx3] = 0;
  if (idebug >= 1) {
    cout << "First interpolated point: " << xdisn[nx3] << " " << sigdisn[nx3] << endl;
    cout << "                          " << x1 << " <= " << x << " <= " << x2 << endl;
  }
  ...
  if (idebug >= 1) {
    cout << "----BEFORE while with ix1=" << ix1 << ", ix1l=" << ix1l << ", ix2=" << ix2 << ", ix2l=" << ix2l << endl;
  }
  ...
  while ((ix1 < ix1l) | (ix2 < ix2l)) {
    if (idebug >= 1)
      cout << "----Top of while with ix1=" << ix1 << ", ix1l=" << ix1l << ", ix2=" << ix2 << ", ix2l=" << ix2l << endl;
    ...
    if (i12type == 1) {
      if (idebug >= 3) {
        cout << "Pair for i12type=1: " << sigdis2[ix2] << " " << sigdis1[ix1] << " " << sigdis2[ix2 + 1] << endl;
      }
      x1 = axis1->GetBinLowEdge(ix1 + 1);
      y = sigdis1[ix1];
      ...
      if (idebug >= 3) {
        cout << "Pair for i12type=2: " << sigdis1[ix1] << " " << sigdis2[ix2] << " " << sigdis1[ix1 + 1] << endl;
      }
      x2 = axis2->GetBinLowEdge(ix2 + 1);
      y = sigdis2[ix2];
    ...
    if (y > yprev) {
      nx3 = nx3 + 1;
      if (idebug >= 1) {
        cout << " ---> y > yprev: i12type=" << i12type << ", nx3=" << nx3 << ", x= " << x << ", y=" << y
             << ", yprev=" << yprev << endl;
      }
      yprev = y;
      xdisn[nx3] = x;
      sigdisn[nx3] = y;
      if (idebug >= 1) {
        cout << "    ix1=" << ix1 << ", ix2= " << ix2 << ", i12type= " << i12type << ", sigdis1[ix1]=" << sigdis1[ix1]
             << endl;
        cout << "        "
             << ", nx3=" << nx3 << ", x=" << x << ", y= " << sigdisn[nx3] << endl;
      }
    }
  }
  if (idebug >= 3)
  ...
  if (idebug >= 1)
    cout << "------> Any final bins to set? " << x << " " << xdisn[nx3] << endl;
  while (x >= xdisn[nx3]) {
    sigdisf[ix] = sigdisn[nx3];
    if (idebug >= 2)
      cout << "   Setting final bins" << ix << " " << x << " " << sigdisf[ix] << endl;
    ix = ix - 1;
    x = bedgesn[ix];
  }
  Int_t ixl = ix + 1;
  if (idebug >= 1)
    cout << " Now ixl=" << ixl << " ix=" << ix << endl;
  ...
  x = bedgesn[ix + 1];
  if (idebug >= 1)
    cout << "Start setting initial bins at x=" << x << endl;
  while (x <= xdisn[0]) {
    sigdisf[ix] = sigdisn[0];
    if (idebug >= 1)
      cout << "   Setting initial bins " << ix << " " << x << " " << xdisn[1] << " " << sigdisf[ix] << endl;
    ix = ix + 1;
    x = bedgesn[ix + 1];
  }
  Int_t ixf = ix;
  ...
  for (ix = ixf; ix < ixl; ix++) {
    ...
    if (x < xdisn[0]) {
      ...
      if (xdisn[ix3 + 1] - x > 1.1 * dx2) {  // Empty bin treatment
        ...
        y = 0;
        cout << "Warning - th1fmorph: This probably shoudn't happen! " << endl;
        cout << "Warning - th1fmorph: Zero slope solving x(y)" << endl;
      }
    }
    sigdisf[ix] = y;
    if (idebug >= 3) {
      cout << ix << ", ix3=" << ix3 << ", xdisn=" << xdisn[ix3] << ", x=" << x << ", next xdisn=" << xdisn[ix3 + 1]
           << endl;
      cout << "   cdf n=" << sigdisn[ix3] << ", y=" << y << ", next point=" << sigdisn[ix3 + 1] << endl;
    }
  }
