...
#include "combine/CascadeMinimizer.h"  // must be early
#include <TFile.h>
#include <TF1.h>
#include <TKey.h>
#include <TLine.h>
...
#include "combine/RooFitGlobalKillSentry.h"
#include "combine/SimplerLikelihoodRatioTestStat.h"
#include "combine/ProfiledLikelihoodRatioTestStat.h"
#include "combine/SimplerLikelihoodRatioTestStatExt.h"
#include "combine/ProfiledLikelihoodRatioTestStatExt.h"
#include "combine/BestFitSigmaTestStat.h"
...
HybridNew::HybridNew() : LimitAlgo("HybridNew specific options") {
  options_.add_options()(
      ...
      "Fit the nuisances first, before generating the toy data. Set this option to false to acheive the same results "
      "as with --bypassFrequentistFit. When not generating toys, eg as in when running with --readHybridresult, this "
      "has no effect")("searchAlgo",
      ...
      "nCPU",
      boost::program_options::value<unsigned int>(&nCpu_)->default_value(nCpu_),
      "Use N CPUs with PROOF Lite (experimental!)")("saveHybridResult", "Save result in the output file")(
      ...
      ("plot",
       ...
       "Save a plot of the result (test statistics distributions or limit scan)")(
          ...
          "Throw less toys far from interesting contours , --toysH scaled by scale when prob is far from any of CL_i = "
          "{importanceContours} ")("importantContours",
                                   boost::program_options::value<std::string>(&scaleAndConfidenceSelection_)
                                       ->default_value(scaleAndConfidenceSelection_),
                                   "Throw less toys far from interesting contours , format : CL_1,CL_2,..CL_N (--toysH "
                                   "scaled down when prob is far from any of CL_i) ")(
          "maxProbability",
          ...
          "(Q_Profile, includes boundaries) --rule CLsplusb")
...
void HybridNew::applyOptions(const boost::program_options::variables_map &vm) {
  ...
  if (vm.count("expectedFromGrid") && !vm["expectedFromGrid"].defaulted()) {
    ...
    g_quantileExpected = quantileForExpectedFromGrid_;
  }
...
void HybridNew::validateOptions() {
  ...
  if (testStat_ == "SimpleLikelihoodRatio" || testStat_ == "SLR") {
    testStat_ = "LEP";
  }
  if (testStat_ == "RatioOfProfiledLikelihoods" || testStat_ == "ROPL") {
    testStat_ = "TEV";
  }
  if (testStat_ == "ProfileLikelihood" || testStat_ == "PL") {
    testStat_ = "Profile";
  }
  if (testStat_ == "ModifiedProfileLikelihood" || testStat_ == "MPL") {
    testStat_ = "LHC";
  }
  if (testStat_ == "SignFlipProfileLikelihood" || testStat_ == "SFPL") {
    testStat_ = "LHCFC";
  }
  if (testStat_ == "Atlas") {
    testStat_ = "LHC";
    std::cout << "Note: the Atlas test statistics is now known as LHC test statistics.\n" << std::endl;
  }
  if (testStat_ != "LEP" && testStat_ != "TEV" && testStat_ != "LHC" && testStat_ != "LHCFC" &&
      testStat_ != "Profile" && testStat_ != "MLZ") {
    throw std::invalid_argument(
        "combine/HybridNew: Test statistics should be one of 'LEP' or 'TEV' or 'LHC' (previously known as 'Atlas') or "
        "'Profile'");
  }
  if (testStat_ == "LEP")
    std::cout << ">>> using the Simple Likelihood Ratio test statistics (Q_LEP)" << std::endl;
  if (testStat_ == "TEV")
    std::cout << ">>> using the Ratio of Profiled Likelihoods test statistics (Q_TEV)" << std::endl;
  if (testStat_ == "LHC")
    std::cout << ">>> using the Profile Likelihood test statistics modified for upper limits (Q_LHC)" << std::endl;
  if (testStat_ == "LHCFC")
    std::cout
        << ">>> using the Profile Likelihood test statistics modified for upper limits and Feldman-Cousins (Q_LHCFC)"
        << std::endl;
  if (testStat_ == "Profile")
    std::cout << ">>> using the Profile Likelihood test statistics not modified for upper limits (Q_Profile)"
              << std::endl;
  if (testStat_ == "MLZ")
    std::cout << ">>> using the Maximum likelihood estimator of the signal strength as test statistics" << std::endl;
...
bool HybridNew::run(RooWorkspace *w,
  ...
  if (rValues_.getSize() == 0)
    setupPOI(mc_s);
  switch (workingMode_) {
    case MakeLimit:
      return runLimit(w, mc_s, mc_b, data, limit, limitErr, hint);
    case MakeSignificance:
      return runSignificance(w, mc_s, mc_b, data, limit, limitErr, hint);
    case MakePValues:
      return runSinglePoint(w, mc_s, mc_b, data, limit, limitErr, hint);
...
bool HybridNew::runSignificance(RooWorkspace *w,
  ...
  if (readHybridResults_) {
    ...
    for (unsigned int i = 1; i < iterations_; ++i) {
      ...
      if (g_verbose)
        std::cout << "\t1 - CLb = " << hcResult->CLb() << " +/- " << hcResult->CLbError() << std::endl;
    }
  }
  if (hcResult.get() == 0) {
  ...
  if (saveHybridResult_) {
    ...
    g_writeToysHere->WriteTObject(new HypoTestResult(*hcResult), name);
    if (g_verbose)
      std::cout << "Hybrid result saved as " << name << " in " << g_writeToysHere->GetFile()->GetName() << " : "
                << g_writeToysHere->GetPath() << std::endl;
  }
  if (g_verbose > 1) {
  ...
  std::cout << "\n -- Hybrid New -- \n";
  std::cout << "combine/Significance: " << sig << "  " << sigLo << "/+" << sigHi << "\n";
...
bool HybridNew::runLimit(RooWorkspace *w,
                         ...
                         double &limitErr,
                         const double *hint) {
  if (mc_s->GetParametersOfInterest()->getSize() != 1)
    throw std::logic_error("Cannot run limits in more than one dimension, for the moment.");
  ...
  if (readHybridResults_) {
    if (g_verbose > 0)
      std::cout << "Search for upper limit using pre-computed grid of p-values" << std::endl;

    if (!gridFile_.empty()) {
      if (grid_.empty()) {
        std::unique_ptr<TFile> gridFile(TFile::Open(gridFile_.c_str()));
        if (gridFile.get() == 0)
          throw std::runtime_error(("Can't open grid file " + gridFile_).c_str());
        TDirectory *toyDir = gridFile->GetDirectory("toys");
        if (!toyDir)
          throw std::logic_error("Cannot use readHypoTestResult: empty toy dir in input file empty");
        readGrid(toyDir, rMinSet_ ? rMin : -99e99, rMaxSet_ ? rMax : +99e99);
      }
      if (grid_.size() <= 1)
        throw std::logic_error("The grid must contain at least 2 points.");
      if (noUpdateGrid_) {
        if (testStat_ == "LHCFC" && rule_ == "FC" && (saveGrid_ || g_lowerLimit)) {
    ...
    for (int i = 0, n = limitPlot_->GetN(); i < n; ++i) {
      ...
      if (g_verbose > 0)
        printf("  r %.2f: %s = %6.4f +/- %6.4f\n", x, CLs_ ? "CLs" : "CLsplusb", y, ey);
      if (saveGrid_) {
        limit = x;
        limitErr = ey;
        Combine::commitPoint(false, y);
      }
      if (y - 3 * max(ey, 0.01) >= clsTarget) {
        rMin = x;
        clsMin = CLs_t(y, ey);
      }
      if (fabs(y - clsTarget) < minDist) {
        nearest = x;
        minDist = fabs(y - clsTarget);
      }
      rMax = x;
      clsMax = CLs_t(y, ey);
      if (y + 3 * max(ey, 0.01) <= clsTarget && !fullGrid_)
        break;
    }
    limit = nearest;
    if (g_verbose > 0)
      std::cout << " after scan x ~ " << limit << ", bounds [ " << rMin << ", " << rMax << "]" << std::endl;
    ...
    if (limitErr < std::max(rAbsAccuracy_, rRelAccuracy_ * limit) && (!doFC_)) {  // need to look for intervals for FC
      if (g_verbose > 1)
        std::cout << "  reached accuracy " << limitErr << " below " << std::max(rAbsAccuracy_, rRelAccuracy_ * limit)
                  << std::endl;
      done = true;
    }
  } else {
    ...
    for (int tries = 0; tries < 6; ++tries) {
      ...
      if (g_lowerLimit)
        break;  // we can't search for lower limits this way
      if (clsMax.first == 0 || clsMax.first + 3 * fabs(clsMax.second) < clsTarget)
        break;
      rMax += rMax;
      if (tries == 5) {
        std::cerr << "Cannot set higher limit: at " << r->GetName() << " = " << rMax << " still get "
                  << (CLs_ ? "CLs" : "CLsplusb") << " = " << clsMax.first << std::endl;
        return false;
      }
    }
    if (g_verbose > 0)
    ...
    if (!g_lowerLimit && clsMin.first != 1 && clsMin.first - 3 * fabs(clsMin.second) < clsTarget) {
      if (CLs_) {
        rMin = 0;
        ...
        for (int tries = 0; tries < 6; ++tries) {
          ...
          rMin += rMin;
          if (tries == 5) {
            std::cerr << "Cannot set lower limit: at " << r->GetName() << " = " << rMin << " still get "
                      << (CLs_ ? "CLs" : "CLsplusb") << " = " << clsMin.first << std::endl;
            return false;
          }
        }
      }
    ...
    do {
      ...
      if (algo_ == "logSecant" && clsMax.first != 0 && clsMin.first != 0) {
        ...
        if (clsMax.second != 0 && clsMin.second != 0) {
          limitErr = hypot((logTarget - logMax) * (clsMin.second / clsMin.first),
                           (logTarget - logMin) * (clsMax.second / clsMax.first));
          limitErr *= (rMax - rMin) / ((logMax - logMin) * (logMax - logMin));
        }
        // disallow "too precise" interpolations
        if (limitErr < interpAccuracy_ * (rMax - rMin))
          limitErr = interpAccuracy_ * (rMax - rMin);
      }
      r->setError(limitErr);
      ...
      if (fabs(clsMid.first - clsTarget) >= 2 * clsMid.second) {
        ...
        if (g_verbose > 0)
          std::cout << "Trying to move the interval edges closer" << std::endl;
  ...
  if (!done) {  // didn't reach accuracy with scan, now do fit
    ...
    if (doFC_) {
      ...
      if (points.size() < 2) {
        ...
        limit = points[ib].first;
        limitErr = points[ib].second;
        Combine::commitPoint(false, clsTarget);
      } else {
        std::cout << "combine/HybridNew -- found  " << 100 * g_confidenceLevel << "%% confidence regions " << std::endl;
        if (g_verbose)
          Logger::instance().log(
              std::string(Form("HybridNew.cc: %d found %g %% confidence regions", __LINE__, 100 * g_confidenceLevel)),
              Logger::kLogLevelInfo,
              __func__);
        for (unsigned int ib = 1; ib < points.size() - 2; ib += 2) {
          std::cout << "  " << points[ib].first << " (+/-" << points[ib].second << ")"
          ...
          limitErr = points[ib].second;
          Combine::commitPoint(false, clsTarget);
          limit = points[ib + 1].first;
          limitErr = points[ib + 1].second;
          Combine::commitPoint(false, clsTarget);
        }
      }
    } else {
      double rMinBound, rMaxBound;
      expoFit.GetRange(rMinBound, rMaxBound);
      if (g_verbose) {
        std::cout << "\n -- HybridNew, before fit -- \n";
        std::cout << "Limit: " << r->GetName() << " < " << limit << " +/- " << limitErr << " [" << rMinBound << ", "
                  << rMaxBound << "]\n";
      }
      ...
      for (int j = 0; j < limitPlot_->GetN(); ++j) {
        if (limitPlot_->GetX()[j] >= rMinBound && limitPlot_->GetX()[j] <= rMaxBound)
          npoints++;
      }
      for (int i = 0, imax = (readHybridResults_ ? 0 : 8); i <= imax; ++i, ++npoints) {
        limitPlot_->Sort();
        limitPlot_->Fit(&expoFit,
                        (g_verbose <= 1 ? "QNR EX0" : "NR EX0"));  // For FC, might be more appropriate to fit pol2?
        if (g_verbose) {
          std::cout << "Fit to " << npoints << " points: " << expoFit.GetParameter(2) << " +/- "
                    << expoFit.GetParError(2) << std::endl;
        }
        if ((rMinBound < expoFit.GetParameter(2)) && (expoFit.GetParameter(2) < rMaxBound) &&
            (expoFit.GetParError(2) < 0.5 * (rMaxBound - rMinBound))) {
  ...
  if (!doFC_)
    points.push_back(std::pair<double, double>(limit, limitErr));  // add single Limit if not FC intervals
  if (!plot_.empty() && limitPlot_.get()) {
    ...
    for (int j = 0; j < limitPlot_->GetN(); ++j) {
      if (limitPlot_->GetY()[j] > 1.4 * clsTarget || limitPlot_->GetY()[j] < 0.6 * clsTarget)
        continue;
...
bool HybridNew::runTestStatistics(RooWorkspace *w,
  ...
  if (isProfile)
    limit = -limit;  // there's a sign flip for these two
  std::cout << "\n -- Hybrid New -- \n";
...
std::pair<double, double> HybridNew::eval(RooWorkspace *w,
  ...
  for (RooRealVar *rIn = (RooRealVar *)it.Next(); rIn != 0; rIn = (RooRealVar *)it.Next()) {
    ...
    r->setVal(rIn->getVal());
    if (g_verbose)
      std::cout << "  " << r->GetName() << " = " << rIn->getVal() << " +/- " << r->getError() << std::endl;
  }
  std::unique_ptr<RooStats::HybridCalculator> hc(create(w, mc_s, mc_b, data, rVals, setup));
...
std::unique_ptr<RooStats::HybridCalculator> HybridNew::create(RooWorkspace *w,
                                                              RooStats::ModelConfig *mc_s,
                                                              RooStats::ModelConfig *mc_b,
                                                              RooAbsData &data,
                                                              double rVal,
...
std::unique_ptr<RooStats::HybridCalculator> HybridNew::create(RooWorkspace *w,
  ...
  if (poi.getSize() == 1) {  // here things are a bit more convoluted, although they could probably be cleaned up
    ...
    if (testStat_ != "MLZ" && testStat_ != "Profile")
      r->setMax(rVal);
    r->setVal(rVal);
    ...
    else if (testStat_ == "LEP" || testStat_ == "TEV")
  ...
  if (fitNuisances_ &&
      nNonPoiFloatingParameters) {  // We need to fit the model first (to the data) if there are nuisance parameters (constrained or unconstrained)
    TStopwatch timer;
    ...
    std::unique_ptr<RooAbsReal> nll(
        pdfB->createNLL(data, RooFit::Extended(isExt), RooFit::Constrain(*mc_s->GetNuisanceParameters())));
    {
      CloseCoutSentry sentry(g_verbose < 3);
      CascadeMinimizer minim(*nll, CascadeMinimizer::Constrained, r);
      minim.minimize(g_verbose - 3);
    ...
    if (g_verbose > 1) {
      std::cout << "Zero signal fit" << std::endl;
      fitZero.Print("V");
    }
    if (g_verbose > 1) {
      std::cout << "Fitting of the background hypothesis done in " << timer.RealTime() << " s" << std::endl;
    }
    ...
    if (pdfB != mc_s->GetPdf()) {
      ...
      nll.reset(
          mc_s->GetPdf()->createNLL(data, RooFit::Extended(isExt), RooFit::Constrain(*mc_s->GetNuisanceParameters())));
    }
    {
      CloseCoutSentry sentry(g_verbose < 3);
      CascadeMinimizer minim(*nll, CascadeMinimizer::Constrained, r);
    ...
    if (g_verbose > 1) {
      std::cout << "Reference signal fit" << std::endl;
      fitMu.Print("V");
    }
    if (g_verbose > 1) {
      std::cout << "Fitting of the signal-plus-background hypothesis done in " << timer.RealTime() << " s" << std::endl;
    }
    ...
    if (g_verbose > 2) {
      ...
      reportParams.add(*paramsToFit);
      reportParams.add(poi);
  ...
  if (fitNuisances_ && paramsToFit.get()) {
    params.add(*paramsToFit);
    fitMu.writeTo(params);
  }
  if (fitNuisances_ && paramsToFit.get()) {
    paramsZero.addClone(*paramsToFit);
    fitZero.writeTo(paramsZero);
  }
  ...
  if (g_withSystematics && optimizeProductPdf_ && !genGlobalObs_) {
    ...
    if (factorizedPdf_s != mc_s->GetPdf())
      setup.cleanupList.addOwned(*factorizedPdf_s);
    if (factorizedPdf_b != mc_b->GetPdf())
  ...
  if (testStat_ == "LEP") {
    ...
    if (poi.getSize() == 1)
      pdfB = factorizedPdf_s;  // in this case we can remove the arbitrary constant from the test statistics.
    if (optimizeTestStatistics_) {
      ...
      params.snapshot(paramsSnap);  // needs a snapshot
    ...
    if (poi.getSize() == 1)
      pdfB = factorizedPdf_s;  // in this case we can remove the arbitrary constant from the test statistics.
    if (optimizeTestStatistics_) {
      setup.qvar.reset(new ProfiledLikelihoodRatioTestStatOpt(
          *mc_s->GetObservables(), *pdfB, *mc_s->GetPdf(), mc_s->GetNuisanceParameters(), paramsZero, params));
      ((ProfiledLikelihoodRatioTestStatOpt &)*setup.qvar).setPrintLevel(g_verbose);
    } else {
    ...
    if (optimizeTestStatistics_) {
      ...
      if (testStat_ == "LHCFC")
        side = ProfiledLikelihoodTestStatOpt::signFlipDef;
      if (testStat_ == "Profile")
        side = ProfiledLikelihoodTestStatOpt::twoSidedDef;
      if (workingMode_ == MakeSignificance)
        r->setVal(0.0);
      setup.qvar.reset(new ProfiledLikelihoodTestStatOpt(*mc_s->GetObservables(),
                                                         *mc_s->GetPdf(),
    ...
    if (workingMode_ == MakeSignificance)
      r->setVal(0.0);
    setup.qvar.reset(new BestFitSigmaTestStat(
        *mc_s->GetObservables(), *mc_s->GetPdf(), mc_s->GetNuisanceParameters(), params, g_verbose));
  }
  ...
  if (!mc_b->GetPdf()->canBeExtended())
    setup.toymcsampler->SetNEventsPerToy(1);

  if (nCpu_ > 0) {
    std::cerr << "ALERT: running with proof not validated." << std::endl;
    if (g_verbose > 1)
      std::cout << "  Will use " << nCpu_ << " CPUs." << std::endl;
  ...
  if (workingMode_ == MakeSignificance) {
    ...
    if (adaptiveToys_ > 0.) {
      ...
      if ((1. - prob) > maxProbability_)
        nToyssc = 1.;
      else {
        for (int CL_i = 0; CL_i < nCL_; CL_i++) {
          bool isClose = fabs(prob - (1 - scaleAndConfidences[CL_i])) <
                         confidenceToleranceForToyScaling_ * (1 - scaleAndConfidences[CL_i]);
          if (isClose)
            scaleNumberOfToys = 1.;
        }
      }

      nToyssc = (int)nToyssc * scaleNumberOfToys;
      nToyssc = nToyssc > 0 ? nToyssc : 1;
...
std::pair<double, double> HybridNew::eval(RooStats::HybridCalculator &hc,
  ...
  if (g_verbose)
    std::cout << (CLs_ ? "\tCLs = " : "\tCLsplusb = ") << cls.first << " +/- " << cls.second << std::endl;
  if (adaptive) {
    if (CLs_) {
  ...
  if (g_verbose > 0) {
    std::cout << "\tCLs      = " << hcResult->CLs() << " +/- " << hcResult->CLsError() << "\n"
...
std::pair<double, double> HybridNew::eval(const RooStats::HypoTestResult &hcres, double rVal) {
  ...
  if (testStat_ == "LHCFC") {
    RooStats::SamplingDistribution *bDistribution = hcres.GetNullDistribution(),
                                   *sDistribution = hcres.GetAltDistribution();
    ...
    Double_t absdata;
    if (rule_ == "FC") {
      for (int i = 0, n = absbdist.size(); i < n; ++i)
        absbdist[i] = fabs(bdist[i]);
      for (int i = 0, n = abssdist.size(); i < n; ++i)
        abssdist[i] = fabs(sdist[i]);
      absdata = fabs(data) - 2 * minimizerTolerance_;  // needed here since zeros are not exact by more than tolerance
    } else {
      for (int i = 0, n = absbdist.size(); i < n; ++i)
        absbdist[i] = max(0., bdist[i]);
      for (int i = 0, n = abssdist.size(); i < n; ++i)
        abssdist[i] = max(0., sdist[i]);
...
void HybridNew::applyExpectedQuantile(RooStats::HypoTestResult &hcres) {
  if (expectedFromGrid_) {
    if (workingMode_ == MakeSignificance || workingMode_ == MakeSignificanceTestStatistics) {
      ...
      Double_t testStat =
          btoys[std::min<int>(floor((1. - quantileForExpectedFromGrid_) * btoys.size() + 0.5), btoys.size())];
      if (g_verbose > 0)
        std::cout << "Text statistics for " << quantileForExpectedFromGrid_ << " quantile: " << testStat << std::endl;
...
void HybridNew::applyClsQuantile(RooStats::HypoTestResult &hcres) {
  ...
  bcumul.reserve(bdist.size());
  std::vector<std::pair<double, double> > scumul;
  scumul.reserve(sdist.size());
  double btot = 0, stot = 0;
  for (std::vector<Double_t>::const_iterator it = bdist.begin(), ed = bdist.end(), itw = bweight.begin(); it != ed;
       ++it, ++itw) {
    bcumul.push_back(std::pair<double, double>(*it, *itw));
    btot += *itw;
  }
  for (std::vector<Double_t>::const_iterator it = sdist.begin(), ed = sdist.end(), itw = sweight.begin(); it != ed;
       ++it, ++itw) {
  ...
  runningSum = 0;
  for (std::vector<std::pair<double, double> >::reverse_iterator it = scumul.rbegin(), ed = scumul.rend(); it != ed;
       ++it) {
    runningSum += it->second;
    it->second = runningSum * sinv;
  }
  std::sort(bcumul.begin(), bcumul.end());
  std::vector<std::pair<double, std::pair<double, double> > > xcumul;
  xcumul.reserve(bdist.size());
  runningSum = 0;
  std::vector<std::pair<double, double> >::const_iterator sbegin = scumul.begin(), send = scumul.end();
  //int k = 0;
  for (std::vector<std::pair<double, double> >::const_reverse_iterator it = bcumul.rbegin(), ed = bcumul.rend();
       it != ed;
       ++it) {
    runningSum += it->second;
    std::vector<std::pair<double, double> >::const_iterator match =
        std::upper_bound(sbegin, send, std::pair<double, double>(it->first, 0));
    if (match == send) {
  ...
  // get quantile
  runningSum = 0;
  double cut = quantileForExpectedFromGrid_ * btot;
  for (std::vector<std::pair<double, std::pair<double, double> > >::const_iterator it = xcumul.begin(),
                                                                                   ed = xcumul.end();
       it != ed;
       ++it) {
...
RooStats::HypoTestResult *HybridNew::evalWithFork(RooStats::HybridCalculator &hc) {
  ...
  char tmpfile[999];
  snprintf(tmpfile, 998, "%s/rstats-XXXXXX", P_tmpdir);
  ...
  for (ich = 0; ich < fork_; ++ich) {
    ...
    if (!fork())
      break;  // spawn children (but only in the parent thread)
  }
  if (ich == fork_) {  // if i'm the parent
    int cstatus, ret;
    do {
      do {
        ret = waitpid(-1, &cstatus, 0);
      } while (ret == -1 && errno == EINTR);
    } while (ret != -1);
    if (ret == -1 && errno != ECHILD)
      throw std::runtime_error("Didn't wait for child");
    for (ich = 0; ich < fork_; ++ich) {
      TFile *f = TFile::Open(TString::Format("%s.%d.root", tmpfile, ich));
      if (f == 0)
        throw std::runtime_error(TString::Format("Child didn't leave output file %s.%d.root", tmpfile, ich).Data());
      RooStats::HypoTestResult *res = dynamic_cast<RooStats::HypoTestResult *>(f->Get("result"));
      if (res == 0)
        throw std::runtime_error(TString::Format("Child output file %s.%d.root is corrupted", tmpfile, ich).Data());
      if (result.get())
        result->Append(res);
      else
        result.reset(dynamic_cast<RooStats::HypoTestResult *>(res->Clone()));
      f->Close();
    ...
    throw std::runtime_error(
        "done");  // I have to throw instead of exiting, otherwise there's no proper stack unwinding
                  // and deleting of intermediate objects, and when the statics get deleted it crashes
                  // in 5.27.06 (but not in 5.28)
  }
  if (g_verbose > 1) {
    std::cout << "      Evaluation of p-values done in  " << timer.RealTime() << " s" << std::endl;
  }
...
RooStats::HypoTestResult * HybridNew::evalFrequentist(RooStats::HybridCalculator &hc) {
    ...
    *parS = *snapGlobalObs_;
    ...
    if (g_verbose > 2) std::cout << "Test statistics on data: " << tsData << std::endl;
    for (int i = 0; i < toysSB; ++i) {
       ...
       *parS = *hc.GetAlternateModel()->GetSnapshot(); 
       ...
       if (g_verbose > 2) { std::cout << "Generating global obs starting from " << std::endl; parS->Print("V"); }
       std::unique_ptr<RooDataSet> gdata(nuisPdf->generate(gobs, 1));
       *parS = *gdata->get(0);
       if (g_verbose > 2) { std::cout << "Generated global obs" << std::endl; utils::printRAD(&*gdata); }
       // Throw observables
       if (g_verbose > 2) { std::cout << "Generating obs starting from " << std::endl; parS->Print("V"); }
       std::unique_ptr<RooDataSet> data(factorS->generate(obs, RooFit::Extended()));
       if (g_verbose > 2) { std::cout << "Generated obs" << std::endl; utils::printRAD(&*data); }
       // Evaluate T.S.
    ...
    for (int i = 0; i < toysB; ++i) {
       ...
       *parB = *hc.GetNullModel()->GetSnapshot();
       ...
       // Throw global observables, and set them
       if (g_verbose > 2) { std::cout << "Generating global obs starting from " << std::endl; parB->Print("V"); }
       std::unique_ptr<RooDataSet> gdata(nuisPdf->generate(gobs, 1));
       *parB = *gdata->get(0);
       if (g_verbose > 2) { std::cout << "Generated global obs" << std::endl; utils::printRAD(&*gdata); }
       // Throw observables
       if (g_verbose > 2) { std::cout << "Generating obs starting from " << std::endl; parB->Print("V"); }
...
RooStats::HypoTestResult *HybridNew::readToysFromFile(const RooAbsCollection &rVals) {
  if (!g_readToysFromHere)
    throw std::logic_error("Cannot use readHypoTestResult: option toysFile not specified, or input file empty");
  TDirectory *toyDir = g_readToysFromHere->GetDirectory("toys");
  if (!toyDir)
    throw std::logic_error("Cannot use readHypoTestResult: empty toy dir in input file empty");
  if (g_verbose)
    std::cout << "Reading toys for ";
  TString prefix1 = TString::Format("HypoTestResult_mh%g", mass_);
  TString prefix2 = TString::Format("HypoTestResult");
  RooLinkedListIter it = rVals.iterator();
  for (RooRealVar *rIn = (RooRealVar *)it.Next(); rIn != 0; rIn = (RooRealVar *)it.Next()) {
    if (g_verbose)
      std::cout << rIn->GetName() << " = " << rIn->getVal() << "   ";
  ...
  TKey *k;
  while ((k = (TKey *)next()) != 0) {
    if (TString(k->GetName()).Index(prefix1) != 0 && TString(k->GetName()).Index(prefix2) != 0)
      continue;
  ...
  if (g_verbose > 0) {
    std::cout << "\tCLs      = " << ret->CLs() << " +/- " << ret->CLsError() << "\n"
...
void HybridNew::readGrid(TDirectory *toyDir, double rMin, double rMax) {
  if (rValues_.getSize() != 1)
    throw std::runtime_error("Running limits with grid only works in one dimension for the moment");
  clearGrid();

  TIter next(toyDir->GetListOfKeys());
  TKey *k;
  const char *poiName = rValues_.first()->GetName();
  while ((k = (TKey *)next()) != 0) {
    TString name(k->GetName());
    if (name.Index("HypoTestResult_mh") == 0) {
      if (name.Index(TString::Format("HypoTestResult_mh%g_%s", mass_, poiName)) != 0 ||
          name.Index("_", name.Index("_") + 1) == -1)
        continue;
      name.ReplaceAll(TString::Format("HypoTestResult_mh%g_%s", mass_, poiName), "");  // remove the prefix
      if (name.Index("_") == -1)
        continue;                                   // check if it has the _<uniqueId> postfix
    ...
    if (rVal < rMin || rVal > rMax)
      continue;
    if (g_verbose > 2)
      std::cout << "  Do " << k->GetName() << " -> " << name << " --> " << rVal << std::endl;
    RooStats::HypoTestResult *toy = dynamic_cast<RooStats::HypoTestResult *>(toyDir->Get(k->GetName()));
    RooStats::HypoTestResult *&merge = grid_[rVal];
    if (merge == 0)
      merge = new RooStats::HypoTestResult(*toy);
    else
      merge->Append(toy);
    merge->ResetBit(1);
  }
  if (g_verbose > 1) {
    ...
    for (point it = grid_.begin(), ed = grid_.end(); it != ed; ++it) {
      ...
      std::cout << "  - " << it->first << Form("  (%s = ", rule_.c_str()) << clsv << " +/- " << clse << ")"
                << std::endl;
    }
  }
}
void HybridNew::updateGridData(RooWorkspace *w,
  ...
  if (!smart) {
    ...
    std::vector<CLs_t> values;
    values.reserve(grid_.size());
    for (point it = grid_.begin(), ed = grid_.end(); it != ed; ++it) {
      points.push_back(it);
      values.push_back(CLs_t(-99, -99));
    }
    int iMin = 0, iMax = points.size() - 1;
    while (iMax - iMin > 3) {
      if (g_verbose > 1)
        std::cout << "Bisecting range [" << iMin << ", " << iMax << "]" << std::endl;
      int iMid = (iMin + iMax) / 2;
      CLs_t clsMid = values[iMid] = updateGridPoint(w, mc_s, mc_b, data, points[iMid]);
      if (g_verbose > 1)
        std::cout << "    Midpoint " << iMid << " value " << clsMid.first << " +/- " << clsMid.second << std::endl;
      if (clsMid.first - 3 * max(clsMid.second, 0.01) > clsTarget_) {
        if (g_verbose > 1)
          std::cout << "    Replacing Min" << std::endl;
        iMin = iMid;
        continue;
      } else if (clsMid.first + 3 * max(clsMid.second, 0.01) < clsTarget_) {
        if (g_verbose > 1)
          std::cout << "    Replacing Max" << std::endl;
        iMax = iMid;
        continue;
      } else {
        if (g_verbose > 1)
          std::cout << "    Tightening Range" << std::endl;
        while (iMin < iMid - 1) {
          int iLo = (iMin + iMid) / 2;
          CLs_t clsLo = values[iLo] = updateGridPoint(w, mc_s, mc_b, data, points[iLo]);
          if (g_verbose > 1)
            std::cout << "        Lowpoint " << iLo << " value " << clsLo.first << " +/- " << clsLo.second << std::endl;
          if (clsLo.first - 3 * max(clsLo.second, 0.01) > clsTarget_)
            iMin = iLo;
          else
            break;
        }
        while (iMax > iMid + 1) {
          ...
          if (g_verbose > 1)
            std::cout << "        Highpoint " << iHi << " value " << clsHi.first << " +/- " << clsHi.second
                      << std::endl;
          if (clsHi.first + 3 * max(clsHi.second, 0.01) < clsTarget_)
            iMax = iHi;
          else
            break;
        }
    ...
    if (g_verbose > 1)
      std::cout << "Final range [" << iMin << ", " << iMax << "]" << std::endl;
    for (int i = 0; i < iMin; ++i) {
      points[i]->second->SetBit(1);
      updateGridPoint(w, mc_s, mc_b, data, points[i]);
      if (g_verbose > 1)
        std::cout << "  Will not use point " << i << " (r " << points[i]->first << ")" << std::endl;
    }
    for (int i = iMin; i <= iMax; ++i) {
      points[i]->second->ResetBit(1);
      if (values[i].first < -2) {
        if (g_verbose > 1)
          std::cout << "   Updating point " << i << " (r " << points[i]->first << ")" << std::endl;
        updateGridPoint(w, mc_s, mc_b, data, points[i]);
      } else if (g_verbose > 1)
        std::cout << "   Point " << i << " (r " << points[i]->first << ") was already updated during search."
                  << std::endl;
    }
...
void HybridNew::updateGridDataFC(RooWorkspace *w,
                                 ...
                                 double clsTarget_) {
  typedef std::map<double, RooStats::HypoTestResult *>::iterator point;
  std::vector<Double_t> rToUpdate;
  std::vector<point> pointToUpdate;
  double minimizerTolerance_ = ROOT::Math::MinimizerOptions::DefaultTolerance();
  for (point it = grid_.begin(), ed = grid_.end(); it != ed; ++it) {
    it->second->ResetBit(1);
  ...
  if (g_verbose > 0)
    std::cout << "A total of " << rToUpdate.size() << " points will be updated." << std::endl;
  Setup setup;
  ...
  if (g_verbose > 0)
    std::cout << "All points have been updated." << std::endl;
}
...
std::pair<double, double> HybridNew::updateGridPoint(RooWorkspace *w,
  ...
  if (expectedFromGrid_) {
    ...
    RooArgSet nullPOI(*setup.modelConfig_bonly.GetSnapshot());
    if (isProfile)
      nullPOI.setRealValue(r->GetName(), point->first);
    double testStat = setup.qvar->Evaluate(data, nullPOI);
...
void HybridNew::useGrid() {
  ...
  for (std::map<double, RooStats::HypoTestResult *>::iterator itg = grid_.begin(), edg = grid_.end(); itg != edg;
       ++itg, ++i) {
    ...
    if (val.first == -1)
      continue;
    if (val.second == 0 && (val.first != 1 && val.first != 0))
      continue;
    limitPlot_->Set(n + 1);
...
std::vector<std::pair<double, double> > HybridNew::findIntervalsFromSplines(TGraphErrors *limitPlot_,
                                                                            double clsTarget) {
  ...
  for (int pti = 0; pti < npoints_plot - 1; pti++) {
    if (((limitPlot_->GetY()[pti] < clsTarget) && (limitPlot_->GetY()[pti + 1] > clsTarget)) ||
      ...
      for (int tpi = previousCross; tpi <= pti + 1; tpi++) {
        ...
        if (g_verbose) {
          std::cout << " Adding local point to calculate interval boundaries, " << count
  ...
  if (limitPlot_->GetY()[0] > clsTarget)
    points.push_back(std::pair<double, double>(limitPlot_->GetX()[0], 0));
  if (limitPlot_->GetY()[npoints_plot - 1] > clsTarget)
    points.push_back(std::pair<double, double>(limitPlot_->GetX()[npoints_plot - 1], 0));
