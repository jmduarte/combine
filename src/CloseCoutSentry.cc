...
CloseCoutSentry::CloseCoutSentry(bool silent) : silent_(silent), stdOutIsMine_(false) {
  if (silent_) {
    if (open_) {
      open_ = false;
...
CloseCoutSentry::~CloseCoutSentry() { clear(); }
...
void CloseCoutSentry::breakFree() { reallyClear(); }
...
FILE *CloseCoutSentry::trueStdOut() {
  ...
  if (trueStdOut_)
    return trueStdOut_;
  if (owner_ != this && owner_ != 0)
    return owner_->trueStdOut();
  assert(owner_ == this);
  stdOutIsMine_ = true;
  fdOutDup_ =
      dup(fdOut_);  // When clear() calls fclose(trueStdOut_), this makes sure fdOutDup_ gets closed instead of fdOut_
