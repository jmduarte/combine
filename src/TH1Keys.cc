...
TH1Keys::TH1Keys(
    const char *name, const char *title, Int_t nbinsx, Double_t xlow, Double_t xup, TString options, Double_t rho)
...
void TH1Keys::FillH1() const {
  if (dataset_->numEntries() == 0) {
    ...
    if (cache_->Integral())
      cache_->Scale(1.0 / cache_->Integral());
...
void TH1Keys::dont(const char *msg) const {
  TObject::Error("combine/TH1Keys", msg);
