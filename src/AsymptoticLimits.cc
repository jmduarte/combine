  ...
  if (vm.count("singlePoint") && !vm["singlePoint"].defaulted()) {
    ...
    if (what_ != "observed" && what_ != "expected" && what_ != "both" && what_ != "blind")
      throw std::invalid_argument(
          "AsymptoticLimits: option 'run' can only be 'observed', 'expected' or 'both' (the default) or 'blind' "
          "(a-priori expected)");
  }
...
bool AsymptoticLimits::runLimit(RooWorkspace *w,
  ...
  for (int tries = 0; tries < 5; ++tries) {
    ...
    if (strictBounds_ && rMax == r->getMax()) {
      std::cout << rule_ << " at upper bound " << r->GetName() << " = " << r->getVal() << " is " << cls
                << ". Stopping search and using that as a limit.\n"
                << std::endl;
      limit = rMax;
      limitErr = -1.0;
      return true;
...
float AsymptoticLimits::findExpectedLimitFromCrossing(
    RooAbsReal &nll, RooRealVar *r, double rMin, double rMax, double nll0, double clb) {
...
float AsymptoticLimits::calculateLimitFromGrid(RooRealVar *r, double quantile, double alpha) {
  ...
  std::sort(thevals.begin(),
            thevals.end(),
            boost::bind(&std::pair<float, float>::first, _1) < boost::bind(&std::pair<float, float>::first, _2));
