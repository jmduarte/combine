...
void cmsmath::OneDimMinimizer::moveTo(double x) {
  if (x == xmax_) {
    ...
    xi_[1] = x;
    yi_[0] = eval(x);
    xi_[0] = std::max(xmin_, x - dx);
    yi_[0] = eval(xi_[0]);
    xi_[2] = std::min(xmax_, x + dx);
    yi_[2] = eval(xi_[2]);
  }
}
...
bool cmsmath::SequentialMinimizer::SetVariable(unsigned int ivar, const std::string &name, double val, double step) {
  DEBUGV_SM_printf(
      "combine/SequentialMinimizer::SetVariable(idx %u, name %s, val %g, step %g)\n", ivar, name.c_str(), val, step);
  assert(ivar < nDim_);
  func_->x[ivar] = val;
...
bool cmsmath::SequentialMinimizer::Minimize() { return minimize(); }
  ...
  for (int i = 0; i < bigsteps; ++i) {
    ...
    if (newstate == Done) {
      ...
      while (it != doneWorkers.end()) {
        Worker &w = **it;
        if (nFailWakeUpAttempts && w.nUnaffected >= nFailWakeUpAttempts) {
          ++it;
          continue;
        }
    ...
    state_ = newstate;
    if (func_->nCalls > MaxFunctionCalls())
      break;
  }
...
bool cmsmath::SequentialMinimizer::doFullMinim() {
  ...
  for (int i = 0, n = workers_.size(); i < n; ++i) {
    if (workers_[i].state == Active)
      subspaceIndices_.push_back(i);
  }
...
namespace {
  static int load_seqmin() {
    gPluginMgr->AddHandler("ROOT::Math::Minimizer",
                           ...
                           "combine/SequentialMinimizer(const char *)");
    return 1;
  }
