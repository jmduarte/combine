...
RooPiecewisePolynomial::RooPiecewisePolynomial(const char* name,
                                               const char* title,
                                               RooAbsReal& xvar_,
...
double RooPiecewisePolynomial::eval(double x, std::vector<double> const& par) const {
  ...
  if ((nfcn >= 2 && (int)par.size() != 2 * ndof_endfcn + (nfcn - 2) * ndof_middlefcn + nnodes) ||
      (nfcn == 1 && (int)par.size() != polyndof) || nfcn <= 0)
    assert(0);
  ...
  for (int index = 0; index < nfcn; index++) {
    if (index == 0 || index == nfcn - 1)
      npars_reduced[index] = ndof_endfcn;
    else
      npars_reduced[index] = ndof_middlefcn;
  }
  ...
  for (int index = 0; index < nfcn; index++) {
    for (int ipar = 0; ipar < npars_reduced[index]; ipar++) {
      if (!(index == (nfcn - 1) && ipar == (npars_reduced[index] - 1)))
        pars_full[index][ipar] = par[pos_ctr];
      else
        pars_full[index][ipar + 1] =
            par[pos_ctr];  // Special case to avoid singular matrix. This corresponds to having the x^n contribution free instead of x^(n-1)
      pos_ctr++;
    }
  }
  ...
  if (testdet != 0) {
    ...
    for (int index = 0; index < nfcn; index++) {
      for (int ip = npars_reduced[index]; ip < polyndof; ip++) {
        if (!(index == (nfcn - 1) && ip == npars_reduced[index]))
          pars_full[index][ip] = unknowncoeffs[pos_ctr];
        else
          pars_full[index][ip - 1] = unknowncoeffs[pos_ctr];
        pos_ctr++;
      }
    }
    ...
    double res = 0;
    for (int ip = 0; ip < polyndof; ip++)
      res += pars_full[index_chosen][ip] * pow(x, ip);
    return res;
  } else {
    cerr << "combine/RooPiecewisePolynomial::eval: Something went wrong, and the determinant is 0!" << endl;
    return d_epsilon;
  }
}

double RooPiecewisePolynomial::evaluate() const {
  if (parList.getSize() == 0)
    return 0;
  std::vector<double> par;
  par.reserve(parList.getSize());
  for (int ip = 0; ip < parList.getSize(); ip++)
    par.push_back((dynamic_cast<RooAbsReal*>(parList.at(ip)))->getVal());
...
double RooPiecewisePolynomial::evaluate(double* x, double* p) const {  // For calling in a TF1 object
  ...
  //cout << "combine/RooPiecewisePolynomial::evaluate: N parameters = " << psize << endl;
  std::vector<double> par;
  par.reserve(psize);
  for (unsigned int ip = 0; ip < psize; ip++)
    par.push_back(p[ip]);
  //cout << "combine/RooPiecewisePolynomial::evaluate: Calling eval at x=" << x[0] << endl;
  return eval(x[0], par);
}
