...
ClassImp(HZZ4L_RooSpinZeroPdf_phase)
    ...
    HZZ4L_RooSpinZeroPdf_phase::HZZ4L_RooSpinZeroPdf_phase(const char* name,
                                                           const char* title,
                                                           RooAbsReal& _kd,
                                                           RooAbsReal& _kdint,
                                                           RooAbsReal& _ksmd,
...

{
  ...
  while ((func = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "ERROR: :HZZ4L_RooSpinZeroPdf_phase(" << GetName() << ") funcficient " << func->GetName()
...
Int_t HZZ4L_RooSpinZeroPdf_phase::getAnalyticalIntegral(RooArgSet& allVars,
                                                        RooArgSet& analVars,
...
Double_t HZZ4L_RooSpinZeroPdf_phase::analyticalIntegral(Int_t code, const char* rangeName) const {
  switch (code) {
    ...
    case 4: {
      ...
      double integral = (1. - fabs(fai)) * Int_T1 + fabs(fai) * Int_T2 +
                        sqrt((1. - fabs(fai)) * fabs(fai)) * (cos(phi) * Int_T4 + sin(phi) * Int_T5);
