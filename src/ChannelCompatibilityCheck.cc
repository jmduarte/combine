...
bool ChannelCompatibilityCheck::runSpecific(RooWorkspace *w,
  ...
  if (sim == 0)
    throw std::logic_error("Cannot use ChannelCompatibilityCheck if the pdf is not a RooSimultaneous");

  RooAbsCategoryLValue *cat = (RooAbsCategoryLValue *)sim->indexCat().Clone();
  int nbins = cat->numBins((const char *)0);
  TString satname = TString::Format("%s_freeform", sim->GetName());
  std::unique_ptr<RooSimultaneous> newsim((typeid(*sim) == typeid(RooSimultaneousOpt))
                                              ? new RooSimultaneousOpt(satname, "", *cat)
                                              : new RooSimultaneous(satname, "", *cat));
  std::map<std::string, std::string> rs;
  RooArgList minosVars, minosOneVar;
  if (runMinos_)
    minosOneVar.add(*r);
  for (int ic = 0, nc = nbins; ic < nc; ++ic) {
    ...
    TString riName =
        TString::Format("_ChannelCompatibilityCheck_%s_%s", r->GetName(), nameForLabel(cat->getLabel()).c_str());
    ...
    if (runMinos_ && !minosVars.find(riName))
  ...
  if (result_nominal.get() == 0)
    return false;
  if (result_freeform.get() == 0)
    return false;

  //double nll_nominal   = result_nominal->minNll();
  //double nll_freeform = result_freeform->minNll();
  if (fabs(nll_nominal) > 1e10 || fabs(nll_freeform) > 1e10)
    return false;
  ...
  if (fixedMu_) {
    ...
    if (runMinos_ && do95_) {
      printf("Nominal fit  : %s = %7.4f  %+6.4f/%+6.4f (68%% CL)\n",
             r->GetName(),
             rNominal->getVal(),
             rNominal->getAsymErrorLo(),
             rNominal->getAsymErrorHi());
      printf("               %s = %7.4f  %+6.4f/%+6.4f (95%% CL)\n",
             r->GetName(),
             rNominal->getVal(),
             rNominal->getMin("err95") - rNominal->getVal(),
             rNominal->getMax("err95") - rNominal->getVal());
    } else if (runMinos_) {
  ...
  for (std::map<std::string, std::string>::const_iterator it = rs.begin(), ed = rs.end(); it != ed; ++it) {
    ...
    if (runMinos_ && do95_) {
      ...
      printf("               %s = %7.4f  %+6.4f/%+6.4f (95%% CL) in channel %s\n",
             r->GetName(),
             ri->getVal(),
             ri->getMin("err95") - ri->getVal(),
             ri->getMax("err95") - ri->getVal(),
             it->first.c_str());
    } else if (runMinos_) {
