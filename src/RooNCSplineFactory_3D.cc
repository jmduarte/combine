...
RooNCSplineFactory_3D::RooNCSplineFactory_3D(RooAbsReal& XVar_,
                                             RooAbsReal& YVar_,
                                             RooAbsReal& ZVar_,
                                             TString appendName_,
    ...
    : appendName(appendName_),
      ...
      fcn(0),
      PDF(0) {}
RooNCSplineFactory_3D::~RooNCSplineFactory_3D() { destroyPDF(); }
...
const std::vector<splineQuadruplet_t> RooNCSplineFactory_3D::getPoints(const std::vector<RooNCSplineCore::T>& XList,
  ...
  if (nX * nY * nZ != n) {
    cerr << "RooNCSplineFactory_3D::getPoints: nX=" << nX << " x nY=" << nY << " x nZ=" << nZ << " != nFcn=" << n
         << endl;
    assert(0);
  }

  std::vector<splineQuadruplet_t> pList;
  pList.reserve(n);
...
void RooNCSplineFactory_3D::destroyPDF() {
  delete PDF;
  PDF = 0;
  delete fcn;
  fcn = 0;
...
void RooNCSplineFactory_3D::initPDF(const std::vector<splineQuadruplet_t>& pList) {
  ...
  TString title = name;
  fcn = new RooNCSpline_3D_fast(name.Data(),
                                title.Data(),
                                *XVar,
                                *YVar,
                                *ZVar,
...
void RooNCSplineFactory_3D::setPoints(TTree* tree) {
  ...
  for (int ip = 0; ip < n; ip++) {
    tree->GetEntry(ip);
    pList.push_back(splineQuadruplet_t(x, y, z, fcn));
  }
  setPoints(pList);
}

void RooNCSplineFactory_3D::setEndConditions(RooNCSplineCore::BoundaryCondition const bcBegin,
