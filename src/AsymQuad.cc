...
AsymQuad::AsymQuad(const char* name,
                   const char* title,
  ...
  if (inFuncList.getSize() != 2 * inCoefList.getSize() + 1) {
    coutE(InputArguments) << "combine/AsymQuad::AsymQuad(" << GetName()
                          << ") number of functions and coefficients inconsistent, must have Nfunc=1+2*Ncoef"
                          << std::endl;
    assert(0);
  }
  ...
  while ((func = (RooAbsArg*)funcIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "ERROR: AsymQuad::AsymQuad(" << GetName() << ") function  " << func->GetName()
  ...
  while ((coef = (RooAbsArg*)coefIter->Next())) {
    if (!dynamic_cast<RooAbsReal*>(coef)) {
      coutE(InputArguments) << "ERROR: AsymQuad::AsymQuad(" << GetName() << ") coefficient " << coef->GetName()
                            << " is not of type RooAbsReal" << std::endl;
      assert(0);
    }
    _coefList.add(*coef);
