#include "combine/FastTemplate_Old.h"
...
void FastTemplate::CopyValues(const TH1 &other) {
  for (unsigned int i = 0; i < size_; ++i)
    values_[i] = other.GetBinContent(i + 1);
}
...
FastHisto::FastHisto(const TH1 &hist) : FastTemplate(hist), binEdges_(size() + 1), binWidths_(size()) {
...
FastHisto::FastHisto(const FastHisto &other)
    : FastTemplate(other), binEdges_(other.binEdges_), binWidths_(other.binWidths_) {}

int FastHisto::FindBin(const T &x) const {
  auto match = std::lower_bound(binEdges_.begin(), binEdges_.end(), x);
  if (match == binEdges_.begin())
    return -1;
  if (match == binEdges_.end())
    return values_.size();
...
FastHisto::T FastHisto::GetAt(const T &x) const {
  auto match = std::lower_bound(binEdges_.begin(), binEdges_.end(), x);
  if (match == binEdges_.begin() || match == binEdges_.end())
    return T(0.0);
...
FastHisto::T FastHisto::GetMax() const { return *std::max(values_.begin(), values_.end()); }
...
FastHisto2D::T FastHisto2D::GetAt(const T &x, const T &y) const {
  ...
  if (matchx == binEdgesX_.begin() || matchx == binEdgesX_.end())
    return T(0.0);
  int ix = (matchx - binEdgesX_.begin() - 1);
  auto matchy = std::lower_bound(binEdgesY_.begin(), binEdgesY_.end(), y);
  if (matchy == binEdgesY_.begin() || matchy == binEdgesY_.end())
    return T(0.0);
...
void FastHisto2D::Dump() const {
  ...
  for (unsigned int i = 0; i < size_; ++i) {
    printf(" bin %3d, x = %6.2f, y = %6.2f: yval = %9.5f, width = %6.3f\n",
           i,
           0.5 * (binEdgesX_[i / binY_] + binEdgesX_[i / binY_ + 1]),
...
FastHisto2D::T FastHisto2D::GetMaxOnXY() const { return *std::max(values_.begin(), values_.end()); }
...
FastHisto3D::T FastHisto3D::GetAt(const T &x, const T &y, const T &z) const {
  ...
  if (matchz == binEdgesZ_.begin() || matchz == binEdgesZ_.end())
    return T(0.0);
...
void FastHisto3D::Dump() const {
  ...
  for (unsigned int i = 0; i < size_; ++i) {
    printf(" bin %3d, x = %6.2f, y = %6.2f, z = %6.2f : zval = %9.5f, width = %6.3f\n",
           i,
           0.5 * (binEdgesX_[i / (binY_ * binZ_)] + binEdgesX_[i / (binY_ * binZ_) + 1]),
...
namespace {
  ...
  void sumdiff(FastTemplate::T *__restrict__ sum,
               FastTemplate::T *__restrict__ diff,
               unsigned int n,
               const FastTemplate::T *__restrict__ h1,
               const FastTemplate::T *__restrict__ h2) {
  ...
  void meld(FastTemplate::T *__restrict__ out,
            unsigned int n,
            FastTemplate::T const *__restrict__ diff,
            FastTemplate::T const *__restrict__ sum,
            FastTemplate::T x,
            FastTemplate::T y) {
    for (unsigned int i = 0; i < n; ++i) {
...
void FastTemplate::Subtract(const FastTemplate &ref) { subtract(&values_[0], size_, &ref[0]); }
void FastTemplate::LogRatio(const FastTemplate &ref) { logratio(&values_[0], size_, &ref[0]); }
void FastTemplate::SumDiff(const FastTemplate &h1, const FastTemplate &h2, FastTemplate &sum, FastTemplate &diff) {
