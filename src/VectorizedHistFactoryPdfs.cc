...
void cacheutils::VectorizedHistFunc::fill() {
...
const std::vector<Double_t> &cacheutils::VectorizedHistFunc::eval(const RooAbsData &data) {
  if (data_ != &data) {
    data_ = &data;
    fill();
  }
...
cacheutils::VectorizedParamHistFunc::VectorizedParamHistFunc(const ParamHistFunc &pdf,
...
void cacheutils::VectorizedParamHistFunc::fill(std::vector<Double_t> &out) const {
...
namespace {
  ...
  public:
    PiecewiseInterpolationWithAccessor(const PiecewiseInterpolation &p) : PiecewiseInterpolation(p) {}
...
cacheutils::CachingPiecewiseInterpolation::CachingPiecewiseInterpolation(const PiecewiseInterpolation &pdf,
                                                                         const RooArgSet &obs)
    : pdf_(&pdf) {
...
void cacheutils::CachingPiecewiseInterpolation::setDataDirty() {
  ...
  for (CachingPdfBase &pdf : cachingPdfsHi_)
    pdf.setDataDirty();
  for (CachingPdfBase &pdf : cachingPdfsLow_)
    pdf.setDataDirty();
}
