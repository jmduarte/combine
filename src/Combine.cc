...
Combine::Combine()
  ...
  statOptions_.add_options()(
      "systematics,S",
      po::value<bool>(&withSystematics)->default_value(true),
      "Include constrained systematic uncertainties, -S 0 will ignore systematics constraint terms in the datacard.")(
      ...
      "'1/sqrt(r)'")("significance", "Compute significance instead of upper limit (works only for some methods)")(
      ...
      "toysFrequentist",
      "Generate all toys in the frequentist way. Note that this affects the toys generated by option '-t' that happen "
      "in the main loop, not the ones within the Hybrid(New) algorithm.")(
      "expectSignal",
      po::value<float>(&expectSignal_)->default_value(0.),
      ...
      "setParameters",
      po::value<string>(&setPhysicsModelParameterExpression_)->default_value(""),
      ...
      "setParameterRanges",
      po::value<string>(&setPhysicsModelParameterRangeExpression_)->default_value(""),
      ...
      "CV=0.0,2.0:CF=0.0,5.0")("defineBackgroundOnlyModelParameters",
                               po::value<string>(&defineBackgroundOnlyModelParameterExpression_)->default_value(""),
...
void Combine::applyOptions(const boost::program_options::variables_map &vm) {
  ...
  if (!withSystematics)
    toysNoSystematics_ = true;  // if no systematics, also don't expect them for the toys
  toysFrequentist_ = vm.count("toysFrequentist");
  if (toysNoSystematics_ && toysFrequentist_)
    throw std::logic_error("You can't set toysNoSystematics and toysFrequentist options at the same time");
  ...
  if (method == "combine/MultiDimFit" || (method == "combine/FitDiagnostics" && vm.count("justFit")) || method == "combine/MarkovChainMC") {
    //CMSDAS new default,
...
void Combine::run(
  ...
  if (hlfFile.EndsWith(".hlf")) {
    ...
    if (compiledExpr_)
      options += " --compiled ";
    if (verbose > 1)
      options += TString::Format(" --verbose %d", verbose - 1);
    if (algo->name() == "combine/FitDiagnostics" || algo->name() == "combine/MultiDimFit")
      options += " --for-fits";
  ...
  if (isBinary) {
    ...
    if (MH != 0) {
      if (verbose > 2)
        std::cerr << "Setting variable 'MH' in workspace to the higgs mass " << mass_ << std::endl;
      MH->setVal(mass_);
    }
  ...
  if (floatAllNuisances_ && mc->GetNuisanceParameters() && withSystematics)
  ...
  if (floatAllNuisances_ && mc_bonly && mc_bonly->GetNuisanceParameters() && withSystematics)
  ...
  RooAbsPdf *genPdf = (expectSignal_ > 0 || setPhysicsModelParameterExpression_ != "" || !mc_bonly)
                          ? mc->GetPdf()
  ...
  if (nToys <= 0) {  // observed or asimov
        ...
        if (toysFrequentist_ && mc->GetGlobalObservables()) {
          ...
          if (!snap) {
            std::cerr << "Snapshot of global observables toy_asimov_snapshot not found in "
                      << readToysFromHere->GetName() << ". List follows:\n";
            readToysFromHere->ls();
            return;
          }
    ...
    if (saveToys_) {
      ...
      if (toysFrequentist_ && mc->GetGlobalObservables()) {
        ...
        if (snap)
          writeToysHere->WriteTObject(snap, "toy_asimov_snapshot");
      }
    }
  ...
  if (nToys > 0) {
    ...
    if (nFloatingNonPoiParameters && !toysNoSystematics_ && (readToysFromHere == 0)) {
      ...
      nuisancePdf.reset(utils::makeNuisancePdf(
          expectSignal_ || setPhysicsModelParameterExpression_ != "" || noMCbonly_ ? *mc : *mc_bonly));
      if (toysFrequentist_) {
	...
	*/
        ...
        if (toysFrequentist_ && mc->GetGlobalObservables()) {
          RooAbsCollection *snap =
              dynamic_cast<RooAbsCollection *>(readToysFromHere->Get(TString::Format("toys/toy_%d_snapshot", iToy)));
          if (!snap) {
    ...
    if (nLimits > 1) {
      ...
      cout << "mean   expected limit: r < " << expLimit << " +/- " << rms << " @ " << cl * 100 << "%CL (" << nLimits
           << " toyMC)" << endl;
    } else {
    ...
    if (nLimits > 0) {
      ...
      cout << "median expected limit: r < " << medianLimit << " @ " << cl * 100 << "%CL (" << nLimits << " toyMC)"
           << endl;
...
void Combine::toggleGlobalFillTree(bool flag) { g_fillTree_ = flag; }
