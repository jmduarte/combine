...
//_____________________________________________________________________________
RooRealFlooredSumPdf::RooRealFlooredSumPdf(
    const char* name, const char* title, const RooArgList& inFuncList, const RooArgList& inCoefList, Bool_t extended)
    : RooAbsPdf(name, title),
      _normIntMgr(this, 10),
      _haveLastCoef(kFALSE),
      _funcList("!funcList", "List of functions", this),
  ...
  if (!(inFuncList.getSize() == inCoefList.getSize() + 1 || inFuncList.getSize() == inCoefList.getSize())) {
    coutE(InputArguments) << "combine/RooRealFlooredSumPdf::RooRealFlooredSumPdf(" << GetName()
  ...
  while ((coef = (RooAbsArg*)coefIter->Next())) {
    ...
    if (!dynamic_cast<RooAbsReal*>(coef)) {
      coutW(InputArguments) << "combine/RooRealFlooredSumPdf::RooRealFlooredSumPdf(" << GetName() << ") coefficient "
                            << coef->GetName() << " is not of type RooAbsReal, ignored" << endl;
      continue;
    }
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutW(InputArguments) << "combine/RooRealFlooredSumPdf::RooRealFlooredSumPdf(" << GetName() << ") func "
                            << func->GetName() << " is not of type RooAbsReal, ignored" << endl;
      continue;
    }
    _funcList.add(*func);
    _coefList.add(*coef);
  ...
  if (func) {
    if (!dynamic_cast<RooAbsReal*>(func)) {
      coutE(InputArguments) << "combine/RooRealFlooredSumPdf::RooRealFlooredSumPdf(" << GetName() << ") last func "
                            << coef->GetName() << " is not of type RooAbsReal, fatal error" << endl;
      assert(0);
    }
    _funcList.add(*func);
  } else
...
//_____________________________________________________________________________
void RooRealFlooredSumPdf::setFloor(Double_t val) { _floorVal = val; }
...
Double_t RooRealFlooredSumPdf::evaluate() const {
  ...
  while ((coef = (RooAbsReal*)coefIter.next())) {
    ...
    if (coefVal) {
      cxcoutD(Eval) << "combine/RooRealFlooredSumPdf::eval(" << GetName() << ") coefVal = " << coefVal
  ...
  if (!_haveLastCoef) {
    ...
    cxcoutD(Eval) << "combine/RooRealFlooredSumPdf::eval(" << GetName() << ") lastCoef = " << lastCoef
                  << " funcVal = " << func->getVal() << endl;

    // Warn about coefficient degeneration
    if (lastCoef < 0 || lastCoef > 1) {
      coutW(Eval) << "combine/RooRealFlooredSumPdf::evaluate(" << GetName()
...
//_____________________________________________________________________________
Int_t RooRealFlooredSumPdf::getAnalyticalIntegralWN(RooArgSet& allVars,
                                                    RooArgSet& analVars,
                                                    const RooArgSet* normSet2,
                                                    const char* rangeName) const {
  //cout << "combine/RooRealFlooredSumPdf::getAnalyticalIntegralWN:"<<GetName()<<"("<<allVars<<",analVars,"<<(normSet2?*normSet2:RooArgSet())<<","<<(rangeName?rangeName:"<none>") << endl;
  ...
  if (cache) {
    //cout << "combine/RooRealFlooredSumPdf("<<this<<")::getAnalyticalIntegralWN:"<<GetName()<<"("<<allVars<<","<<analVars<<","<<(normSet2?*normSet2:RooArgSet())<<","<<(rangeName?rangeName:"<none>") << " -> " << _normIntMgr.lastIndex()+1 << " (cached)" << endl;
  ...
  //cout << "combine/RooRealFlooredSumPdf("<<this<<")::getAnalyticalIntegralWN:"<<GetName()<<"("<<allVars<<","<<analVars<<","<<(normSet2?*normSet2:RooArgSet())<<","<<(rangeName?rangeName:"<none>") << " -> " << code+1 << endl;
  return code + 1;
}

//_____________________________________________________________________________
Double_t RooRealFlooredSumPdf::analyticalIntegralWN(Int_t code,
                                                    const RooArgSet* normSet2,
                                                    const char* rangeName) const {
  //cout << "combine/RooRealFlooredSumPdf::analyticalIntegralWN:"<<GetName()<<"("<<code<<","<<(normSet2?*normSet2:RooArgSet())<<","<<(rangeName?rangeName:"<none>") << endl;
  ...
  if (cache == 0) {  // revive the (sterilized) cache
    //cout << "combine/RooRealFlooredSumPdf("<<this<<")::analyticalIntegralWN:"<<GetName()<<"("<<code<<","<<(normSet2?*normSet2:RooArgSet())<<","<<(rangeName?rangeName:"<none>") << ": reviving cache "<< endl;
  ...
  if (!_haveLastCoef) {
    ...
    // Warn about coefficient degeneration
    if (lastCoef < 0 || lastCoef > 1) {
      coutW(Eval) << "combine/RooRealFlooredSumPdf::integral(" << GetName()
                  << ") WARNING: Sum of FUNC coefficients not in range [0-1], value=" << 1 - lastCoef << endl;
    }
  }
  ...
  Double_t result = 0;
  if (normVal > 0)
    result = value / normVal;
  if (result <= 0. && _doFloor) {
    coutW(Eval) << "combine/RooRealFlooredSumPdf::integral(" << GetName() << ") WARNING: Integral " << result
