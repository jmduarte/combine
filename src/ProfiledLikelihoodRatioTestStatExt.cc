...
#define DBG_TestStat_params \
  0  // ProfiledLikelihoodRatioTestStatOpt::Evaluate; 1 = dump nlls; 2 = dump params at each eval
...
//============================================================================================================================================
ProfiledLikelihoodRatioTestStatOpt::ProfiledLikelihoodRatioTestStatOpt(const RooArgSet &observables,
                                                                       RooAbsPdf &pdfNull,
                                                                       RooAbsPdf &pdfAlt,
                                                                       const RooArgSet *nuisances,
                                                                       const RooArgSet &paramsNull,
  ...
  DBG(DBG_TestStat_params, (snapAlt_.Print("V"))) if (nuisances) {
...
Double_t ProfiledLikelihoodRatioTestStatOpt::Evaluate(RooAbsData &data, RooArgSet &nullPOI) {
  RooArgSet initialStateAlt;
  paramsAlt_->snapshot(initialStateAlt);
  RooArgSet initialStateNull;
  paramsNull_->snapshot(initialStateNull);
...
double ProfiledLikelihoodRatioTestStatOpt::minNLL(std::unique_ptr<RooAbsReal> &nll_) {
  ...
  if (verbosity_ > 0)
    std::cout << "Profiling likelihood for pdf " << pdf.GetName() << std::endl;
...
ProfiledLikelihoodTestStatOpt::ProfiledLikelihoodTestStatOpt(const RooArgSet &observables,
    ...
    : pdf_(&pdf), gobsParams_(gobsParams), gobs_(gobs), verbosity_(verbosity), oneSided_(oneSided) {
  ...
  DBG(DBG_PLTestStat_ctor, (std::cout << "POI: " << std::endl))
  ...
  for (RooAbsArg *a = (RooAbsArg *)it.Next(); a != 0; a = (RooAbsArg *)it.Next()) {
    ...
    if (pp == 0) {
      std::cerr << "WARNING: NLL does not depend on POI " << a->GetName() << ", cannot profile" << std::endl;
      continue;
    }
    if (ps == 0) {
      std::cerr << "WARNING: no snapshot for POI " << a->GetName() << ", cannot profile" << std::endl;
      continue;
    }
    poi_.add(*ps);
    poiParams_.add(*pp);
...
Double_t ProfiledLikelihoodTestStatOpt::Evaluate(RooAbsData &data, RooArgSet & /*nullPOI*/) {
  ...
  RooArgSet initialState;
  params_->snapshot(initialState);

  DBG(DBG_PLTestStat_pars,
      std::cout << "Being evaluated on " << data.GetName() << ": params before snapshot are " << std::endl)
  ...
  *params_ = snap_;
  ...
  DBG(DBG_PLTestStat_pars,
      std::cout << "Being evaluated on " << data.GetName() << ": params after snapshot are " << std::endl)
  DBG(DBG_PLTestStat_pars, params_->Print("V"))
  ...
  if (poi_.getSize() == 1) {
    ...
    if (oneSided_ == oneSidedDef)
      r->setMin(0);
    if (oneSided_ != twoSidedDef) {
      if (initialR == 0 || (oneSided_ != oneSidedDef))
        r->removeMax();
      else
        r->setMax(1.1 * initialR);
    }
    r->setVal(initialR == 0 ? (std::isnormal(oldMax) && fabs(oldMax) < 1e28 ? 0.1 * oldMax : 0.5)
                            : 0.5 * initialR);  //best guess
    r->setConstant(false);
  } else {
  ...
  DBG(DBG_PLTestStat_pars, std::cout << std::endl)
  DBG(DBG_PLTestStat_pars,
      std::cout << "Was evaluated on " << data.GetName() << ": params before snapshot are " << std::endl)
  DBG(DBG_PLTestStat_pars, params_->Print("V"))
  ...
  if (initialR == 0 || oneSided_ != oneSidedDef || bestFitR < initialR) {
    ...
    if (do_debug)
      std::cout << "NLL shift from unconstrained fit before re-profiling: " << nll_->getVal() - nullNLL << std::endl;
    ...
    if (initialR == 0) {  // NOTE: signs are flipped for the zero case!
      if (oneSided_ == signFlipDef && bestFitR < initialR) {
        DBG(DBG_PLTestStat_main,
            (printf("   fitted signal %7.4f is negative, discovery test statistics will be negative.\n", bestFitR)))
        std::swap(thisNLL, nullNLL);
      }
    } else if (bestFitR > initialR && oneSided_ == signFlipDef) {
      DBG(DBG_PLTestStat_main,
          (printf("   fitted signal %7.4f > %7.4f, test statistics will be negative.\n", bestFitR, initialR)))
      std::swap(thisNLL, nullNLL);
    }
  } else {
    DBG(DBG_PLTestStat_main,
...
std::vector<Double_t> ProfiledLikelihoodTestStatOpt::Evaluate(RooAbsData &data,
  ...
  *params_ = snap_;
  ...
  double bestFitR = r->getVal();
  // Take snapshot of initial state, to restore it at the end
  RooArgSet bestFitState;
  params_->snapshot(bestFitState);
  ...
  for (int iR = 0, nR = rVals.size(); iR < nR; ++iR) {
    if (fabs(ret[iR]) > 10 * EPS)
      continue;  // don't bother re-update points which were too far from zero anyway.
...
bool nllutils::robustMinimize(RooAbsReal &nll, RooMinimizer &minim, int verbosity, bool zeroPoint) {
  ...
  for (int tries = 0, maxtries = 4; tries <= maxtries; ++tries) {
    ...
    if (status == 0 && nll.getVal() > initialNll + 0.02) {
      ...
      DBG(DBG_PLTestStat_main,
          (printf("\n  --> false minimum, status %d, cov. quality %d, edm %10.7f, nll initial % 10.4f, nll final % "
                  "10.4f, change %10.5f\n",
                  status,
                  res->covQual(),
                  res->edm(),
                  initialNll,
                  nll.getVal(),
                  initialNll - nll.getVal())))
      if (pars.get() == 0)
        pars.reset(nll.getParameters((const RooArgSet *)0));
      *pars = res->floatParsInit();
      ...
      DBG(DBG_PLTestStat_main,
          (printf("\n  --> partial fail: status %d, cov. quality %d, edm %10.7f, nll initial % 10.4f, nll final % "
                  "10.4f, change %10.5f\n",
                  status,
                  res->covQual(),
                  res->edm(),
      ...
      std::unique_ptr<RooFitResult> res(do_debug ? minim.save() : 0);
      DBG(DBG_PLTestStat_main,
          (printf("\n  --> final fail: status %d, cov. quality %d, edm %10.7f, nll initial % 10.4f, nll final % 10.4f, "
                  "change %10.5f\n",
                  status,
                  res->covQual(),
                  res->edm(),
