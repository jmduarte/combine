...
ClassImp(RooNCSpline_1D_fast)
    ...
    : RooNCSplineCore(), bcBeginX(RooNCSplineCore::bcNaturalSpline), bcEndX(RooNCSplineCore::bcNaturalSpline) {}

RooNCSpline_1D_fast::RooNCSpline_1D_fast(const char* name, const char* title)
    : RooNCSplineCore(name, title),
      bcBeginX(RooNCSplineCore::bcNaturalSpline),
      bcEndX(RooNCSplineCore::bcNaturalSpline) {}

RooNCSpline_1D_fast::RooNCSpline_1D_fast(const char* name,
                                         const char* title,
                                         RooAbsReal& inXVar,
  ...
  if (npointsX() > 1) {
    ...
    if (det == 0.) {
      coutE(InputArguments) << "RooNCSpline_1D::interpolateFcn: Matrix xA could not be inverted. Something is wrong "
                               "with the x coordinates of points!"
                            << endl;
      assert(0);
    }
...
RooNCSpline_1D_fast::RooNCSpline_1D_fast(const RooNCSpline_1D_fast& other, const char* name)
...
RooNCSplineCore::T RooNCSpline_1D_fast::interpolateFcn(Int_t code, const char* rangeName) const {
  ...
  if (verbosity == RooNCSplineCore::kVerbose)
    cout << "RooNCSpline_1D_fast(" << GetName() << ")::interpolateFcn begin with code: " << code << endl;
  ...
  if (code == 0 || code % 2 != 0) {  // Case to just compute the value at x
    ...
    cropValueForRange(coordmin);
    RooNCSplineCore::T coordmax = theXVar.max(rangeName);
    cropValueForRange(coordmax);
...
RooNCSplineCore::T RooNCSpline_1D_fast::getTVar(const vector<RooNCSplineCore::T>& kappas,
...
Double_t RooNCSpline_1D_fast::evaluate() const {
  ...
  if (useFloor && value < floorEval) {
    if (verbosity >= RooNCSplineCore::kError)
      coutE(Eval) << "RooNCSpline_1D_fast ERROR::RooNCSpline_1D_fast(" << GetName() << ") evaluation returned " << value
                  << " at x = " << theXVar << endl;
    value = floorEval;
  }
  if (verbosity == RooNCSplineCore::kVerbose) {
...
Int_t RooNCSpline_1D_fast::getAnalyticalIntegral(RooArgSet& allVars,
                                                 RooArgSet& analVars,
...
Double_t RooNCSpline_1D_fast::analyticalIntegral(Int_t code, const char* rangeName) const {
  ...
  if (useFloor && value < floorInt) {
    if (verbosity >= RooNCSplineCore::kError)
      coutE(Integration) << "RooNCSpline_1D_fast ERROR::RooNCSpline_1D_fast(" << GetName() << ") integration returned "
                         << value << " for code = " << code << endl;
    value = floorInt;
  }
  if (verbosity == RooNCSplineCore::kVerbose) {
    cout << "RooNCSpline_1D_fast(" << GetName() << ")::analyticalIntegral = " << value << " for code = " << code
         << endl;
  }
  return value;
}
